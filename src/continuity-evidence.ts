...
export function appendExchangeEvidence(db: Database.Database, exchangeId: string): number {
  ...
  if (db.prepare(`SELECT 1 FROM workstream_evidence
    ...
    .get(row.workstream_id, exchangeId, row.content_generation)) return 0;
  ...
  `).all(exchangeId) as Array<Record<string, unknown>>;
  const base = {
  ...
  if (JSON.stringify(full).length <= 8_000) {
    ...
    const split = (text: string, make: (chunk: string, offset: number) => Record<string, unknown>) => {
  ...
  for (const [part, payload] of parts.entries()) {
    insert.run(row.workstream_id, exchangeId, row.session_id, row.workspace_id,
      row.content_generation, row.content_hash, part,
      JSON.stringify({ ...payload, part, parts: parts.length }), new Date().toISOString());
  }
  return parts.length;
}
...
export function appendSessionEvidence(
  ...
  const candidates = db.prepare(SESSION_EVIDENCE_CANDIDATES).all(sessionId) as Array<{ id: string }>;
  if (candidates.length === 0) return;
  db.transaction(() => {
    options.onTransactionStart?.();
    for (const candidate of candidates) appendExchangeEvidence(db, candidate.id);
  }).immediate();
}
...
export function readCapsulePage(db: Database.Database, checkpointId: string): CapsulePage {
  ...
  if (targetSeq === null || state.target_revision !== state.revision) {
    ...
    `).get(state.workstream_id) as { n: number }).n);
  ...
  if (!rows.length) throughSeq = Math.max(throughSeq, targetSeq);
  return { fromSeq: state.through_seq, throughSeq, targetSeq, revision: state.revision, evidence };
}

export function commitCapsulePage(db: Database.Database, workstreamId: string, page: CapsulePage): boolean {
 ...
 */
export function capsulePageHintAtFloor(db: Database.Database, checkpointId: string): boolean {
  const row = db.prepare(
    "SELECT page_items_hint, page_chars_hint FROM capsule_checkpoint_state WHERE checkpoint_id = ?",
  ).get(checkpointId) as { page_items_hint: number | null; page_chars_hint: number | null } | undefined;
  if (!row || row.page_items_hint === null || row.page_chars_hint === null) return false;
