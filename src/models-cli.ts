...
if (!MODELS_SUBCOMMANDS.has(subcommand)) {
  console.error(
    `Unknown 'memex models' subcommand: ${subcommand}\n` +
...
function envReport(): Record<string, string | null> {
  return {
...
function catalogReport(catalog: CodexCatalog): {
  source: CodexCatalog['source'];
  path: string | null;
  fetchedAt: string | null;
  codexHome: string;
  models: Array<{ slug: string; visible: boolean; reasoningEfforts: string[] }>;
} {
  return {
...
function embeddingReport(): {
  ...
  model: string;
  source: 'env' | 'default';
  cache: { present: boolean; files: number; bytes: number; modelDir: string; stub: boolean };
} {
...
interface DbFacts {
  ...
  exists: boolean;
  holds: HoldRow[];
  heldJobs: Array<{ reason: string; jobs: number; oldestHeldAt: string | null }>;
 ...
 */
async function readDbFacts(fingerprint: string): Promise<DbFacts> {
  const dbPath = getDbPath();
  const empty: DbFacts = { dbPath, exists: false, holds: [], heldJobs: [], lastProbe: null };
  ...
  try {
    ...
    const ledgerColumns = tableExists
      ? new Set(
          (db.prepare('PRAGMA table_info(model_work_attempts)').all() as Array<{ name: string }>).map((c) => c.name),
        )
      : new Set<string>();
    const ledgerMigrated = ['model', 'reasoning_effort', 'error_class', 'duration_ms'].every((c) =>
      ledgerColumns.has(c),
    );
...
function formatHold(hold: HoldRow, indent: string): string[] {
  const lines: string[] = [];
  lines.push(
    `${hold.current ? 'HELD' : 'held (another selection)'} — the provider rejected the request envelope`,
  );
  const status = `${hold.status ?? '?'} ${hold.providerType ?? 'provider error'}`;
  lines.push(`${indent}${status}: "${hold.providerMessage}"`);
  lines.push(
    `${indent}model ${hold.model}${hold.reasoningEffort ? ` / ${hold.reasoningEffort}` : ''} · ` +
      `first seen ${hold.heldAt} · observed ${hold.observedCount}x`,
  );
  if (hold.current) {
    lines.push(
      `${indent}Model work is paused. No job was failed and no attempt was consumed.`,
    );
...
async function runShow(): Promise<void> {
  ...
  lines.push(
    `  ${pad('reasoning')}${selection.reasoning ?? '(no flag)'}   (${sourceLabel(selection.reasoningSource)})`,
  );
  lines.push(`  ${pad('default')}${DEFAULT_LLM_MODEL} / (no reasoning flag)`);
  if (catalog.source === 'none') {
    lines.push(
      `  ${pad('catalog')}not found in this Codex installation (${codexHome()})`,
    );
    lines.push(
      `  ${' '.repeat(12)}any id is accepted and verified by the first call: memex models test`,
    );
  } else {
    const listable = listableModels(catalog);
    lines.push(
      `  ${pad('catalog')}${catalog.models.length} model(s) — ${catalog.path}` +
  ...
  lines.push('');
  lines.push(
    `Settings file  ${modelSettingsPath()}` +
  ...
  lines.push(
    `Environment    ${Object.entries(envReport())
 ...
 */
async function settleAfterSelectionChange(
  previousFingerprint: string,
  nextFingerprint: string,
): Promise<{ clearedHolds: number; releasedJobs: number } | null> {
...
async function runSet(): Promise<void> {
  ...
  if (modelArg !== undefined && !isValidModelId(modelArg)) {
    refuse(
      `--model must be 1-256 characters matching [\\w./:@+-]. (got ${JSON.stringify(modelArg)})`,
    );
  }
  let reasoning: ReasoningEffort | null | undefined;
  if (reasoningArg !== undefined) {
    ...
    if (reasoningArg === 'unset') {
      ...
      if (!normalized) {
        refuse(
          `--reasoning must be one of ${ALLOWED_REASONING_EFFORTS.join('|')}` +
  ...
  if (modelArg !== undefined) {
    if (catalog.source === 'none') {
      notes.push(
        'this Codex installation publishes no model catalog — the id is verified by the first call',
      );
    } else if (!catalogModel) {
      notes.push(
        `${modelArg} is not in this installation's catalog (${catalog.path}) — saved anyway; ` +
  ...
  if (savedReasoning && effectiveLevels) {
    if (effectiveLevels.includes(savedReasoning)) {
      ...
      notes.push(
        `warning: the catalog says ${checkedModel} supports ${effectiveLevels.join('/')}, ` +
  ...
  if (after.modelSource === 'env' && modelArg !== undefined) {
    notes.push(
      `${LLM_MODEL_ENV}=${process.env[LLM_MODEL_ENV]} is set and takes precedence — ` +
  ...
  if (after.reasoningSource === 'env' && reasoning !== undefined) {
    notes.push(
      `${LLM_REASONING_ENV}=${process.env[LLM_REASONING_ENV]} is set and takes precedence — ` +
        `this environment calls with reasoning ${after.reasoning ?? '(no flag)'}`,
    );
  }
  notes.push(
    'background workers apply this from their next Codex session; a running MCP server from its next model call',
  );
  if (after.reasoning && ['high', 'xhigh', 'max', 'ultra'].includes(after.reasoning)) {
    notes.push(
      `reasoning ${after.reasoning} raises per-call latency — if extraction starts hitting the run ` +
...
async function runReset(): Promise<void> {
  ...
  lines.push(
    `  LLM  ${after.model} / ${after.reasoning ?? '(no reasoning flag)'}   (${sourceLabel(after.modelSource)})`,
  );
  if (after.modelSource === 'env' || after.reasoningSource === 'env') {
    lines.push(
      `  note: ${LLM_MODEL_ENV}/${LLM_REASONING_ENV} is still set in this environment and wins over the file.`,
    );
  }
  if (settled && (settled.clearedHolds > 0 || settled.releasedJobs > 0)) {
    lines.push(
      `  released ${settled.clearedHolds} configuration hold(s) and ${settled.releasedJobs} waiting job(s).`,
    );
  }
  lines.push(
    `The effective EMBEDDING model is unchanged (${embedding.model}) — the database owns it, ` +
...
async function runTest(): Promise<void> {
  ...
  if (timeoutArg !== undefined) {
    if (!/^\d+$/.test(timeoutArg)) refuse('--timeout-ms must be a non-negative integer');
  ...
  if (result.ok) {
    ...
    if (result.clearedHold) {
      lines.push(
        `cleared ${result.clearedHold.clearedHolds} configuration hold(s) and released ` +
  ...
  if (result.rejection) {
    ...
    lines.push(
      `  ${result.rejection.status ?? '?'} ${result.rejection.type ?? 'provider error'} — ${result.rejection.message}`,
    );
    lines.push(
      'The model-work hold stays in place. Fix the selection and run again:',
      '  memex models set --model <id> [--reasoning <level>]',
      '  memex models test',
    );
  } else {
...
try {
  if (subcommand === 'show') await runShow();
  else if (subcommand === 'set') await runSet();
  else if (subcommand === 'reset') await runReset();
  else await runTest();
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
  if (json) console.log(JSON.stringify({ ok: false, error: message }, null, 2));
  else console.error(`memex models ${subcommand}: ${message}`);
  process.exitCode = 1;
}
