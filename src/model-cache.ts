...
export const EMBEDDING_MODEL =
  process.env.MEMEX_EMBEDDING_MODEL || DEFAULT_EMBEDDING_MODEL;
...
const LEGACY_CACHE_RELATIVE = path.join(
  "node_modules",
  "@xenova",
  "transformers",
  ".cache",
...
export function embeddingCacheSource(): EmbeddingCacheSource {
...
export function embeddingModelCacheDir(
 ...
 */
export function inspectModelCacheDir(modelDir: string): {
  present: boolean;
  files: number;
  bytes: number;
} {
  ...
  walk(modelDir, (file, size) => {
    files++;
    bytes += size;
    if (file.toLowerCase().endsWith(".onnx") && size > 0) weights = true;
  });
  const config = (() => {
    try {
...
export function embeddingCacheStatus(
  model = EMBEDDING_MODEL,
...
export function legacyEmbeddingCacheCandidates(options: {
  ...
  const roots: Array<{ root: string; kind: LegacyCacheCandidate["kind"] }> = [
  ...
  const found: LegacyCacheCandidate[] = [];
  const seen = new Set<string>();
  for (const { root, kind } of roots) {
    const cacheDir = path.join(root, LEGACY_CACHE_RELATIVE);
    if (seen.has(cacheDir)) continue;
    seen.add(cacheDir);
    try {
      if (!fs.statSync(cacheDir).isDirectory()) continue;
    } catch {
      continue;
    }
...
export function adoptLegacyEmbeddingCache(options: {
  ...
  const none = (reason: LegacyCacheAdoption["reason"]): LegacyCacheAdoption => ({
...
export function ensureEmbeddingCacheDir(): string {
  const dir = embeddingCacheDir();
  try {
