...
export function initDatabase(
  options: {
     ...
     */
    onSchemaMigration?: (outcome: { ran: boolean; skipped: string[] }) => void;
  } = {},
): Database.Database {
...
export function applySchemaMigrations(
  ...
  const dbPath = options.dbPath ?? getDbPath();
  fs.mkdirSync(path.dirname(dbPath), { recursive: true });
  let outcome = { ran: false, skipped: [] as string[] };
...
export const ROW_NORMALIZATION_INVARIANTS: RowNormalizationInvariant[] = [
  {
  ...
  },
  {
    ...
    pendingSql:
      "SELECT COUNT(*) AS n FROM facts WHERE semantic_generation < 1 OR lifecycle_generation < 1",
  ...
  },
  {
    ...
    pendingSql:
      "SELECT COUNT(*) AS n FROM exchanges WHERE project_id IS NULL AND project <> '' AND project <> 'unknown'",
  },
  {
...
function runSchemaMigrations(db: Database.Database): string[] {
  ...
  const factColumns = new Set(
    (
      db.prepare("PRAGMA table_info(facts)").all() as Array<{ name: string }>
    ).map((r) => r.name),
  );
  ...
  const ontologyCategoryColumns = new Set(
    (
      db.prepare("PRAGMA table_info(ontology_categories)").all() as Array<{
        name: string;
      }>
    ).map((row) => row.name),
  );
  ...
  const mergeTaxonomyDuplicates = db.transaction(() => {
    ...
    const domainKeeper = new Map<string, string>();
    ...
    const categories = db
      ...
      .all() as Array<{ id: string; domain_id: string; name: string; created_at: string }>;
    const categoryKeeper = new Map<string, string>();
 ...
 * tree. */
function isLlmWorkdirEvidencePath(canonical: string | null): boolean {
...
function isCompositeShellCommand(command: string): boolean {
  ...
  for (let i = 0; i < command.length; i++) {
    ...
    if (ch === "|" || ch === ";" || ch === "&" || ch === "\n" || ch === "\r")
      return true;
    if (ch === ">" || ch === "<") return true; // 리다이렉션도 출력 신뢰성을 깬다
  }
  return false;
}
...
function renameExchangeWithin(
  ...
  const factRows = db
    .prepare(
      "SELECT id, source_exchange_ids FROM facts WHERE source_exchange_ids LIKE ?",
    )
    .all(`%${oldId}%`) as Array<{ id: string; source_exchange_ids: string }>;
  for (const fact of factRows) {
    try {
      const ids: unknown = JSON.parse(fact.source_exchange_ids);
      if (Array.isArray(ids) && ids.includes(oldId)) {
        db.prepare("UPDATE facts SET source_exchange_ids = ? WHERE id = ?").run(
          JSON.stringify([...new Set(ids.map((i) => (i === oldId ? newId : i)))]),
          fact.id,
        );
      }
    } catch {
  ...
  db.prepare(
    "UPDATE fact_revisions SET source_exchange_id = ? WHERE source_exchange_id = ?",
  ).run(newId, oldId);
  ...
  const vec = db
    ...
    .get(oldId) as { embedding: unknown } | undefined;
  if (vec) {
    const dt = getVecTableDtype(db, "vec_exchanges");
    db.prepare("DELETE FROM vec_exchanges WHERE id = ?").run(oldId);
    db.prepare(
      `INSERT INTO vec_exchanges (id, embedding) VALUES (?, ${vecParamSql(dt)})`,
    ).run(newId, embeddingToVecBlob(vec.embedding as number[], dt));
  }
  db.prepare("UPDATE exchanges SET id = ? WHERE id = ?").run(newId, oldId);
  db.prepare("UPDATE tool_calls SET exchange_id = ? WHERE exchange_id = ?").run(
    newId,
    oldId,
  );
}
 ...
 */
export function reconcileArchiveExchanges(
  db: Database.Database,
  input: { archivePath: string; desired: Array<{ id: string; lineStart: number }> },
): ReconcileArchiveResult {
  const desiredByLine = new Map(
    input.desired.map((d) => [d.lineStart, d.id] as const),
  );
  const rows = db
    .prepare(
      "SELECT id, line_start, last_indexed FROM exchanges WHERE archive_path = ?",
    )
    .all(input.archivePath) as Array<{
    id: string;
    line_start: number;
    last_indexed: number;
  }>;
  const byLine = new Map<number, Array<{ id: string; last_indexed: number }>>();
  ...
  for (const [lineStart, group] of byLine) {
    ...
    if (desiredId === undefined) {
      for (const row of group) deletions.push(row.id);
      continue;
    }
    ...
    if (exact) {
      ...
      for (const row of group) {
        if (row.id !== desiredId) deletions.push(row.id);
      }
      continue;
    }
    ...
    for (const row of group) {
      if (row.id !== newest.id) deletions.push(row.id);
    }
  }
  ...
  const tx = db.transaction(() => {
    ...
    for (const [oldId, newId] of renames) renameExchangeWithin(db, oldId, newId);
    for (const id of deletions) {
      ...
      const factRows = db
        ...
        .all(`%${id}%`) as Array<{ id: string; source_exchange_ids: string }>;
      for (const fact of factRows) {
        try {
          const ids: unknown = JSON.parse(fact.source_exchange_ids);
          if (Array.isArray(ids) && ids.includes(id)) {
            const next = (ids as string[]).filter((i) => i !== id);
            db.prepare("UPDATE facts SET source_exchange_ids = ? WHERE id = ?").run(
      ...
      db.prepare(
        "UPDATE fact_revisions SET source_exchange_id = NULL WHERE source_exchange_id = ?",
      ).run(id);
