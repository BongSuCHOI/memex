...
if (subcommand !== null) {
  if (!SYNC_SUBCOMMANDS.has(subcommand)) {
    console.error(
      `Unknown 'memex sync' subcommand: ${subcommand}\n` +
  ...
  const {
    exportGenerationArchive, formatSyncStatus, getSyncStatus, importArchive, previewImportArchive,
    runSyncExport, runSyncImport, setDeviceAlias, setSyncEnabled,
  } = await import('./sync-control.js');
  const emit = (payload: unknown, text: string): void => {
  ...
  try {
    if (subcommand === 'alias') {
      ...
      if (!name && !args.includes('--clear')) throw new Error('alias needs a name (or --clear)');
      setDeviceAlias(deviceId, args.includes('--clear') ? null : (name as string));
      ...
      if (dirIndex >= 0 && !dir) throw new Error('--dir needs a path');
      ...
      emit(
        archive,
        `sync archive written: ${archive.path}\n` +
      ...
      if (!source) throw new Error('--archive needs the path of a generation zip or directory');
      if (args.includes('--dry-run')) {
        const preview = previewImportArchive(source);
        emit(
          preview,
          `sync archive preview: device ${preview.deviceId}${preview.deviceAlias ? ` "${preview.deviceAlias}"` : ''} ` +
        ...
        emit(
          outcome,
          `sync archive imported: device ${outcome.deviceId}${outcome.deviceAlias ? ` "${outcome.deviceAlias}"` : ''} ` +
      ...
      if (outcome.skipped === 'disabled') {
        ...
        emit(
          outcome,
          `sync import: facts +${r.newFacts}/~${r.updatedFacts}/-${r.deletedFacts}, ` +
            `+${r.newRevisions} revisions, +${r.newTombstones} tombstones, ` +
            `+${r.newRecallEvents}/~${r.updatedRecallEvents} recall events` +
            (r.malformedRows.length
              ? `\n${r.malformedRows.map((issue) => `  rejected ${issue.file}:${issue.line} — ${issue.error}`).join('\n')}`
