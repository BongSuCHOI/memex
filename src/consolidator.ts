...
import { assertMutationPolicy, captureMutationPolicy, captureSourceSnapshot, consolidationEligibility, consolidationSnapshotValid, hasLocalMeaningEvidence,
  type SourceSnapshot } from './fact-policy.js';
...
async function consolidateOne(
  ...
  if (newFact.scope_type === 'global') scope = { type: 'global' };
  ...
  if (!sources) return { called: false, verdict: 'none' };
...
export async function applyConsolidationResult(
  ...
  const preserveReason = (reason: string): false => {
  ...
  if (!['DUPLICATE', 'CONTRADICTION', 'EVOLUTION', 'INDEPENDENT'].includes(result.relation)) return preserveReason('invalid model verdict');
  if (result.relation === 'INDEPENDENT') return false;
  ...
  if (!sources) return preserveReason('source evidence is missing');
  ...
  switch (result.relation) {
    case 'DUPLICATE': {
      ...
      const apply = db.transaction((): boolean => {
        ...
        const liveSources = [...new Set([existingFact.id, newFact.id].flatMap(id => {
          const row = db.prepare('SELECT source_exchange_ids FROM facts WHERE id = ?').get(id) as { source_exchange_ids: string };
          const ids: unknown = JSON.parse(row.source_exchange_ids);
          if (!Array.isArray(ids) || !ids.every(value => typeof value === 'string')) throw new Error('invalid participant lineage');
    ...
    case 'CONTRADICTION':
    case 'EVOLUTION': {
      if (result.same_subject !== true || result.same_conditions !== true) return preserveReason('subject or applicability is unconfirmed');
      if (!hasLocalMeaningEvidence(db, newFact)) return preserveReason('incoming meaning has no current local verification');
