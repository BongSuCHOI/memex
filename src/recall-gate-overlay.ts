...
const INTENTS: readonly GateIntent[] = [
...
function emptyWords(): LoadedRecallGateOverlay["words"] {
  return { add: { ack: [], continue: [], filler: [] }, disable: { ack: [], continue: [], filler: [] } };
}

const EMPTY_OVERLAY: LoadedRecallGateOverlay = Object.freeze({
  present: false,
  hash: null,
  revision: 0,
...
export function emptyRecallGateOverlay(): LoadedRecallGateOverlay {
...
const BUILTIN_SOURCES = new Set(
  BUILTIN_GATE_PATTERNS.map((pattern) => `${pattern.source}\u0000${pattern.flags}`),
);
 ...
 */
export function validateRecallGateOverlayDoc(
  raw: unknown,
  opts: { forWrite?: boolean; bytes?: number } = {},
): ValidationResult {
  const issues: Issue[] = [];
  const error = (code: string, message: string, extra?: Parameters<typeof overlayIssue>[3]) => {
    issues.push(overlayIssue("error", code, message, extra));
  };
  const warn = (code: string, message: string, extra?: Parameters<typeof overlayIssue>[3]) => {
  ...
  const add: UserGatePattern[] = [];
  const disable: string[] = [];
  const seenIds = new Set<string>();
  const perIntent = new Map<GateIntent, number>();
  ...
  if (patternsRaw !== undefined && (typeof patternsRaw !== "object" || patternsRaw === null || Array.isArray(patternsRaw))) {
    ...
    const section = (patternsRaw ?? {}) as Record<string, unknown>;
    ...
    if (rawAdd !== undefined && !Array.isArray(rawAdd)) {
      ...
      list.forEach((entry, index) => {
        ...
        const intent = item.intent as GateIntent;
    ...
    if (rawDisable !== undefined && !Array.isArray(rawDisable)) {
      ...
      const list = (rawDisable ?? []) as unknown[];
  ...
  if (wordsRaw !== undefined && (typeof wordsRaw !== "object" || wordsRaw === null || Array.isArray(wordsRaw))) {
    ...
    const section = (wordsRaw ?? {}) as Record<string, unknown>;
    for (const side of ["add", "disable"] as const) {
      const sideRaw = section[side];
      if (sideRaw === undefined) continue;
      ...
      for (const [lexicon, value] of Object.entries(sideRaw as Record<string, unknown>)) {
        if (!LEXICONS.includes(lexicon as GateLexicon)) {
        ...
        value.forEach((word, index) => {
          ...
          words[side][lexicon as GateLexicon].push(word);
        });
      }
    }
  }
...
export async function validateRecallGateOverlay(
  doc: unknown,
...
let cache: { key: string; loaded: LoadedRecallGateOverlay } | null = null;
...
function readValidateCompile(file: string): LoadedRecallGateOverlay {
  ...
  const patterns: UserPatternSpec[] = [];
  ...
  for (const pattern of doc.patterns?.add ?? []) {
    ...
    const row = quarantine.find(
      (entry) => entry.pattern_id === pattern.id && entry.source_sha8 === sha8,
    );
    if (row) {
 ...
 */
export function loadRecallGateOverlay(): LoadedRecallGateOverlay {
...
export function currentRecallGateRevision(): number {
...
export function resetRecallGateOverlayCache(): void {
  cache = null;
}

/* -------------------------------------------------------------------------- */
/* Catalogue + gate plumbing                                                   */
/* -------------------------------------------------------------------------- */
 ...
 */
export function toUserIntentHits(
  overlay: LoadedRecallGateOverlay,
  hits: { intents: Partial<Record<GateIntent, string[]>> },
): UserIntentHits | undefined {
  const hasWords = LEXICONS.some(
    (lexicon) =>
      (overlay.words.add?.[lexicon]?.length ?? 0) > 0 ||
      (overlay.words.disable?.[lexicon]?.length ?? 0) > 0,
  );
  const hasIntents = Object.values(hits.intents).some((ids) => (ids?.length ?? 0) > 0);
  if (!hasWords && !hasIntents && overlay.disabled.length === 0) return undefined;
  return {
...
/* -------------------------------------------------------------------------- */
/* Dry run (§2.7) — `memex gate test` / the Web UI `test` action               */
/* -------------------------------------------------------------------------- */
...
export interface RecallExplanation {
  prompt: { chars: number; tokens: string[] };
  ...
  diffCause?: Array<{ id: string; source: string; intent: GateIntent }>;
  stateSource: "neutral" | "session";
}

export function neutralGateState(): RecallGateState {
  return {
    contextEpoch: 0,
...
function sourceOfId(id: string, overlay: LoadedRecallGateOverlay): string {
  const user = overlay.doc?.patterns?.add?.find((pattern) => pattern.id === id);
  if (user) return user.source;
...
export async function explainRecall(
  ...
  const intents = {} as RecallExplanation["intents"];
  ...
  let diffCause: RecallExplanation["diffCause"];
  if (builtinExplained) {
    diffCause = [];
    for (const intent of INTENTS) {
 ...
 */
export async function recallGateOverlayChecks(
  matcherProbe?: () => Promise<boolean>,
  ...
  if (overlaysDisabled()) {
    ...
    checks.push({
      ...
      status: "fail",
      detail:
        `unreadable/invalid — running on BUILT-IN DEFAULTS: ${errors.map((issue) => issue.code).join(", ")}` +
  ...
  checks.push(
    quarantined.length === 0
      ...
      : {
          ...
          status: "fail",
          detail:
            `${quarantined.length} pattern(s) quarantined after exceeding the 50 ms match budget — ` +
    ...
    checks.push(
      available
        ...
        : {
            ...
            status: "fail",
            detail:
              "matcher worker unavailable — gate patterns are not applied (fail-safe); " +
...
async function defaultMatcherProbe(overlay: LoadedRecallGateOverlay): Promise<boolean> {
