...
export interface BackfillWorkStatus {
  ...
  active: { total: number; extract: number };
  unresolved: {
    total: number;
    extract: number;
...
export function getBackfillWorkStatus(
  opts: { dbPath?: string } = {},
): BackfillWorkStatus {
  const dbPath = opts.dbPath ?? getDbPath();
  const empty: BackfillWorkStatus = {
    total: 0,
  ...
  try {
    ...
    if (pendingExtraction && tableExists(db, "memory_jobs")) {
      const row = db
        ...
        .get(new Date().toISOString(), ...pendingExtraction.params) as {
        pending: number;
        active: number;
      };
      activeExtractionSessions = Number(row.active);
      extractionSessions = Math.max(
        0,
        Number(row.pending) - activeExtractionSessions,
      );
    }
    ...
    const legacyPermanentSessions = tableExists(db, "extraction_log")
      ? count(
          db,
          `SELECT COUNT(*) AS n FROM extraction_log l
           WHERE l.extracted = ?
             ${pendingExtraction
               ? `AND NOT EXISTS (
    ...
    if (
      tableExists(db, "ontology_categories") &&
      tableExists(db, "vec_categories_rowids")
    ) {
      const pending = buildCategoryReembedPending(EMBEDDING_VERSION);
      categoryVectors = count(
        db,
        `SELECT COUNT(*) AS n FROM ontology_categories c WHERE ${pending.clause}`,
        ...pending.params,
      );
    }
    ...
    if (tableExists(db, "facts")) {
      if (tableExists(db, "vec_facts_rowids")) {
        ...
        factVectors = count(
          db,
          `SELECT COUNT(*) AS n FROM facts f WHERE ${pending.clause}`,
          ...pending.params,
        );
      } else {
        factVectors = count(
          db,
          "SELECT COUNT(*) AS n FROM facts WHERE is_active = 1",
        );
      }
    ...
    if (
      tableExists(db, "exchanges") &&
      tableExists(db, "vec_exchanges_rowids")
    ) {
      const pending = buildReembedPending(EMBEDDING_VERSION);
      exchangeVectors = count(
        db,
        `SELECT COUNT(*) AS n FROM exchanges e WHERE ${pending.clause}`,
        ...pending.params,
      );
    }
    ...
    const stages = {
      ...
      embeddings:
        categoryVectors + factVectors + koreanFactVectors + exchangeVectors,
    };
    const status = {
