 ...
 */
export function classifyTextLanguage(text: string | null | undefined): {
  language: ExtractionLanguage | null;
  hangul: number;
  latin: number;
  koScore: number;
  ...
  if (counts.hangul === 0 && counts.latin === 0) return { language: null, ...shape };
  if (koScore === enScore) return { language: null, ...shape };
