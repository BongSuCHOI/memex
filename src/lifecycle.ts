...
async function injectDaemonCheck(): Promise<Check> {
  ...
  if (!probe.owner) {
    return {
      name, status: "warn",
      detail:
        `a listener holds the socket but did not identify itself (${probe.problem ?? "no answer"}). ` +
  ...
  return {
    name, status: "warn",
    detail:
      `the socket is owned by a DIFFERENT build, so the fast path is refused and every prompt falls back ` +
