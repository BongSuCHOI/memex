...
import { listCaptureGapMarkers } from "./capture-gap-markers.js";
...
import { extractionRulesChecks, type HeldJobCount } from "./extraction-rules.js";
import {
...
async function injectDaemonCheck(): Promise<Check> {
  ...
  if (!probe.owner && (probe.code === "EPERM" || probe.code === "EACCES")) {
    return {
      name, status: "warn",
      detail:
        `not reachable from this process — connect() was denied (${probe.code}). This is what a ` +
...
export function captureGapCheck(): Check {
  ...
  let markers: ReturnType<typeof listCaptureGapMarkers>;
  try {
    markers = listCaptureGapMarkers();
  } catch {
  ...
  const lines = markers.slice(0, CAPTURE_GAP_DETAIL_LIMIT).map(({ marker }) =>
    `capture skipped at ${marker.event} ${marker.ts} (${
      marker.transcriptBytes ?? 0} uncaptured bytes); ${CAPTURE_GAP_LOSS_STATEMENT}`,
  );
  const more = markers.length - lines.length;
  return {
    name,
    status: "warn",
    detail:
      `${markers.length} skipped capture(s), oldest ${markers[0].marker.ts} — ` +
 ...
 */
function workerLockHolderLine(): string {
  try {
...
export function hookLatencyCheck(now: number = Date.now()): Check {
  ...
  if (killed.length > 0) {
    const markers = (() => {
      try {
        return listCaptureGapMarkers().length;
      } catch {
        return 0;
      }
    })();
  ...
  return {
    name,
    status: "ok",
    detail: `${paired} hook run(s) completed, max ${maxDuration} ms` + holder,
  };
}
 ...
 */
function readHeldJobCounts(): HeldJobCount[] {
  try {
    const dbPath = getDbPath();
    ...
    try {
      const hasTable =
        db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name = 'memory_jobs'").get() !==
        undefined;
      if (!hasTable) return [];
      const columns = new Set(
        (db.prepare("PRAGMA table_info(memory_jobs)").all() as Array<{ name: string }>).map(
          (row: { name: string }) => row.name,
        ),
      );
      if (!columns.has("hold_reason")) return [];
      return (db.prepare(`
        SELECT hold_reason AS reason, COUNT(*) AS jobs FROM memory_jobs
        WHERE hold_reason IS NOT NULL AND state NOT IN ('completed','superseded','dead')
        GROUP BY hold_reason ORDER BY hold_reason
      `).all() as Array<{ reason: string; jobs: number }>).map((row) => ({
...
export async function doctor(): Promise<DoctorReport> {
  ...
  try {
    for (const check of extractionRulesChecks(readHeldJobCounts())) checks.push(check);
  } catch {
    checks.push({
