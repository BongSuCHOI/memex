 ...
 * continuity tail was pending #163, for thirty days. Four classes, four
 * statements, and only `capture` and `epoch` are worth a warning — #168 split the
 * transcript-less capture event out of `capture` for the same reason.
 */
type CaptureGapClass = "capture" | "epoch" | "no-transcript" | "neutral";
...
function captureGapMarkerClass(marker: CaptureGapMarker): CaptureGapClass {
  if (CAPTURE_MARKER_EVENTS.has(marker.event)) {
    ...
    if (!marker.transcriptPath && !marker.transcriptBytes) return "no-transcript";
    return "capture";
  }
