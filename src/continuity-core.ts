...
export function capsuleMaxChars(): number {
  const raw = process.env.MEMEX_CAPSULE_MAX_CHARS;
  if (raw === undefined) return DEFAULT_MAX_CAPSULE_CHARS;
  const text = String(raw).trim();
  if (!/^\d+$/.test(text)) return DEFAULT_MAX_CAPSULE_CHARS;
...
export function scheduleCapsuleForCheckpoint(
  ...
  options: { onTransactionStart?: () => void } = {},
): void {
 ...
 */
export function scheduleCapsuleBacklog(
  db: Database.Database,
  options: {
  ...
  for (const stream of streams) {
    ...
    const run = (markStart: () => void) =>
      scheduleCapsuleForCheckpoint(db, checkpointId, undefined, force, {
        onTransactionStart: markStart,
      });
    index++;
    if (options.timeTransaction) options.timeTransaction(`scheduleCapsuleBacklog#${index}`, run);
    else run(() => { /* untimed */ });
  }
}
...
interface TruncationLedger {
  fields: string[];
  itemCaps: Record<string, CapsuleItemCap>;
  /** Issue #178: original length of each scalar the 500-char bound clamped. */
  scalarClamps: Record<string, number>;
}
 ...
 */
function noteScalarClamp(ledger: TruncationLedger, field: string, originalLength: number): void {
...
function fitCapsulePatch(
  ...
  const done = (finalChars: number, overBudget: boolean): CapsuleTruncation => ({
  ...
  if (originalChars <= max) return done(originalChars, false);
  ...
  for (const limit of [4, 2, 1, 0]) {
    for (const field of ["touchedAreas", "openQuestions", "nextActions", "hypotheses"] as const) {
  ...
  for (const field of ["currentState", "objective"] as const) {
    if (size() <= max) break;
  ...
  for (const limit of [4, 2, 1]) shrinkList("verifiedProgress", limit);
  if (size() > max) {
  ...
  while (size() > max && (patch.objective.length > SCALAR_TRUNCATION_FLOOR ||
    ...
    for (const field of ["currentState", "objective"] as const) {
      if (patch[field].length <= SCALAR_TRUNCATION_FLOOR) continue;
      patch[field] = patch[field].slice(
        0,
        Math.max(SCALAR_TRUNCATION_FLOOR, Math.ceil(patch[field].length / 2)),
      );
      note(field);
    }
  }
 ...
 * records the detail issue #85 needs — which fields lost content, how many
 * items each list kept and dropped, and whether the projection is still over
 * budget (issue #74). Rows written before 0.6.3 hold a bare array of field
 * names, which `parseTruncationRecord` still reads.
 */
...
function parseTruncationRecord(raw: unknown): TruncationRecord {
  const empty: TruncationRecord = { fields: [], itemCaps: {}, scalarClamps: {}, overBudget: false };
  ...
  const scalarClamps: Record<string, number> = {};
  if (record.scalarClamps && typeof record.scalarClamps === "object" && !Array.isArray(record.scalarClamps)) {
    for (const [field, value] of Object.entries(record.scalarClamps as Record<string, unknown>)) {
...
interface ParsedWorkCapsulePatch {
  ...
  /**
   * Issue #178: original length of each scalar the 500-character bound clamped
   * while parsing, recorded against the final patch beside `itemCapDrops`.
   */
  scalarClamps: Record<string, number>;
}
 ...
 */
function codePointSafeSlice(text: string, end: number): string {
 ...
 * bound.
 */
function clampCapsuleScalar(raw: string): string {
  const head = codePointSafeSlice(raw, MAX_CAPSULE_SCALAR_CHARS);
  let boundary = -1;
  for (let i = head.length - 1; i >= 0; i--) {
    if (/\s/.test(head[i])) { boundary = i; break; }
  }
...
function parseWorkCapsulePatchStructure(value: unknown): ParsedWorkCapsulePatch {
  ...
  const strictScalar = (field: "objective" | "currentState"): string => {
    ...
    if (raw.length <= MAX_CAPSULE_SCALAR_CHARS) return raw.trim();
...
function flattenContextData(value: unknown): string {
  return String(value ?? "")
    .replace(/[\u0000-\u001f\u007f]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
 ...
 */
function contextScalar(value: unknown, max = 500): string {
...
export function buildDeterministicTailBaton(
  ...
  if (latestUser) lines.push(`Request: ${contextScalar(latestUser)}`);
  if (plan) lines.push(`Next: ${contextScalar(plan, 300)}`);
...
export function handleContinuityHook(
   ...
   */
  const finalize = (deliveryError?: unknown) => {
    if (finalized) return;
    if (deliveryError) {
   ...
   * and reported `ok`.
   */
  const enterPhase = (write: boolean) => {
    const left = deadlineAt - Date.now();
    if (left < HOOK_PHASE_FLOOR_MS) {
      throw new HookDeadlineExceeded(
        `hook budget exhausted before the next phase (${Math.max(0, left)} ms left)`,
      );
    }
    if (!write) return;
    try {
  ...
  try {
    ...
    if (outcome) {
      if (strictCapture && kind) throw cause;
      return { stdout: "", warning: `continuity hook skipped (${outcome})` };
    }
    if (captureFailure) {
      if (strictCapture) throw cause;
      return { stdout: "", warning: error.message };
    }
    throw cause;
  } finally {
...
function runContinuityHook(
...
): HandleHookResult {
  {
    ...
    if (payload.hookEventName === "SessionStart") {
      ...
      options.enterPhase(true);
      const recoveryWarning = options.measureAttempt(() =>
        recoverContinuitySession(db, payload.sessionId));
