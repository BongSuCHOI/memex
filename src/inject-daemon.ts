...
import { persistentMatcher, type MatcherHandle } from './overlay-matcher.js';
...
export function startInjectDaemon(): net.Server | null {
   ...
   * shared worker.
   */
  let matcher: MatcherHandle | null = null;
  const sharedMatcher = (): MatcherHandle => (matcher ??= persistentMatcher());
   ...
   */
  const unreadableLocksSeen = new Set<string>();
   ...
   */
  function recordBindFailure(error: unknown, when: string): void {
    const errno = (error as NodeJS.ErrnoException | null)?.code ?? null;
