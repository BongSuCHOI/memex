...
interface WatchLedgerEntry {
  key: string;
  epoch: number;
  /** informative prompt counter value at emission */
  at: number;
...
export async function computeInjectContext(
  ...
  try {
    ...
    const corrections: Array<{ text: string; revision: ResidentFactRevision }> = [];
    let dedupedCount = 0;
    ...
    if (residentById.size > 0 && canQuery(db)) {
      ...
      `).all(...residentById.keys()) as Array<{ id: string; fact: string; semantic_generation: number; lifecycle_generation: number }>;
    ...
    const wantsWorkNow = !!capsule && (
      decision.triggers.includes("capsule_generation_changed") ||
    ...
    if ((decision.intents.trace || decision.intents.memory) && canQuery(db)) {
      const traceItems: Array<{ text: string }> = [];
