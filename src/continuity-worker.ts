...
export function workerTransactionLogPath(): string {
...
function recordWorkerTransaction(label: string, waitMs: number, heldMs: number): boolean {
  ...
  try {
    ...
    fs.appendFileSync(
      file,
      `${JSON.stringify({ ts: new Date().toISOString(), pid: process.pid, label, wait_ms, held_ms })}\n`,
    );
    return true;
  } catch {
...
export function timeWorkerTransaction<T>(label: string, run: (markStart: () => void) => T): T {
  ...
  try {
    ...
    if (startedAt !== null) recordWorkerTransaction(label, startedAt - calledAt, performance.now() - startedAt);
    return value;
  } catch (error) {
    const finishedAt = performance.now();
    if (startedAt === null) recordWorkerTransaction(label, finishedAt - calledAt, 0);
    else recordWorkerTransaction(label, startedAt - calledAt, finishedAt - startedAt);
    throw error;
  }
}
...
async function processCapsule(
  ...
  try {
    ...
    timeWorkerTransaction("appendSessionEvidence", (markStart) =>
      appendSessionEvidence(db, checkpoint.session_id, { onTransactionStart: markStart }));
    const page = timeWorkerTransaction("readCapsulePage", (markStart) =>
      db.transaction(() => {
        markStart();
