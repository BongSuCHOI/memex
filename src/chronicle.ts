...
const EVENT_ONLY_KINDS = new Set<ChronicleEventKind>(["VALIDATED", "INCIDENT", "CONTRADICTED", "SYNC_IMPORTED"]);
...
export function recordChronicleEvent(
  ...
  if (input.factId && input.newValue && input.projectionApplied && effectiveAtSource !== 'peer' &&
    ...
    if (!recordLocalMeaningEvidence(db, input.factId, input.newValue, input.actor, sourceExchangeIds)) {
      console.error(
        `local meaning evidence NOT recorded for fact ${input.factId} (${input.actor}): ` +
