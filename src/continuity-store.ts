...
export function ensureContinuitySchema(
  ...
  const migrate = db.transaction(() => {
    ...
    if (priorVersion < 7) {
      ...
      const rows = db.prepare("SELECT id FROM exchanges ORDER BY rowid").all() as Array<{ id: string }>;
...
/**
 * The hash of an exchange AS STORED, prepared once for a whole pass.
 *
 * #169: `refreshExchangeMetadata` and the invariant that checks it must read the
 * same columns and reduce them the same way, or the check would pass while the
 * pass still rewrote the row. One factory, both callers.
 */
function storedExchangeHasher(db: Database.Database): (row: {
  id: string;
  user_message: string;
...
export function readJobFailureToClear(
  db: Database.Database,
  jobId: string,
): ClearedJobFailure | null {
  const row = db.prepare(
    "SELECT state, attempts, last_error FROM memory_jobs WHERE job_id = ?",
  ).get(jobId) as { state: string; attempts: number; last_error: string | null } | undefined;
  if (!row || row.last_error == null) return null;
 ...
 */
export function recordClearedJobFailure(
  db: Database.Database,
  input: {
    jobId: string;
  ...
  const row = db.prepare("SELECT retry_history FROM memory_jobs WHERE job_id = ?")
    .get(input.jobId) as { retry_history: string | null } | undefined;
  if (!row) return;
  let history: unknown[] = [];
...
export function completeMemoryJob(
  ...
  if (completed) recordClearedJobFailure(db, { jobId: input.jobId, cleared, now: nowIso });
  return completed;
}
...
export function ensureExtractionTarget(
  ...
  `).get(input.sessionId) as { line_end: number; exchange_seq: number } | undefined;
  ...
  `).all(
    ...
    input.sessionId,
    firstOpen ? 1 : null,
    firstOpen?.line_end ?? 0,
    firstOpen?.line_end ?? 0,
    firstOpen?.exchange_seq ?? 0,
 ...
 * it unconditionally.
 */
export function setExtractionTargetRulesHash(
  db: Database.Database,
  targetId: string,
  rulesHash: string | null,
): boolean {
  if (!columnNames(db, "extraction_targets").has("rules_hash")) return false;
  return db.prepare(
 ...
 */
export function requeueCompletedExtractionTarget(
  db: Database.Database,
  input: { targetId: string; jobId?: string | null; checkpointId?: string | null; now?: string },
): Record<string, number> {
  const now = input.now ?? new Date().toISOString();
  const changed: Record<string, number> = {};
  const bump = (table: string, changes: number): void => {
  ...
  if (input.checkpointId) {
    bump("checkpoints", db.prepare(
      "UPDATE checkpoints SET state = 'pending' WHERE checkpoint_id = ? AND state = 'processed'",
    ).run(input.checkpointId).changes);
  }
  bump("extraction_target_items", db.prepare(
    "UPDATE extraction_target_items SET state = 'pending' WHERE target_id = ? AND state <> 'pending'",
