...
export interface MemoryJobDetail extends MemoryJobSummary {
  ...
  checkpoint: { checkpointId: string; kind: string; state: string; sessionId: string } | null;
  capsuleCheckpointState: { state: string; lastError: string | null } | null;
  target: { targetId: string; sessionId: string; state: string; itemCount: number; lastError: string | null } | null;
  targetItemStates: Record<string, number>;
  failedRanges: Array<{ failureId: string; fromOrdinal: number; throughOrdinal: number; state: string; errorKind: string; errorMessage: string }>;
}
 ...
 */
export function ensureJobRecoverySchema(db: Database.Database): void {
  if (db.readonly || !tableExists(db, "memory_jobs")) return;
  if (!columnExists(db, "memory_jobs", "retry_history")) {
    db.exec("ALTER TABLE memory_jobs ADD COLUMN retry_history TEXT");
  }
}

function parseRetryHistory(raw: unknown): RetryHistoryEntry[] {
  if (typeof raw !== "string" || !raw.trim()) return [];
  try {
    const parsed = JSON.parse(raw);
    return Array.isArray(parsed) ? parsed as RetryHistoryEntry[] : [];
  } catch {
    return [];
  }
}

function toSummary(row: Record<string, unknown>, nowIso: string): MemoryJobSummary {
...
export function listMemoryJobs(
  ...
  options: { state?: string; kind?: string; limit?: number; now?: Date } = {},
): MemoryJobSummary[] {
...
export function showMemoryJob(
  ...
  const nowIso = (options.now ?? new Date()).toISOString();
  const row = db.prepare("SELECT * FROM memory_jobs WHERE job_id = ?").get(jobId) as
  ...
  const checkpoint = summary.checkpointId
    ? db.prepare("SELECT checkpoint_id, kind, state, session_id FROM checkpoints WHERE checkpoint_id = ?")
        .get(summary.checkpointId) as { checkpoint_id: string; kind: string; state: string; session_id: string } | undefined
    : undefined;
  const capsuleState = summary.checkpointId && tableExists(db, "capsule_checkpoint_state")
    ? db.prepare("SELECT state, last_error FROM capsule_checkpoint_state WHERE checkpoint_id = ?")
        .get(summary.checkpointId) as { state: string; last_error: string | null } | undefined
    : undefined;
  const target = summary.targetId && tableExists(db, "extraction_targets")
    ? db.prepare("SELECT target_id, session_id, state, item_count, last_error FROM extraction_targets WHERE target_id = ?")
        .get(summary.targetId) as { target_id: string; session_id: string; state: string; item_count: number; last_error: string | null } | undefined
    : undefined;
  const itemStates = summary.targetId && tableExists(db, "extraction_target_items")
    ? db.prepare("SELECT state, COUNT(*) AS c FROM extraction_target_items WHERE target_id = ? GROUP BY state")
        .all(summary.targetId) as Array<{ state: string; c: number }>
    : [];
  const failedRanges = summary.targetId && tableExists(db, "extraction_failed_ranges")
    ? db.prepare(`SELECT failure_id, from_ordinal, through_ordinal, state, error_kind, error_message
        ...
        .all(summary.targetId) as Array<{ failure_id: string; from_ordinal: number; through_ordinal: number; state: string; error_kind: string; error_message: string }>
    : [];
  return {
    ...summary,
...
function resolveUnits(
  db: Database.Database,
  input: { jobId?: string; targetId?: string; allDead?: boolean; kind?: string },
  nowIso: string,
): RecoveryUnit[] {
  const jobRow = (where: string, ...params: unknown[]) =>
    db.prepare(`SELECT * FROM memory_jobs WHERE ${where}`).get(...params) as Record<string, unknown> | undefined;
  const unitFromJob = (row: Record<string, unknown>): RecoveryUnit => ({
    jobId: String(row.job_id),
    targetId: row.target_id == null ? null : String(row.target_id),
    checkpointId: row.checkpoint_id == null ? null : String(row.checkpoint_id),
    kind: String(row.kind),
  ...
  if (input.allDead) {
    const jobs = db.prepare(`SELECT * FROM memory_jobs WHERE state = ?${input.kind ? " AND kind = ?" : ""}`)
      .all(...(input.kind ? [RECOVERABLE_JOB_STATE, input.kind] : [RECOVERABLE_JOB_STATE])) as Array<Record<string, unknown>>;
    ...
    if (!input.kind && tableExists(db, "extraction_targets")) {
      ...
      const orphans = db.prepare("SELECT target_id, last_error, attempts FROM extraction_targets WHERE state = ?")
        .all(RECOVERABLE_TARGET_STATE) as Array<{ target_id: string; last_error: string | null; attempts: number }>;
      for (const orphan of orphans) {
        if (claimed.has(orphan.target_id)) continue;
        units.push({
  ...
  if (!id) throw new Error("recover requires a job id, a target id, or --all-dead");
  ...
  const byTarget = tableExists(db, "extraction_targets")
    ? db.prepare("SELECT target_id, state, last_error, attempts FROM extraction_targets WHERE target_id = ?")
        .get(id) as { target_id: string; state: string; last_error: string | null; attempts: number } | undefined
    : undefined;
  if (!byTarget) throw new Error(`no memory job or extraction target with id ${id}`);
  ...
  if (owner) {
    ...
    if (leaseLive && ownerState !== RECOVERABLE_JOB_STATE && ownerState !== "retry") {
      throw new Error(
        `target ${id} is owned by job ${String(owner.job_id)} which is '${ownerState}' ` +
 ...
 */
export function recoverTerminalWork(
  db: Database.Database,
  input: {
    jobId?: string;
    targetId?: string;
  ...
  const run = () => {
    ...
    for (const unit of units) {
      ...
      const bump = (table: string, changes: number) => {
        if (changes > 0) reset[table] = (reset[table] ?? 0) + changes;
      };
      if (unit.jobId) {
        const history: RetryHistoryEntry[] = [
          ...unit.retryHistory,
          { at: nowIso, fromState: unit.fromState, attempts: unit.attempts, lastError: unit.lastError, action: "retry" as const },
        ].slice(-32);
        ...
        if (changes === 0) {
          notes.push(
            `job ${unit.jobId} left '${unit.fromState}' before it could be reset ` +
        ...
        if (tableExists(db, "capsule_checkpoint_state")) {
          ...
          const skip = hasSkipColumns
            ? db.prepare(`SELECT workstream_id, skipped_seq, frontier_before_skip
                ...
                .get(unit.checkpointId) as
                  | { workstream_id: string; skipped_seq: number | null; frontier_before_skip: number | null }
                  | undefined
            : undefined;
            ...
            : db.prepare(`UPDATE capsule_checkpoint_state
                SET state = 'pending', last_error = NULL, updated_at = ?,
                    ...
                    page_items_hint = NULL, page_chars_hint = NULL${
                      hasSkipColumns ? ", skipped_seq = NULL, frontier_before_skip = NULL" : ""}
      ...
      if (unit.targetId) {
        ...
        bump("extraction_target_items", dryRun
          ? (db.prepare(`SELECT COUNT(*) AS c FROM extraction_target_items WHERE target_id = ? AND ${pendingItems}`)
              .get(unit.targetId) as { c: number }).c
      ...
      if (unit.jobId && !dryRun && tableExists(db, "model_work_budgets")) {
        ...
        `).get(unit.jobId) as { state: string } | undefined;
        if (spent) {
          const next = startNewModelWorkRunForJob(db, { jobId: unit.jobId, now });
          notes.push(
            `job ${unit.jobId} was bound to a ${spent.state} model work budget; ` +
...
function appendRecoveryAudit(event: Record<string, unknown>): string | null {
  try {
 ...
 */
export function dismissMemoryJob(
  db: Database.Database,
  input: { jobId: string; reason: string; now?: Date },
): DismissResult {
  ensureJobRecoverySchema(db);
  const reason = input.reason.trim();
  if (!reason) throw new Error("dismiss requires --reason \"<why>\"");
  const nowIso = (input.now ?? new Date()).toISOString();
  const row = db.prepare("SELECT * FROM memory_jobs WHERE job_id = ?").get(input.jobId) as
    | Record<string, unknown>
    | undefined;
  if (!row) throw new Error(`no memory job with id ${input.jobId}`);
  ...
  const history: RetryHistoryEntry[] = [
    ...
    { at: nowIso, fromState, attempts: Number(row.attempts), lastError: row.last_error == null ? null : String(row.last_error), action: "dismiss" as const },
  ].slice(-32);
  const reset: Record<string, number> = {};
  const bump = (table: string, changes: number) => {
    if (changes > 0) reset[table] = (reset[table] ?? 0) + changes;
...
export function recordRecoveryAudit(result: RecoveryResult, source: string): string | null {
