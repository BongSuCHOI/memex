 ...
 */
export const HOOK_HOST_TIMEOUT_MS: Record<string, number> = {
  SessionStart: 10_000,
  Stop: 10_000,
  PostCompact: 10_000,
  PreCompact: 15_000,
...
export function hookHostTimeoutMs(hookEventName: string): number {
...
export const HOOK_BUDGET_PRECOMPACT_MS =
  HOOK_HOST_TIMEOUT_MS.PreCompact - HOOK_EXIT_MARGIN_MS;
...
export type IngestFit =
  | { ok: true }
  | { ok: false; reason: "deadline" | "oversize"; detail: string };
 ...
 */
export function ingestFitsBudget(
  bytesToIngest: number,
  remainingMs: number,
  bytesPerMs: number = hookIngestBytesPerMs(),
): IngestFit {
  if (remainingMs < HOOK_INGEST_RESERVE_MS) {
    return {
      ok: false,
      reason: "deadline",
      detail: `hook budget exhausted before the capture could start (${
        Math.max(0, Math.round(remainingMs))} ms left, ${HOOK_INGEST_RESERVE_MS} ms reserved)`,
    };
  }
  ...
  if (estimateMs > usableMs) {
    return {
      ok: false,
      reason: "oversize",
      detail: `${bytesToIngest} pending bytes need about ${
        Math.round(estimateMs)} ms and only ${Math.round(usableMs)} ms of the hook budget is usable`,
    };
  }
  return { ok: true };
}
...
export function markCaptureGapRecorded(error: unknown): void {
  if (error && typeof error === "object") {
    try {
      (error as Record<symbol, unknown>)[CAPTURE_GAP_RECORDED] = true;
    } catch {
...
export function captureGapAlreadyRecorded(error: unknown): boolean {
