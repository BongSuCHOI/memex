...
export function isSafeZipName(name: string): boolean {
  if (!name || name.length > MAX_ENTRY_NAME) return false;
  if (name.includes("\0") || name.includes("\\")) return false;
  if (name.startsWith("/") || name.endsWith("/") || /^[A-Za-z]:/.test(name)) return false;
...
export function createZip(entries: ZipEntry[]): Buffer {
  ...
  for (const entry of entries) {
    if (!isSafeZipName(entry.name)) throw new Error(`zip entry name is not writable: ${entry.name}`);
 ...
 */
export function readZip(buffer: Buffer, limits: ZipReadLimits = {}): Map<string, Buffer> {
  const maxEntries = limits.maxEntries ?? 64;
  const maxTotalBytes = limits.maxTotalBytes ?? 256 * 1024 * 1024;
  if (buffer.length < EOCD_BYTES) throw new Error("zip is too short to contain a central directory");
  const eocd = findEndOfCentralDirectory(buffer);
  if (eocd < 0) throw new Error("zip central directory not found (not a zip container?)");
  const count = buffer.readUInt16LE(eocd + 10);
  const centralSize = buffer.readUInt32LE(eocd + 12);
  const centralOffset = buffer.readUInt32LE(eocd + 16);
  if (count > maxEntries) throw new Error(`zip carries ${count} entries, more than the ${maxEntries} allowed`);
  ...
  for (let i = 0; i < count; i++) {
    if (cursor + CENTRAL_HEADER_BYTES > buffer.length) throw new Error("zip central directory ends early");
    if (buffer.readUInt32LE(cursor) !== CENTRAL_SIGNATURE) throw new Error("zip central directory record is malformed");
    ...
    if (name.endsWith("/")) continue; // directory marker: nothing to read
    if (!isSafeZipName(name)) throw new Error(`zip entry name is not readable: ${JSON.stringify(name)}`);
    total += uncompressedSize;
    if (total > maxTotalBytes) throw new Error(`zip expands to more than the ${maxTotalBytes} bytes allowed`);
    if (localOffset + LOCAL_HEADER_BYTES > buffer.length) throw new Error(`zip entry ${name} points outside the file`);
    if (buffer.readUInt32LE(localOffset) !== LOCAL_SIGNATURE) throw new Error(`zip entry ${name} has no local header`);
    const localNameLength = buffer.readUInt16LE(localOffset + 26);
    const localExtraLength = buffer.readUInt16LE(localOffset + 28);
    const start = localOffset + LOCAL_HEADER_BYTES + localNameLength + localExtraLength;
    if (start + compressedSize > buffer.length) throw new Error(`zip entry ${name} is truncated`);
    const body = buffer.subarray(start, start + compressedSize);
    let data: Buffer;
    if (method === 0) data = Buffer.from(body);
    else if (method === 8) data = inflateRawSync(body, { maxOutputLength: maxTotalBytes });
    else throw new Error(`unsupported zip compression method ${method} for entry ${name}`);
    if (data.length !== uncompressedSize) throw new Error(`zip entry ${name} does not match its declared size`);
    if (crc32(data) !== crc) throw new Error(`zip entry ${name} fails its CRC-32 check`);
    files.set(name, data);
  }
  return files;
}
