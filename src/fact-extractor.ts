...
import {
  claimSessionSql, renewClaimSql, failureMarkerUpsertSql,
...
export async function runFactExtraction(
  ...
  const writeInternalFailureMarker = () => {
    ...
    try {
      ...
      console.error(
        `extraction: session ${sessionId} 내부 실패 (attempt ${attempts}/${MAX_INTERNAL_RETRIES}`
  ...
  try {
    ...
    if (providerFailure) releaseClaim();
    else writeInternalFailureMarker();
    throw e;
  }
  ...
  try {
    ...
    let markerWritten: { c: number } | undefined;
    if (hasDropped) {
      db.prepare(`
      ...
      markerWritten = db.prepare('SELECT changes() AS c').get() as { c: number };
    } else {
      db.prepare(`
        INSERT INTO extraction_log (session_id, processed_at, extracted, saved)
        VALUES (?, ?, ?, ?)
    ...
    if (markerWritten && markerWritten.c === 0) {
      console.error(
        `extraction: session ${sessionId} 완료 마커가 기록되지 않았습니다(소유권 상실 추정) — `
