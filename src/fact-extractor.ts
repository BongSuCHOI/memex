...
export async function runFactExtraction(
  ...
  try {
    ...
    try {
      if (preClaim) {
        db.prepare(
          `UPDATE extraction_log SET extracted = ?, saved = ? `
          + `WHERE session_id = ? AND extracted = ${EXTRACTION_STATE.CLAIMED}`,
        ).run(preClaim.extracted, preClaim.saved, sessionId);
      } else {
        db.prepare(
          `DELETE FROM extraction_log WHERE session_id = ? AND extracted = ${EXTRACTION_STATE.CLAIMED}`,
        ).run(sessionId);
      }
