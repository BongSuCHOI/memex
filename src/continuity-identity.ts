...
/**
 * One git config entry, named git's way: section and key are lowercased, a
 * subsection keeps its case, and a valueless key (`[init]\n\tbare`) carries
 * `value: null` — git's "boolean true", never a branch name.
 */
 ...
 */
function parseGitConfigText(text: string): GitConfigEntry[] {
  ...
  const readSectionHeader = (): boolean => {
    let name = "";
    for (;;) {
      const char = next();
  ...
  const readValue = (): string | null => {
    ...
    for (;;) {
      ...
      if (char === "\\") {
        ...
        else return null; // git rejects every other escape
        continue;
      }
      if (char === '"') {
        quoted = !quoted;
  ...
  const readEntry = (first: string): boolean => {
...
/** What an `includeIf` condition is matched against. */
interface GitIncludeContext {
  /**
   * `$GIT_DIR` with symlinks resolved. git compares `gitdir:` against the
   * realpath and nothing else — its `strbuf_add_absolute_path` retry operates on
   * a `git_dir` that setup has already resolved — so a pattern written with a
   * symlinked path matches in neither git nor here (checked against git 2.50.1).
   */
  gitDir: string | null;
  /** `HEAD`'s short name, for `onbranch:`. */
  branch: string | null;
}
...
function globComponentSource(component: string): string | null {
  let source = "";
 ...
 */
function pathGlobToRegExp(pattern: string, icase: boolean): RegExp | null {
...
function expandTildePath(value: string): string | null {
...
function realpathOrAbsolute(value: string): string {
  try {
 ...
 */
function prepareGitdirPattern(
  pattern: string,
  file: string | null,
): { pattern: string; prefix: number } | null {
  let value = expandTildePath(pattern);
  if (value === null) return null;
  let prefix = 0;
  if (value[0] === "." && (value[1] === "/" || value[1] === path.sep)) {
    if (!file) return null; // git: a relative conditional must come from a file
...
function matchGitdirPattern(
  prepared: { pattern: string; prefix: number },
  text: string,
  icase: boolean,
): boolean {
 ...
 */
function gitIncludeConditionIsTrue(
  condition: string,
  file: string | null,
  context: GitIncludeContext,
): boolean {
  const gitdir = (pattern: string, icase: boolean): boolean => {
...
function isGitIncludeEntry(entry: GitConfigEntry): boolean {
...
function resolveIncludePath(value: string, file: string | null): string | null {
...
function gitConfigEntries(
  ...
  const collected: GitConfigEntry[] = [];
  for (const entry of parseGitConfigText(text)) {
    ...
    if (entry.section === "includeif"
      && !gitIncludeConditionIsTrue(entry.subsection as string, file, context)) continue;
...
function lastInitDefaultBranch(entries: readonly GitConfigEntry[]): string | null {
...
function isReadableFile(value: string): boolean {
  try {
 ...
 */
function gitConfigFilesInReadOrder(): string[] {
  const files: string[] = [];
