...
export function getPipelineStatus(opts: { dbPath?: string } = {}): PipelineStatus {
  const dbPath = opts.dbPath ?? getDbPath();
  const dbExists = fs.existsSync(dbPath);

  // Lifecycle observation log lives outside the DB and is always safe to read.
  const lifecycleLastEventAt: Partial<Record<string, string>> = readHookEvents();

  if (!dbExists) {
    return {
      dataRootEmpty: true,
      conversations: { sessionsIndexed: 0, exchanges: 0, archiveFiles: countArchiveFiles(getArchiveDir()), ready: false },
      extraction: { total: 0, done: 0, pending: 0, claimed: 0, failedPermanent: 0, retriable: 0, lastSuccessAt: null, lastErrorAt: null },
      embeddings: { activeFacts: 0, factVectorsPending: 0 },
      ontology: { classifiedFacts: 0, pendingFacts: 0 },
      relations: 0,
      lifecycleLastEventAt,
      readiness: { conversationReady: false, factReady: false, graphReady: false },
    };
  }
  ...
  try {
    ...
    let extraction: PipelineStatus['extraction'] = {
      total: 0, done: 0, pending: 0, claimed: 0,
      failedPermanent: 0, retriable: 0, lastSuccessAt: null, lastErrorAt: null,
    };
    if (hasExchanges && hasExtractionLog) {
      ...
      const times = db.prepare(`
        ...
        FROM extraction_log`).get() as { lastOk: string | null; lastErr: string | null };
    ...
    let embeddings = { activeFacts: 0, factVectorsPending: 0 };
    let ontology = { classifiedFacts: 0, pendingFacts: 0 };
    let relations = 0;
    if (hasFacts) {
    ...
    if (hasRelations) relations = count(db, 'SELECT COUNT(*) AS c FROM ontology_relations');
    ...
    return {
      dataRootEmpty: false,
      conversations: { sessionsIndexed, exchanges, archiveFiles, ready: conversationReady },
      extraction,
      embeddings,
      ontology,
      relations,
...
export function formatPipelineStatus(s: PipelineStatus): string {
  const lines: string[] = [];
  if (s.dataRootEmpty) lines.push('Data root: EMPTY (no index database yet — run: memex sync)');
  else lines.push(`Conversations: ${s.conversations.ready ? 'READY' : 'EMPTY'} (${s.conversations.sessionsIndexed} sessions / ${s.conversations.exchanges} exchanges / ${s.conversations.archiveFiles} archived rollouts)`);
