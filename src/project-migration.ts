...
export interface MigrationPlan {
  ...
  movable: Array<{ id: number | bigint; from: string; to: string }>;
  ambiguous: Array<{ id: number | bigint; project: string; reason: string }>;
  factsRescope: Array<{ id: number | bigint; from: string; to: string }>;
}
export function planMigration(db: Database.Database): MigrationPlan {
  const rows = db.prepare(
    'SELECT id, project, cwd FROM exchanges',
  ).all() as Array<{ id: number | bigint; project: string; cwd: string | null }>;

  const movable: MigrationPlan['movable'] = [];
  const ambiguous: MigrationPlan['ambiguous'] = [];
  ...
  const factsRescope: MigrationPlan['factsRescope'] = [];
  if (tables.length > 0) {
    const factRows = db.prepare(
      "SELECT id, scope_project FROM facts WHERE scope_type = 'project' AND scope_project IS NOT NULL",
    ).all() as Array<{ id: number | bigint; scope_project: string }>;
    for (const f of factRows) {
      if (!path.isAbsolute(f.scope_project)) continue; // legacy slug: never guess
...
async function backupDatabase(db: Database.Database): Promise<string> {
  const backup = `${db.name}.pre-project-migration-${Date.now()}`;
  await db.backup(backup);
  return backup;
}

export async function applyMigration(db: Database.Database, dbPath: string): Promise<MigrationResult> {
  const plan = planMigration(db);
  const before = {
    exchanges: (db.prepare('SELECT COUNT(*) AS c FROM exchanges').get() as { c: number }).c,
    facts: (() => {
      const t = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='facts'").all();
      return t.length
        ? (db.prepare('SELECT COUNT(*) AS c FROM facts').get() as { c: number }).c
        : -1;
    })(),
  };
  ...
  const sel = db.prepare('SELECT id, archive_path, cwd, project FROM exchanges').all() as
    Array<{ id: number | bigint; archive_path: string; cwd: string | null; project: string }>;
  ...
  const after = {
    exchanges: (db.prepare('SELECT COUNT(*) AS c FROM exchanges').get() as { c: number }).c,
    facts: before.facts === -1 ? -1 : (db.prepare('SELECT COUNT(*) AS c FROM facts').get() as { c: number }).c,
  };
