...
const CHROME = process.env.MEMORY_BANK_CHROME_PATH || '/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';
const evidenceArg = process.argv.indexOf('--evidence-dir');
const EVIDENCE = evidenceArg >= 0 ? path.resolve(process.argv[evidenceArg + 1]) : fs.mkdtempSync('/tmp/mb-web-ui-evidence-');
...
const MALICIOUS = '<img src=x onerror=globalThis.__mbInjected=true> 한글 사실은 안전하게 표시됩니다';
...
function startServer(port) {
  const child = spawn(process.execPath, [path.join(ROOT, 'ui', 'server.cjs')], {
    cwd: ROOT,
    env: { ...process.env, MEMORY_BANK_HOME: HOME, TEST_DB_PATH: DB_PATH, MEMORY_BANK_PLUGIN_ROOT: ROOT, PORT: String(port) },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  let stderr = '';
  const ready = new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('UI start timeout\n' + stderr)), 10000);
    child.stdout.setEncoding('utf8');
    child.stderr.setEncoding('utf8');
    child.stdout.on('data', (chunk) => {
      if (chunk.includes('Memex UI: http://localhost:' + port)) { clearTimeout(timer); resolve(); }
    });
...
function startChrome() {
  ...
  const ready = new Promise((resolve, reject) => {
    let stderr = '';
    const timer = setTimeout(() => reject(new Error('Chrome start timeout\n' + stderr)), 15000);
    child.stderr.setEncoding('utf8');
...
async function pageProbe(cdp, url, expression, screenshotName, keyboard = false) {
  ...
  try {
    ...
    if (keyboard) {
      ...
      await cdp.send('Input.dispatchKeyEvent', { type: 'keyDown', key: 'Tab', code: 'Tab' }, sessionId);
      await cdp.send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Tab', code: 'Tab' }, sessionId);
      const focus = await cdp.send('Runtime.evaluate', {
        expression: '({tag:document.activeElement?.tagName,text:document.activeElement?.textContent?.trim(),href:document.activeElement?.getAttribute?.("href")})',
        returnByValue: true,
      }, sessionId);
    ...
    return { ...result.result.value, keyboardFocus, screenshot: screenshotPath };
  } finally {
...
function waitFor(condition, value) {
  return "new Promise((resolve,reject)=>{const deadline=setTimeout(()=>reject(new Error('surface timeout')),15000);const done=()=>{if(" +
    condition + "){clearTimeout(deadline);resolve(" + value +
...
try {
  ...
  process.env.MEMORY_BANK_HOME = HOME;
  process.env.TEST_DB_PATH = DB_PATH;
  const { initDatabase } = await import(path.join(ROOT, 'dist', 'db.js'));
  const db = initDatabase();
  const now = new Date().toISOString();
  db.prepare("INSERT INTO facts (id, fact, category, scope_type, scope_project, source_exchange_ids, created_at, updated_at, is_active) VALUES (?, ?, ?, 'global', NULL, '[]', ?, ?, 1)")
    .run('00000000-0000-4000-8000-000000000001', MALICIOUS, 'decision', now, now);
  db.close();
  ...
  const facts = await pageProbe(cdp, base + '/facts', waitFor(
  ...
  const pipeline = await pageProbe(cdp, base + '/pipeline', waitFor(
  ...
  const graph = await pageProbe(cdp, base + '/graph?scope=global', waitFor(
  ...
  ), 'graph-empty.png', true);

  if (facts.hasInjectedImage || facts.injectedFlag || facts.rowCount !== 1 || !facts.factText.includes('한글 사실')) {
  ...
  if (pipeline.rows !== 3 || !pipeline.text.includes('conversation-ready')) {
    throw new Error('Pipeline browser assertion failed: ' + JSON.stringify(pipeline));
  }
  if (graph.facts !== '0' || !graph.emptyState || !graph.pipelineLink || !graph.canvas.every(Boolean)) {
    throw new Error('Graph empty-state assertion failed: ' + JSON.stringify(graph));
  }
  for (const result of [facts, pipeline, graph]) {
    if (result.keyboardFocus?.tag !== 'A') throw new Error('keyboard focus did not enter navigation: ' + JSON.stringify(result.keyboardFocus));
  }
  if (cdp.runtimeErrors.length) throw new Error('browser runtime errors: ' + cdp.runtimeErrors.join('; '));
  console.log('__WEB_UI_RECEIPT__' + JSON.stringify({
    kind: 'memex-web-ui-browser-e2e',
    recordedAt: new Date().toISOString(),
    environment: { browser: 'Google Chrome headless', transport: 'CDP', viewport: '1440x900' },
    verdict: 'PASS',
    checks: { facts, pipeline, graphEmpty: graph, runtimeErrors: cdp.runtimeErrors },
  }));
} finally {
  cdp?.close();
