...
const ENV = { ...process.env, CODEX_HOME, MEMEX_HOME: MEMORY_HOME };
...
function command(commandName, args, options = {}) {
  ...
  if (result.error || result.status !== 0) {
    throw new Error(
      `${commandName} ${args.join(" ")} failed (${result.status}): ${(result.stderr || result.stdout || result.error?.message || "").trim()}`,
    );
  }
  return result;
}
...
function jsonCommand(commandName, args, options = {}) {
  ...
  try {
    ...
    throw new Error(
      `${commandName} ${args.join(" ")} returned invalid JSON: ${e.message}`,
    );
  }
}

/** Guarded JSON file read: a corrupt artifact fails with its path in the message. */
function parseJsonFile(filePath, label) {
  try {
    return JSON.parse(fs.readFileSync(filePath, "utf8"));
  } catch (e) {
    throw new Error(
      `${label} is missing or invalid JSON (${filePath}): ${e.message}`,
    );
  }
}

function stageMarketplace() {
  for (const directory of [CODEX_HOME, MEMORY_HOME, SOURCE])
...
function validateManifest(installedRoot) {
  ...
  const manifest = parseJsonFile(manifestPath, "plugin manifest");
  for (const key of [
    "name",
    "version",
    "description",
  ...
  if ("plugin_hooks" in manifest)
    throw new Error("plugin manifest contains unsupported plugin_hooks field");
  if (manifest.name !== "memex")
    throw new Error(`unexpected plugin name: ${manifest.name}`);
  const hooksPath = path.resolve(installedRoot, manifest.hooks);
  if (
    !hooksPath.startsWith(installedRoot + path.sep) ||
    !fs.existsSync(hooksPath)
  ) {
    throw new Error(
      `hooks path does not resolve inside installed root: ${hooksPath}`,
    );
  }
...
function validateMcpManifest(installedRoot) {
  ...
  if (
    ...
    throw new Error(
      `MCP command does not resolve inside installed root: ${resolved}`,
    );
  }
  return resolved;
}
...
function validateSkills(installedRoot) {
  ...
  for (const name of fs.readdirSync(skillsRoot).sort()) {
    ...
    if (!frontmatter)
      throw new Error(`${name}/SKILL.md lacks YAML frontmatter`);
    const skillName = frontmatter[1].match(/^name:\s*(.+)$/m)?.[1]?.trim();
    const description = frontmatter[1]
      .match(/^description:\s*(.+)$/m)?.[1]
      ?.trim();
    if (skillName !== name)
      throw new Error(`${name}/SKILL.md name mismatch: ${skillName}`);
  ...
  const messages = [
    {
      ...
      params: {
        ...
        capabilities: {},
        clientInfo: { name: "plugin-validator", version: "1" },
      },
    },
    ...
    { jsonrpc: "2.0", id: 2, method: "tools/list", params: {} },
    {
      ...
      id: 3,
      method: "tools/call",
      params: { name: "graph_stats", arguments: { scope: "all" } },
    },
  ];
  const result = command(
    process.execPath,
    [path.join(installedRoot, "cli", "mcp-server-wrapper.js")],
    {
      cwd: installedRoot,
      env: {
        ...ENV,
  ...
  const responses = result.stdout
    ...
    .map((line) => {
      try {
        ...
        throw new Error(
          `MCP response line was not valid JSON: ${line.slice(0, 120)} (${e.message})`,
        );
      }
    });
...
async function main() {
  ...
  const validatorProbe = spawnSync("codex", ["plugin", "validate", "--help"], {
    cwd: ROOT,
    env: ENV,
    encoding: "utf8",
  });
  const validatorOutput =
    `${validatorProbe.stderr || ""}${validatorProbe.stdout || ""}`.trim();
  const validatorAbsent =
    validatorProbe.status !== 0 &&
    /unrecognized subcommand ['"]validate['"]|unknown subcommand/i.test(
      validatorOutput,
    );
  if (!validatorAbsent)
    throw new Error(
      `version-bound substitute is not allowed: validator probe was not an absent-command result\n${validatorOutput}`,
    );
  ...
  if (!added.installedPath || !path.isAbsolute(added.installedPath))
    throw new Error("plugin add did not return installedPath");
  const installedRoot = fs.realpathSync(added.installedPath);
  const installed = jsonCommand("codex", [
    "plugin",
    "list",
    "--json",
  ]).installed.find((plugin) => plugin.pluginId === PLUGIN_ID);
  if (!installed?.version)
    throw new Error("plugin list did not return installed version");
  const expectedCache = fs.realpathSync(
    path.join(
      CODEX_HOME,
      "plugins",
      "cache",
  ...
  if (installedRoot !== expectedCache)
    throw new Error(
      `installedPath is not the Codex cache root: ${installedRoot}`,
    );

  const packagedDependencies = materializePluginDependencies(
    ROOT,
    installedRoot,
  );
  ...
  const marketsAfter =
    jsonCommand("codex", ["plugin", "marketplace", "list", "--json"])
      .marketplaces || [];
  const cleanup = {
    plugin_absent: !pluginsAfter.some(
      (plugin) => plugin.pluginId === PLUGIN_ID,
    ),
    marketplace_absent: !marketsAfter.some(
  ...
  const receipt = {
    ...
    validator: {
      ...
      policy:
        "Codex CLI lacks the command; the acceptance SSOT authorizes the isolated substitute contract below for this version only.",
    },
    checks: [
      {
        name: "manifest schema and plugin hook discovery",
        status: "PASS",
        observed: pluginManifest,
      },
      {
        name: "isolated marketplace add",
        status: "PASS",
        observed: MARKET_NAME,
      },
      {
        name: "isolated plugin add installedPath",
        status: "PASS",
        observed: installedRoot,
      },
      { name: "Codex cache identity", status: "PASS", observed: expectedCache },
      {
        name: "production dependency packaging",
        status: "PASS",
        observed: {
          ...packagedDependencies,
          method: "local copy, no npm install/network",
        },
      },
      {
        name: "MCP manifest installed-root resolution",
        status: "PASS",
        observed: mcpCommand,
      },
      {
      ...
      {
        name: "isolated registration cleanup",
        status: "PASS",
        observed: cleanup,
      },
    ],
    cleanup,
    conclusion:
      "The formal validator command is unavailable in this exact CLI version; every pre-authorized installed-artifact substitute check passed. Re-run the formal validator when a future CLI exposes it.",
  };
