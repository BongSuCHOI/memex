...
const CONCURRENCY = Math.max(
  1,
  boundedInt(process.env.BACKFILL_CONCURRENCY, 4, 8),
);
...
const { minExchanges: MIN_EXCHANGES, excludeProjects: EXCLUDE_PROJECTS } =
  getExtractionConfig();
...
function releaseLock() {
  try {
    if (parseInt(fs.readFileSync(LOCK, "utf8"), 10) === process.pid)
      fs.unlinkSync(LOCK);
  } catch {
    /* ignore */
  }
  ...
  const workers = Array.from({ length: concurrency }, async () => {
    while (queue.length > 0) {
      ...
      try {
        ...
        isolated++;
        log(
          `session ${item.sid ?? "?"}: 격리되지 않은 예외 — 이 세션만 건너뜁니다: ${e instanceof Error ? e.message : e}`,
        );
      }
    }
    // 풀 워커는 개별 반환값이 없다 — 요약은 isolated 외부 카운터로 수합한다.
    // (array-callback-return: 명시적 return 으로 규칙 준수.)
    return undefined;
  });
...
function sessionProject(db, sid) {
  ...
  const bySlug = db
    .prepare("SELECT project FROM exchanges WHERE session_id = ? LIMIT 1")
    .get(sid);
  ...
  try {
      ...
      try {
        ...
        if (result.skipped) {
          if (result.skipped === "excluded_project_unmarked") {
            ...
            escalateFailures += 1;
            log(
              `session ${next.sid}: ERROR (excluded_project_unmarked) — 제외 마커 미기록, 다음 run 재선정 · 점검 필요`,
            );
            ...
            escalateFailures += 1;
            log(
              `session ${next.sid}: ERROR (${result.skipped}) — 선점 보류, 다음 run 재시도 · 점검 필요`,
            );
          }
          done++;
          return;
        ...
        if (result.saved > 0) {
          log(
            `session ${next.sid} (${project ?? "?"}, ${next.n} exch): saved ${result.saved}`,
          );
        }
      } catch (error) {
    ...
    log(
      `backfill-extract: done this run (sessions ${done}, facts saved ${totalSaved}` +
        (buckets.transient > 0
          ? `, transient-deferred ${buckets.transient} — will retry next run`
          : "") +
        (buckets.handoff > 0
        ...
        (escalateFailures > 0
          ? `, INTERNAL failures ${escalateFailures} — 런타임/DB 점검 필요`
          : "") +
        (isolated > 0
          ? `, isolated-exceptions ${isolated} — 격리된 예외(집계 밖), 점검 필요`
          : "") +
        ")",
    );
  } catch (error) {
