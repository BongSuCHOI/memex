...
const TIER = args.includes('--tier') ? args[args.indexOf('--tier') + 1] : 'offline';
...
function step(name, fn) {
  return Promise.resolve()
    .then(fn)
    .then((detail) => { results.push({ name, status: 'PASS', detail: detail || '' }); })
    .catch((err) => { results.push({ name, status: 'FAIL', detail: err && err.message ? err.message : String(err) }); });
}
...
for (const d of [CODEX_HOME, path.join(CODEX_HOME, 'sessions'), MB_HOME, PLUGIN]) fs.mkdirSync(d, { recursive: true });

// Stage the real plugin tree (dist + scripts + cli) so registered commands work.
fs.cpSync(path.join(REPO, 'dist'), path.join(PLUGIN, 'dist'), { recursive: true });
fs.cpSync(path.join(REPO, 'scripts'), path.join(PLUGIN, 'scripts'), { recursive: true });
fs.cpSync(path.join(REPO, 'cli'), path.join(PLUGIN, 'cli'), { recursive: true });
fs.cpSync(path.join(REPO, 'ui'), path.join(PLUGIN, 'ui'), { recursive: true });
fs.cpSync(path.join(REPO, '.codex-plugin'), path.join(PLUGIN, '.codex-plugin'), { recursive: true });
fs.cpSync(path.join(REPO, 'skills'), path.join(PLUGIN, 'skills'), { recursive: true });
fs.copyFileSync(path.join(REPO, '.mcp.json'), path.join(PLUGIN, '.mcp.json'));
fs.copyFileSync(path.join(REPO, 'package.json'), path.join(PLUGIN, 'package.json'));
...
fs.writeFileSync(path.join(marketManifestDir, 'marketplace.json'), JSON.stringify({
  name: MARKET_NAME,
  plugins: [{
    name: 'memex',
...
try {
  fs.symlinkSync(path.join(REPO, 'node_modules'), path.join(PLUGIN, 'node_modules'), 'dir');
} catch (e) {
...
before.userPluginList = spawnSync('codex', ['plugin', 'list', '--json'], { encoding: 'utf8' }).stdout;
before.userMarketplaceList = spawnSync('codex', ['plugin', 'marketplace', 'list', '--json'], { encoding: 'utf8' }).stdout;
...
const TEMP_SOCKET = path.join(MB_HOME, 'conversation-index', 'inject-daemon.sock');

const ENV = {
  ...process.env,
  CODEX_HOME,
  MEMORY_BANK_HOME: MB_HOME,
...
async function main() {
  ...
  await step('pre-setup: no Memex hook entries', () => {
    ...
    if (fs.existsSync(f)) {
      ...
      if (txt.includes('_memoryBank')) throw new Error('unexpected Memex entries before setup');
    }
  ...
  await step('isolated Codex marketplace and plugin registration', () => {
    ...
    const entry = JSON.parse(listed.stdout).installed.find((plugin) => plugin.pluginId === PLUGIN_ID);
    if (!entry || !added.installedPath || !path.isAbsolute(added.installedPath)) {
  ...
  await step('setup-hooks --dry-run shows adds without writing', () => {
    ...
    if (r.status !== 0) throw new Error(r.stderr || r.stdout);
    if (!r.stdout.includes('Add:') || !r.stdout.includes('No files changed.')) throw new Error(`dry-run output:\n${r.stdout}`);
    if (fs.existsSync(path.join(CODEX_HOME, 'hooks.json'))) throw new Error('dry-run wrote hooks.json');
  ...
  await step('setup-hooks registers events; second run is a no-op', () => {
    ...
    if (owned1 !== 6) throw new Error(`expected 6 owned entries, got ${owned1}`);

    r = MB(['setup-hooks']);
    if (r.status !== 0) throw new Error(r.stderr);
    const after2 = fs.readFileSync(file, 'utf8');
    if (after2 !== after1) throw new Error('second setup changed the file (not idempotent)');
    return 'idempotent merge confirmed';
  });
  // ── Step 4 (offline): handler wiring with stub worker ───────────────────
  if (TIER === 'offline') {
    await step('UserPromptSubmit handler emits valid no-match JSON shape', () => {
      // Cold path with an EMPTY data root: must exit 0 with either nothing or
      // valid Codex JSON (no-match => nothing). Never wedge.
      const input = JSON.stringify({ prompt: `a prompt longer than twenty characters for the gate ${RUN_MARKER}`, cwd: TEST_PROJECT, session_id: RUN_MARKER });
      const r = spawnSync('bash', [path.join(ACTIVE_PLUGIN, 'scripts', 'inject-context-hook.sh')], {
        input, env: ENV, encoding: 'utf8', timeout: 120000,
      });
      if (r.status !== 0) throw new Error(`exit ${r.status}: ${r.stderr.slice(0, 300)}`);
      if (r.stdout.trim()) {
        const j = JSON.parse(r.stdout.split('\n')[0]);
        if (!j.hookSpecificOutput || j.hookSpecificOutput.hookEventName !== 'UserPromptSubmit') {
    ...
    await step('SessionEnd handler: empty rollout skips worker (no completion marker)', () => {
      const tp = path.join(TMP, 'rollout-empty.jsonl');
      fs.writeFileSync(tp, JSON.stringify({ type: 'session_meta', timestamp: 't', payload: { id: RUN_MARKER, session_id: RUN_MARKER, cwd: TEST_PROJECT, source: 'cli' } }) + '\n');
      const r = spawnSync(process.execPath, [path.join(ACTIVE_PLUGIN, 'scripts', 'session-end-hook.js')], {
        input: JSON.stringify({ transcript_path: tp, session_id: RUN_MARKER, cwd: TEST_PROJECT }),
        env: ENV, encoding: 'utf8', timeout: 60000,
      });
      if (r.status !== 0) throw new Error(`exit ${r.status}`);
      if (!r.stderr.includes('empty rollout')) throw new Error(`expected empty-rollout guard log, got: ${r.stderr.slice(0, 200)}`);
  ...
  if (TIER === 'authenticated') {
    ...
    if (!fs.existsSync(authSrc)) throw new Error('authenticated tier requires ~/.codex/auth.json');
...
let input=''; process.stdin.on('data',d=>input+=d); process.stdin.on('end',()=>{
  fs.appendFileSync(${JSON.stringify(canaryLog)}, JSON.stringify({event:${JSON.stringify(event)},stdin:JSON.parse(input||'{}')})+'\\n');
    ...
    fs.writeFileSync(path.join(ACTIVE_PLUGIN, 'scripts', 'version-drift-check.js'), canary('SessionStart'));
    fs.writeFileSync(path.join(ACTIVE_PLUGIN, 'scripts', 'session-start-maintenance.js'), canary('SessionStart'));
    fs.writeFileSync(path.join(ACTIVE_PLUGIN, 'scripts', 'session-end-hook.js'), canary('SessionEnd'));
    ...
    const origSync = fs.readFileSync(syncCanaryPath, 'utf8');
    fs.writeFileSync(syncCanaryPath, `#!/usr/bin/env node\nimport fs from 'node:fs';\nlet i='';process.stdin.on('data',d=>i+=d);process.stdin.on('end',()=>{fs.appendFileSync(${JSON.stringify(canaryLog)}, JSON.stringify({event:"SessionStart",stdin:JSON.parse(i||"{}")})+"\\n")});\n`);
    const hookJson = JSON.stringify({ continue: true, hookSpecificOutput: { hookEventName: "UserPromptSubmit", additionalContext: nonce } });
    fs.writeFileSync(path.join(ACTIVE_PLUGIN, 'scripts', 'inject-context-hook.sh'),
    ...
    fs.chmodSync(path.join(ACTIVE_PLUGIN, 'scripts', 'inject-context-hook.sh'), 0o755);
    await step('real codex exec fires SessionStart/UserPromptSubmit/SessionEnd', async () => {
      fs.rmSync(canaryLog, { force: true });
      const workdir = path.join(TMP, 'work');
      fs.mkdirSync(workdir, { recursive: true });
      const r = spawnSync('codex', ['exec', '--skip-git-repo-check', '--dangerously-bypass-hook-trust',
        '-C', workdir, 'If you have additional context, repeat verbatim the text it contains. Otherwise say NONE.'],
        { env: ENV, encoding: 'utf8', timeout: 180000 });
      if (r.status !== 0) throw new Error(`codex exec failed: ${(r.stderr || '').slice(-300)}`);
      const events = fs.existsSync(canaryLog)
        ? fs.readFileSync(canaryLog, 'utf8').trim().split('\n').map((l) => JSON.parse(l))
        : [];
      const names = events.map((e) => e.event).sort();
      for (const expected of ['SessionStart', 'SessionEnd', 'UserPromptSubmit']) {
        if (!names.includes(expected)) throw new Error(`missing canary event ${expected}; got ${names}`);
      }
      const ups = events.find((e) => e.event === 'UserPromptSubmit');
      if (!ups.stdin.prompt || typeof ups.stdin.prompt !== 'string') throw new Error('UserPromptSubmit stdin lacked prompt');
      if (!ups.stdin.session_id) throw new Error('UserPromptSubmit stdin lacked session_id');
      if (ups.stdin.prompt.includes(nonce)) throw new Error('nonce leaked into user prompt (false positive risk)');
      const reply = (r.stdout || '').trim();
      if (!reply.includes(nonce)) throw new Error(`additionalContext not injected; reply: ${reply.slice(0, 200)}`);
      return `events=${names.join(',')} injection=observed nonce=${nonce.slice(0,16)}...`;
    })
    await step('real SessionEnd extraction through Luna (one fact)', async () => {
      // Restore REAL handlers, then drive session-end-hook with a stable
      // transcript containing one obvious decision.
      fs.rmSync(canaryLog, { force: true });
      fs.cpSync(path.join(REPO, 'scripts'), path.join(ACTIVE_PLUGIN, 'scripts'), { recursive: true });
      const sessDir = path.join(CODEX_HOME, 'sessions', '2026', '08', '26');
      fs.mkdirSync(sessDir, { recursive: true });
      const sid = '01a039aa-1111-4222-8333-a44445555666';
      const rollout = path.join(sessDir, `rollout-2026-08-26T01-00-00-${sid}.jsonl`);
      fs.writeFileSync(rollout, [
        JSON.stringify({ timestamp: '2026-08-26T01:00:00Z', type: 'session_meta', payload: { id: sid, session_id: sid, timestamp: '2026-08-26T01:00:00Z', cwd: TEST_PROJECT, originator: 'codex-tui', cli_version: '0.149.1', source: 'cli' } }),
        JSON.stringify({ type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: `We decided the API will use cursor pagination with encrypted opaque cursors. Test marker: ${RUN_MARKER}` }] } }),
        JSON.stringify({ type: 'response_item', payload: { type: 'message', role: 'assistant', content: [{ type: 'output_text', text: `Understood: cursor pagination with encrypted opaque cursors is the chosen approach. Marker ${RUN_MARKER}.` }] } }),
      ].join('\n') + '\n');
      ...
      const syncR = MB(['sync']);
      if (syncR.status !== 0) throw new Error(`sync failed: ${(syncR.stderr || '').slice(-300)}`);
      const r = spawnSync(process.execPath, [path.join(ACTIVE_PLUGIN, 'scripts', 'session-end-hook.js')], {
        input: JSON.stringify({ transcript_path: rollout, session_id: sid, cwd: TEST_PROJECT }),
        env: { ...ENV, MEMORY_BANK_STABILIZE_QUIET_MS: '200', MEMORY_BANK_STABILIZE_POLL_MS: '50', BACKFILL_MIN_EXCHANGES: '1' },
      ...
      if (/no completion evidence/.test(r.stderr)) {
        throw new Error(`hook reported no completion evidence: ${r.stderr.slice(-300)}`);
      }
      let factCount = 0;
      try {
        ...
        db.close();
      } catch { /* DB not created yet counts as not visible */ }
      if (factCount === 0) throw new Error('no project-scoped active fact found in temp DB after SessionEnd');
  ...
  if (TIER === 'authenticated') {
    await step('next-session MCP search_facts returns the freshly extracted fact', async () => {
      process.env.TEST_DB_PATH = path.join(MB_HOME, 'conversation-index', 'db.sqlite');
      ...
      if (reply.isError) throw new Error(`search_facts error: ${reply.content[0].text.slice(0, 200)}`);
  ...
  await step('remove-hooks removes only owned entries', () => {
    ...
    j.hooks.PreToolUse = [{ matcher: '^Bash$', hooks: [{ command: 'atuin hook codex', type: 'command' }] }];
    fs.writeFileSync(file, JSON.stringify(j, null, 2) + '\n');
    const r = MB(['remove-hooks']);
    if (r.status !== 0) throw new Error(r.stderr);
    const afterJ = JSON.parse(fs.readFileSync(file, 'utf8'));
    if (!afterJ.hooks?.PreToolUse?.[0]?.hooks?.[0]?.command) throw new Error('foreign entry removed!');
    if (afterJ.hooks.SessionStart || afterJ.hooks.UserPromptSubmit || afterJ.hooks.SessionEnd) {
  ...
  await step('remove isolated plugin and marketplace registrations', () => {
    ...
    if (r.status !== 0) throw new Error(r.stderr || r.stdout);
    const plugins = JSON.parse(CODEX(['plugin', 'list', '--json']).stdout).installed || [];
    const markets = JSON.parse(CODEX(['plugin', 'marketplace', 'list', '--json']).stdout).marketplaces || [];
    if (plugins.some((plugin) => plugin.pluginId === PLUGIN_ID)) throw new Error('plugin registration survived remove');
    if (markets.some((market) => market.name === MARKET_NAME)) throw new Error('marketplace registration survived remove');
  ...
  await step('cleanup inventory: 7-surface exact check', () => {
    ...
    const pluginRegistrationAbsent = pluginList.status === 0
      && !(JSON.parse(pluginList.stdout).installed || []).some((plugin) => plugin.pluginId === PLUGIN_ID)
      && marketplaceList.status === 0
      && !(JSON.parse(marketplaceList.stdout).marketplaces || []).some((market) => market.name === MARKET_NAME);
    if (!pluginRegistrationAbsent) errors.push('isolated plugin or marketplace registration remains');

    // 2. Owned hooks are absent while the planted foreign hook remains.
    const tempHooksPath = path.join(CODEX_HOME, 'hooks.json');
    let hooks = null;
    try { hooks = JSON.parse(fs.readFileSync(tempHooksPath, 'utf8')); } catch { errors.push('temp hooks.json unreadable'); }
    const ownedHooksAbsent = hooks !== null
      && !JSON.stringify(hooks).includes('_memoryBank')
      && Boolean(hooks.hooks?.PreToolUse?.[0]?.hooks?.[0]?.command);
    if (!ownedHooksAbsent) errors.push('owned hooks remain or foreign hook was lost');
    ...
    const testWorkers = (ps.stdout || '').split('\n').filter((line) =>
      line.includes(TMP) || line.includes(MB_HOME) || line.includes(CODEX_HOME),
    );
    const testWorkersZero = testWorkers.length === 0;
    if (!testWorkersZero) errors.push(`test-owned workers remain: ${testWorkers.join('; ').slice(0, 300)}`);

    const socketOwners = fs.existsSync(TEMP_SOCKET)
      ? spawnSync('lsof', [TEMP_SOCKET], { encoding: 'utf8' })
      : { status: 1, stdout: '' };
    const testListenersZero = socketOwners.status !== 0 || !(socketOwners.stdout || '').trim();
    if (!testListenersZero) errors.push(`test socket still has a live listener: ${socketOwners.stdout.slice(0, 300)}`);
    ...
    const afterHooks = fs.existsSync(USER_HOOKS) ? sha256(USER_HOOKS) : 'ABSENT';
    const afterConfig = fs.existsSync(USER_CONFIG) ? sha256(USER_CONFIG) : 'ABSENT';
    const afterUserPluginList = spawnSync('codex', ['plugin', 'list', '--json'], { encoding: 'utf8' }).stdout;
    const afterUserMarketplaceList = spawnSync('codex', ['plugin', 'marketplace', 'list', '--json'], { encoding: 'utf8' }).stdout;
    const userEnvironmentIsolated = afterHooks === before.hooks
    ...
    if (!userEnvironmentIsolated) errors.push('stable user config/registry changed or test marker leaked into real user data');
  ...
  if (cleanupReceipt) console.log(`__CLEANUP_JSON__${JSON.stringify(cleanupReceipt)}`);
  if (!KEEP) fs.rmSync(TMP, { recursive: true, force: true });
  else console.log(`Kept isolated outer root for diagnosis: ${TMP}`);
