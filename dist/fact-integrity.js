...
const tableExists = (db, name) => !!db.prepare("SELECT 1 FROM sqlite_master WHERE name = ? AND type = 'table'").get(name);
...
export function auditMemoryIntegrity(db) {
    const findings = [];
    const add = (code, disposition, target, evidence) => {
        findings.push({ id: fingerprint([code, target, evidence]), code, disposition, target, evidence });
    };
    const rows = db.prepare('SELECT * FROM facts ORDER BY id').all();
    for (const row of rows) {
        ...
        if (tombstone?.reason === 'source_conversation_excluded')
            add('privacy-resurrection', 'repairable', target, { ...state, tombstone });
        ...
        if (!sourceIds) {
            add('missing-source', 'review', target, { ...state, reason: 'malformed lineage JSON' });
            continue;
        }
        ...
        if (missing.length)
            add('missing-source', 'review', target, { ...state, sourceIds: missing });
        const workstreams = [...new Set(sources.flatMap(source => source?.workstream_id ? [source.workstream_id] : []))].sort();
        if (workstreams.length > 1)
            add('mixed-workstream-lineage', 'review', target, { ...state, workstreamIds: workstreams, sourceIds });
        ...
        if (['decision', 'project-current'].includes(fact.promotion_state ?? '') && workstreams.length > 0) {
            const promotion = db.prepare("SELECT detail_json FROM project_identity_audit WHERE reason = 'explicit fact placement' ORDER BY created_at DESC").all();
            const hasPromotion = promotion.some(row => {
                try {
                    const detail = JSON.parse(row.detail_json);
                    return detail.factId === fact.id && detail.promotionState === fact.promotion_state;
                }
                catch {
                    return false;
                }
            ...
            if (!hasPromotion)
                add('unproven-promotion', 'review', target, { ...state, workstreamIds: workstreams, reason: 'no recorded explicit promotion action; inspect original evidence and revisions' });
        }
        ...
        const unverified = revisions.filter(revision => {
            try {
                return !JSON.parse(revision.outcome_json ?? '{}').verified_input_fact_id;
            }
            catch {
                return true;
            }
        ...
        if (unverified.length)
            add('semantic-revalidation-needed', 'review', target, { ...state, revisionIds: unverified.map(revision => revision.id), reason: 'historical automatic or unknown-origin rewrite; semantic overreach is NOT_PROVEN until original evidence review' });
    }
    for (const table of ['vec_facts', 'vec_facts_kr'])
        if (tableExists(db, table)) {
            const orphans = db.prepare(`SELECT v.id FROM ${table} v LEFT JOIN facts f ON f.id = v.id WHERE f.id IS NULL OR f.is_active = 0 ORDER BY v.id`).all();
            for (const row of orphans)
                add('orphan-vector', 'repairable', { table, id: row.id }, { reason: 'vector belongs to absent or inactive fact' });
        }
    if (tableExists(db, 'ontology_relations')) {
      ...
      WHERE a.id IS NULL OR b.id IS NULL OR a.is_active = 0 OR b.is_active = 0 ORDER BY r.id`).all();
        for (const row of orphans)
            add('orphan-relation', 'repairable', { table: 'ontology_relations', id: String(row.id) }, { sourceId: row.source_fact_id, targetId: row.target_fact_id, fingerprint: fingerprint(row) });
    }
    if (tableExists(db, 'fact_context_dependencies')) {
      ...
      WHERE f.id IS NULL OR e.id IS NULL ORDER BY c.fact_id, c.exchange_id, c.dependency_kind`).all();
        for (const row of orphans)
            add('orphan-context', 'repairable', { table: 'fact_context_dependencies', id: String(row.fact_id),
...
export function applyIntegrityRepairs(db, report, selectedIds) {
    if (report.version !== 1 || report.planId !== fingerprint(report.findings))
        throw new Error('invalid integrity preview fingerprint');
    const selected = [...new Set(selectedIds)].map(id => {
        const finding = report.findings.find(f => f.id === id);
        if (!finding || finding.disposition !== 'repairable')
            throw new Error('selection must name a repairable finding from the preview');
        return finding;
    });
    const tx = db.transaction(() => {
      ...
      target_json TEXT NOT NULL, evidence_json TEXT NOT NULL, applied_at TEXT NOT NULL
        ...
        const result = { applied: [], alreadyApplied: [] };
        for (const finding of selected) {
            if (db.prepare('SELECT 1 FROM fact_integrity_repairs WHERE finding_id = ?').get(finding.id)) {
                if (current.has(finding.id))
                    throw new Error('previously repaired damage has recurred; create a fresh recovery review');
                result.alreadyApplied.push(finding.id);
                continue;
            }
            if (!current.has(finding.id))
                throw new Error('integrity preview is stale; audit again before applying');
            const { target } = finding;
            if (finding.code === 'privacy-resurrection')
                hardDeleteFact(db, target.id, { confirm: true });
            ...
            else if (finding.code === 'orphan-relation')
                db.prepare('DELETE FROM ontology_relations WHERE id = ?').run(target.id);
            else if (finding.code === 'orphan-context')
                db.prepare('DELETE FROM fact_context_dependencies WHERE fact_id = ? AND exchange_id = ? AND dependency_kind = ?')
                    .run(target.id, target.exchangeId, target.dependencyKind);
            else
                throw new Error('unsupported integrity repair');
        ...
        if (after.findings.some(f => result.applied.includes(f.id)))
            throw new Error('repair postcondition failed');
        return result;
    });
    return tx.immediate();
}
