...
function cleanString(value, max = 4_096) {
    return typeof value === "string" ? value.slice(0, max) : "";
}
function optionalString(value, max = 4_096) {
...
export function normalizeHookPayload(input) {
    ...
    const hookEventName = cleanString(value.hook_event_name ?? value.hookEventName ?? value.event_name, 64).trim();
    if (!hookEventName)
        throw new Error("missing hook_event_name");
    const transcriptPath = optionalString(value.transcript_path ?? value.transcriptPath, 16_384);
    return {
        sessionId,
        transcriptPath,
...
export function validateTranscriptPath(candidate) {
    if (!path.isAbsolute(candidate))
        throw new Error("transcript_path must be absolute");
    const resolved = path.resolve(candidate);
    const lst = fs.lstatSync(resolved);
    if (lst.isSymbolicLink())
        throw new Error("transcript_path symlink is not allowed");
    const realpath = fs.realpathSync(resolved);
    const stat = fs.statSync(realpath);
    if (!stat.isFile())
        throw new Error("transcript_path is not a regular file");
    const allowed = allowedTranscriptRoots().some((root) => realpath === root || realpath.startsWith(`${root}${path.sep}`));
    if (!allowed)
        throw new Error("transcript_path is outside allowed session roots");
    return { path: resolved, realpath, stat };
}
function readCanonicalSessionMeta(transcriptPath) {
...
function journalFile(sessionId, streamEpoch) {
    return path.join(getMemexHome(), "journals", sessionId, `${streamEpoch}.jsonl`);
}
function recordCaptureGap(db, input) {
    const now = input.now ?? new Date().toISOString();
    const gapId = stableId("capture-gap", input.sessionId, input.streamEpoch ?? "", input.sourcePath ?? "", input.eventKind, input.reason);
    db.prepare(`
    INSERT OR IGNORE INTO capture_gaps
      (gap_id, session_id, stream_epoch, source_path, event_kind, reason, state, created_at)
    VALUES (?, ?, ?, ?, ?, ?, 'open', ?)
  `).run(gapId, input.sessionId, input.streamEpoch ?? null, input.sourcePath ?? null, input.eventKind, input.reason.slice(0, 1_000), now);
    return gapId;
}
function recoverContinuitySession(db, sessionId) {
...
function captureTranscriptPrefixInTransaction(db, input) {
  ...
  `).get(input.sessionId);
    ...
    const replaced = !!previous && (String(previous.source_realpath) !== source.realpath ||
        String(previous.source_dev) !== sourceDev ||
        String(previous.source_ino) !== sourceIno ||
        source.stat.size < Number(previous.copied_byte_end) ||
    ...
    `).run(input.sessionId, streamEpoch, source.path, source.realpath, sourceDev, sourceIno, source.stat.mtimeMs, sourceGuardStart, sourceGuardHash, sourceThroughByte, throughLine, journalThroughByte, journalPath, prefixHash, CONTINUITY_PARSER_VERSION, now, now);
      ...
      `).get(input.sessionId, streamEpoch).n;
            db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).run(blockId, input.sessionId, streamEpoch, blockOrdinal, copiedByteEnd, sourceThroughByte, journalByteEnd, journalThroughByte, copiedLineEnd + 1, throughLine, segmentHash, prefixHash, now);
        }
    ...
    return {
        checkpointId,
        captureIndexJobId,
        capsuleJobId: db.prepare("SELECT job_id FROM memory_jobs WHERE job_id = ?").get(capsuleJobId) ? capsuleJobId : null,
...
function rememberEpochMarker(db, sessionId, markerId, now) {
  ...
  `).run(sessionId, markerId, now);
    ...
    const cutoff = new Date(Date.parse(now) - CAPTURE_GAP_MARKER_MAX_AGE_MS).toISOString();
    db.prepare(`
...
export function recordResidentFactRevisions(db, sessionId, contextEpoch, revisions, now = new Date().toISOString()) {
 ...
 */
export function readResidentRevisionCorrections(db, sessionId) {
  ...
  `);
    ...
    for (const [id, semantic, lifecycle] of resident) {
        ...
        const prior = row.is_active === 1
            ? previous.get(id)?.previous_fact ?? null
            : null;
        corrections.push({
            ...row,
 ...
 */
function capItems(values, field, drops) {
...
function cleanList(values, field, drops) {
    ...
    return capItems(values, field, drops).map((value) => {
        ...
        if (text.length > 500)
            throw new Error(`${field} contains overlong text`);
        return text;
    });
}
function cleanEvidenceItem(value, field) {
    ...
    const sources = item.sourceExchangeIds;
    if (!text || sources.length === 0)
        throw new Error(`${field} requires text and sources`);
 ...
 */
function cleanEvidence(values, field) {
    if (!Array.isArray(values))
        throw new Error(`${field} must be an array`);
    const kept = [];
...
function parseTruncationRecord(raw) {
    ...
    const fields = Array.isArray(record.fields)
        ? record.fields.filter((field) => typeof field === "string")
        : [];
    const itemCaps = {};
    if (record.itemCaps && typeof record.itemCaps === "object" && !Array.isArray(record.itemCaps)) {
        for (const [field, value] of Object.entries(record.itemCaps)) {
            if (!value || typeof value !== "object" || Array.isArray(value))
                continue;
            const cap = value;
            if (typeof cap.kept !== "number" || typeof cap.dropped !== "number")
                continue;
...
function parseWorkCapsulePatchStructure(value) {
    ...
    if (!Array.isArray(input.carryFactRevisions) || input.carryFactRevisions.length > 64 ||
        input.carryFactRevisions.some((entry) => !Array.isArray(entry) || entry.length !== 3 || typeof entry[0] !== "string" ||
    ...
    const carry = input.carryFactRevisions;
 ...
 */
function finishWorkCapsulePatch(parsed) {
    ...
    const itemCapDrops = { ...parsed.itemCapDrops };
    const capEvidence = (items, field) => {
        const dropped = Math.max(0, items.length - MAX_ARRAY_ITEMS);
        if (dropped > 0)
            itemCapDrops[field] = (itemCapDrops[field] ?? 0) + dropped;
 ...
 * still holds.
 */
function assertCapsuleEvidenceSourcesDeclared(patch) {
 ...
 */
function normalizeCapsuleCarryOverSources(patch, previousSources, page) {
    ...
    if (carried.size === 0)
        return patch;
    const keep = (ids) => ids.filter((id) => !carried.has(id));
    const keepClaims = (items) => items
...
export function applyWorkCapsulePatch(db, input) {
    ...
    const tx = db.transaction(() => {
        ...
        WHERE job_id = ? AND kind = 'capsule_update' AND checkpoint_id = ?
          ...
          AND lease_until > ?
      `).get(input.jobLease.jobId, input.throughCheckpointId, input.jobLease.owner, input.jobLease.leaseGeneration, now);
            if (!owned)
                return null;
        }
...
export function buildDeterministicTailBaton(db, input) {
  ...
  `).all(input.sessionId);
    const touched = [...new Set(tools.flatMap((tool) => (tool.tool_result ?? "").match(/(?:^|\s)([A-Za-z0-9_.-]+\/[A-Za-z0-9_./-]+)/g) ?? []))]
...
function renderCapsule(capsule, maxChars, options = {}) {
  ...
  `);
    ...
    if (capsule) {
      ...
      WHERE checkpoint_id = ? AND kind = 'capture_index' AND state <> 'completed' LIMIT 1`)
            .get(state.latest_checkpoint_id ?? null);
        if (captureJob)
            capsuleStaleReasons.push(`capture/index job is ${captureJob.state}`);
      ...
      ORDER BY j.updated_at DESC, j.rowid DESC LIMIT 1`).get(`workstream:${state.workstream_id}`, capsule.throughCheckpointId, capsule.throughCheckpointId, state.workstream_id);
        if (capsuleJob)
            capsuleStaleReasons.push(`Capsule update job is ${capsuleJob.state}`);
 ...
 */
export function applyPendingEpochAdvance(db, sessionId) {
    let applied = 0;
    try {
 ...
 */
export function applyLatestLifecycleClosure(db, sessionId) {
    const tx = db.transaction(() => {
...
function applyCheckpointClosure(db, sessionId) {
    const checkpoint = db.prepare(`
  ...
  `).get(sessionId, checkpoint.through_line);
    if (!exchange || exchange.closure_state === checkpoint.closure_state)
        return false;
  ...
  `).run(checkpoint.closure_state, exchange.id, exchange.closure_state).changes === 1;
}
