...
export async function analyzeHistory(options = {}) {
    ...
    `).get();
      ...
      `).get();
            ...
            const { sql: pendingSql, params: pendingParams } = pendingExtractionCoreQuery(getExtractionConfig(), tables.has('exchange_extraction_state') && tables.has('extraction_targets')
                ? 'continuity'
                : 'legacy');
            try {
