...
export function exchangeContentHash(userMessage, assistantMessage, toolNames) {
 ...
 */
export function commitExchangeReembed(db, id, expectedContentHash, embedding, embeddingVersion, lastIndexed) {
    const tx = db.transaction(() => {
        const row = db.prepare('SELECT user_message, assistant_message FROM exchanges WHERE id = ?').get(id);
        if (!row)
            return false;
        const toolNames = db.prepare('SELECT tool_name FROM tool_calls WHERE exchange_id = ? ORDER BY rowid')
            .all(id).map((t) => t.tool_name);
        if (exchangeContentHash(row.user_message, row.assistant_message, toolNames) !==
            expectedContentHash) {
            return false;
        }
