...
function reclaimMutexIsDead(mutexPath) {
    let text;
    try {
 ...
 */
function acquireReclaimMutex(lockPath) {
    ...
    try {
        ...
        try {
            ...
            if (error.code !== "EEXIST")
                return null;
            if (!reclaimMutexIsDead(mutexPath))
                return null;
            try {
                fs.unlinkSync(mutexPath);
            }
            catch { /* another reclaimer broke it first */ }
            try {
                fs.linkSync(staging, mutexPath);
            }
            catch {
...
function releaseReclaimMutex(lockPath, payload) {
    const mutexPath = reclaimMutexPath(lockPath);
    if (!holdsOurLock(mutexPath, payload))
        return;
    try {
        fs.unlinkSync(mutexPath);
    }
        ...
        try {
            ...
            try {
                ...
                if (!lockStillAbandoned(lockPath, stamp, attributable ? holder : -1))
                    continue;
                if (reclaimInterleave)
                    await reclaimInterleave(); // test seam only
...
export async function clearQuarantine(patternId, opts = {}) {
    ...
    if (dryRun)
        return { cleared: dropped.length, ids, dryRun: true };
    replaceQuarantine(keep);
