 ...
 */
function clearQuarantineForChangedPatterns(overlay, doc) {
    const live = new Set((doc?.patterns?.add ?? []).map((pattern) => quarantineKey(pattern.id, patternSourceSha8(pattern.source, pattern.flags ?? ""))));
...
export async function applyOverlayChange(overlay, input, opts) {
    ...
    return withOverlayLock(file, async () => {
        ...
        const cleared = clearQuarantineForChangedPatterns(overlay, overlay === "recall-gate" ? applied : null);
