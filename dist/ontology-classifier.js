...
import { MAX_CLASSIFY_ATTEMPTS, ONTOLOGY_STATE_PARKED, buildOntologyParkedRetryClause, ontologyParkToken, } from './ontology-selector.js';
 ...
 */
export function recordOntologyIndexRepairBlocked(db, blocked, detail) {
    try {
        db.prepare(`
...
export function clearOntologyIndexRepairBlocked(db) {
    try {
        db.prepare(`
 ...
 */
function tryDeterministicAssign(db, fact, hits, expectedTaxonomyEpoch) {
    const top = hits[0];
    if (!top)
        return null;
...
async function applyClassification(db, factId, parsed, expectedSemanticGeneration, expectedTaxonomyEpoch) {
    ...
    const commit = db.transaction(() => {
        ...
        if (expectedTaxonomyEpoch !== undefined && getTaxonomyEpoch(db) !== expectedTaxonomyEpoch) {
            throw new StaleFactMutationError(`ontology classification discarded: taxonomy was invalidated during classification (fact ${factId})`);
        }
 ...
 */
export function persistFallbackClassification(db, factId, expectedSemanticGeneration, expectedTaxonomyEpoch) {
    ...
    const result = expectedSemanticGeneration === undefined
        ? db.prepare(`UPDATE facts SET ontology_category_id = ?, updated_at = ?,
             ontology_state = '${ONTOLOGY_STATE_PARKED}', ontology_parked_at = ?, ontology_parked_version = ?
           WHERE id = ? AND ontology_category_id IS NULL
             AND COALESCE(ontology_attempts, 0) >= ?`).run(fallback.categoryId, now, now, parkToken, factId, MAX_CLASSIFY_ATTEMPTS)
        : db.prepare(`UPDATE facts SET ontology_category_id = ?, updated_at = ?,
             ontology_state = '${ONTOLOGY_STATE_PARKED}', ontology_parked_at = ?, ontology_parked_version = ?
           WHERE id = ? AND semantic_generation = ? AND ontology_category_id IS NULL
             AND COALESCE(ontology_attempts, 0) >= ?`).run(fallback.categoryId, now, now, parkToken, factId, expectedSemanticGeneration, MAX_CLASSIFY_ATTEMPTS);
    if (result.changes === 0) {
 ...
 */
export async function classifyFactToOntology(db, fact, modelContext) {
 ...
 */
async function classifyFactsBatchInternal(db, facts, options = {}) {
    const deterministic = [];
    const remaining = [];
     ...
     * 않는다.
     */
    const runModelLane = async (subset) => {
        if (subset.length === 0)
            return;
        ...
        try {
            ...
            if (errorCode === 'MEMEX_MODEL_OUTPUT_LIMIT' || errorCode === 'MEMEX_MODEL_OUTPUT_SCHEMA') {
                if (subset.length > 1) {
                    ...
                    console.error(`Batch classification output budget exceeded for ${subset.length} facts — splitting into ${mid}/${subset.length - mid} and retrying (no attempt burned)`);
 ...
 */
export async function classifyFactsBatch(db, facts, options = {}) {
 ...
 */
export function releaseParkedFact(db, factId) {
    const parkToken = ontologyParkToken(EMBEDDING_VERSION);
    return db
        .prepare(`UPDATE facts SET ontology_category_id = NULL, ontology_attempts = 0,
...
export function countParkedRetryable(db) {
    const { clause, params } = buildOntologyParkedRetryClause({ embeddingVersion: EMBEDDING_VERSION, alias: 'f' });
    const row = db.prepare(`SELECT COUNT(*) AS n FROM facts f WHERE ${clause}`).get(...params);
...
async function detectRelationsInternal(db, newFact, topK = 2, modelContext) {
    const context = getModelWorkContext();
    ...
    for (const { fact: existingFact } of candidates) {
        ...
        try {
            const response = await withModelWorkContext({ stage: 'relation', targetId: newFact.id }, () => callMemoryModel(DETECT_RELATION_SYSTEM_PROMPT, prompt, 256, {
                modelContext,
            }));
    ...
    if (budgetId && allSettled)
        settlePending();
}
export async function detectRelations(db, newFact, topK = 2, modelContext) {
    return withResolvedModelWorkContext({
        ...modelContext,
        db,
        stage: 'relation',
...
export async function backfillRelationBatch(db, factIds, options = {}) {
...
export async function classifyAndLinkFact(db, factId, embedding, modelContext) {
