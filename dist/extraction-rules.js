 ...
 */
export const UI_BADGE_RESERVED_IDS = Object.freeze([
    "active",
    "ASSERTED",
    "cancelled",
    "cancelling",
...
export const RESERVED_FACT_KIND_IDS = new Set([
    ...BUILTIN_FACT_KINDS,
...
export function emptyLoadedExtractionRules() {
    return EMPTY_RULES;
}
/**
 ...
 */
export function emptyExtractionRulesDoc() {
    return {
 ...
 */
export function extractionRulesDocHash(doc) {
    const overrides = doc.project_overrides ?? {};
    const rules = {
        ...ruleSetForHash(doc),
        project_overrides: Object.fromEntries(Object.keys(overrides).sort().map((key) => [key, ruleSetForHash(overrides[key])])),
    };
    return `rules:${sha8(canonicalJson(rules))}`;
}
/** The applied hash, or null when nothing is applied. Cache-aware. */
export function extractionRulesHash() {
 ...
 */
export function composeEffectivePolicyVersion(basePolicyVersion, rulesHash) {
...
/* -------------------------------------------------------------------------- */
const CONTROL_CHARS = /[\u0000-\u001f\u007f]/;
const SCOPES = ["fact_text", "evidence", "both"];
        ...
        if (!Array.isArray(topicsRaw)) {
            ...
            topicsRaw.forEach((topic, index) => {
                ...
                if (typeof topic !== "string" ||
                    ...
                    emit("error", "TOPIC_INVALID", `a topic must be ${excludeTopicChars.min}-${excludeTopicChars.max} characters on a single line`, { path: where, row: index, params: { limit: excludeTopicChars } });
                    return;
                }
        ...
        if (!Array.isArray(patternsRaw)) {
            ...
            patternsRaw.forEach((entry, index) => {
                ...
                if (scopeRaw !== undefined) {
                    if (typeof scopeRaw !== "string" || !SCOPES.includes(scopeRaw)) {
...
function parseCustomFactKinds(raw, prefix, emit) {
    ...
    raw.forEach((entry, index) => {
        ...
        if (RESERVED_FACT_KIND_IDS.has(id)) {
            ...
            const builtin = BUILTIN_FACT_KINDS.includes(id);
            emit("error", "KIND_ID_RESERVED", builtin
...
function parsePattern(entry, where, index, emit, seenIds, kind) {
    ...
    if (rejected)
        return null;
    const id = typeof item.id === "string" && item.id.length > 0 ? item.id : userPatternId(kind, source, flags);
    if (seenIds.has(id)) {
    ...
    if (note !== undefined &&
        ...
        emit("error", "NOTE_INVALID", `a note must be a single line of at most ${EXTRACTION_RULES_LIMITS.noteChars} characters`, { path: `${where}.note`, row: index, field: "note", params: { limit: EXTRACTION_RULES_LIMITS.noteChars } });
        return null;
    }
 ...
 */
export function validateExtractionRulesDoc(raw, opts = {}) {
    const issues = [];
    ...
    if (opts.bytes !== undefined && opts.bytes > EXTRACTION_RULES_LIMITS.fileBytes) {
        emit("error", "OVERLAY_TOO_LARGE", `the overlay file is ${opts.bytes} bytes (limit ${EXTRACTION_RULES_LIMITS.fileBytes})`, { params: { bytes: opts.bytes, limit: EXTRACTION_RULES_LIMITS.fileBytes } });
    ...
    if (doc.schema !== EXTRACTION_RULES_OVERLAY_SCHEMA) {
        emit("error", "OVERLAY_SCHEMA_MISMATCH", `schema must be "${EXTRACTION_RULES_OVERLAY_SCHEMA}" (found ${JSON.stringify(doc.schema ?? null)})`, { path: "schema", params: { expected: EXTRACTION_RULES_OVERLAY_SCHEMA } });
        return { ok: false, issues, doc: null };
    }
    if (Number(doc.version) !== EXTRACTION_RULES_OVERLAY_VERSION) {
        ...
        emit("error", "OVERLAY_VERSION_UNSUPPORTED", `version ${String(doc.version)} is not supported by this build (expected ${EXTRACTION_RULES_OVERLAY_VERSION})`, { path: "version", params: { version: doc.version, expected: EXTRACTION_RULES_OVERLAY_VERSION } });
        return { ok: false, issues, doc: null };
    }
    ...
    if (overridesRaw !== undefined) {
        if (typeof overridesRaw !== "object" || overridesRaw === null || Array.isArray(overridesRaw)) {
            ...
            for (const [project, value] of entries) {
                ...
                // would be nonsense.
                overrides[project] = validateRuleSet(value, where, emit, new Set());
            }
        }
    }
 ...
 */
export async function validateExtractionRules(doc, opts = {}) {
    ...
    const all = [
        ...
        ...Object.entries(structural.doc.project_overrides ?? {}).flatMap(([project, rules]) => collectRuleSetPatterns(rules).map((pattern) => ({ ...pattern, project }))),
    ];
    for (const pattern of all) {
        ...
        if (!probe.ok) {
            issues.push(overlayIssue("error", "PATTERN_TOO_SLOW", `validation did not finish within the ${PROBE_WALL_MS} ms limit`, { path: pattern.path, params: { id: pattern.id, limitMs: PROBE_WALL_MS } }));
        }
    }
    const failed = issues.some((issue) => issue.severity === "error");
    return { ok: !failed, issues, doc: failed ? null : structural.doc };
}
function collectRuleSetPatterns(rules) {
    return [
...
function readValidate(file) {
    ...
    for (const pattern of allNeverExtract(doc)) {
        ...
        issues.push(overlayIssue("error", "PATTERN_QUARANTINED", `never_extract pattern ${pattern.id} exceeded the ${MATCH_WALL_MS} ms match budget — ` +
            "extraction is HELD until it is fixed or cleared", { path: "never_extract_patterns", params: { id: pattern.id, limitMs: MATCH_WALL_MS } }));
    }
    ...
    for (const pattern of allDecisionHints(doc)) {
        ...
        quarantined.push(row);
        issues.push(overlayIssue("warning", "PATTERN_QUARANTINED", `decision hint ${pattern.id} exceeded the ${MATCH_WALL_MS} ms match budget and is not applied`, { path: "always_treat_as_decision_patterns", params: { id: pattern.id, limitMs: MATCH_WALL_MS } }));
    }
...
function allNeverExtract(doc) {
    return [
        ...(doc.never_extract_patterns ?? []),
        ...Object.values(doc.project_overrides ?? {}).flatMap((rules) => rules.never_extract_patterns ?? []),
    ];
}
function allDecisionHints(doc) {
    return [
        ...(doc.always_treat_as_decision_patterns ?? []),
        ...Object.values(doc.project_overrides ?? {}).flatMap((rules) => rules.always_treat_as_decision_patterns ?? []),
    ];
}
/**
 ...
 */
export function reloadExtractionRulesIfChanged() {
...
export function currentExtractionRulesRevision() {
...
export function resetExtractionRulesCache() {
    cache = null;
    lastValid = null;
}
/* -------------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------- */
function resolveFromDoc(doc, projectId, hash, quarantinedHints) {
    ...
    return {
        ...
        preferredLanguage: override?.preferred_language !== undefined
...
export function resolveExtractionRules(projectId, loaded = loadExtractionRules()) {
    if (!loaded.doc)
        return { ...EMPTY_RESOLVED, projectId };
    if (projectId === null)
        return loaded.global;
    const quarantinedHints = new Set(loaded.issues
...
export function renderExtractionConstraintClause(rules) {
    ...
    for (const pattern of rules.neverExtract) {
        lines.push(`- Never emit a fact or observation whose text matches: ${formatRegex(pattern)}`);
    }
 ...
 */
export function composeExtractionSystemPrompt(base, rules) {
    if (!rules)
        return base;
 ...
 */
export async function buildBlockSet(matcher, patterns, candidates, surface = "extractor") {
 ...
 */
export async function extractionMatcherAvailable(patterns) {
 ...
 */
export function extractionRulesPreClaimBlock(rules = loadExtractionRules()) {
    if (rules.disabledByEnv || !rules.present)
        return null;
    ...
    return {
        reason: "extraction_rules_invalid",
        detail: `${errors[0].code}: ${errors[0].message} — run: memex extract rules validate` +
 ...
 */
export async function releaseExtractionRulesHold(db) {
    if (!db)
        return 0;
    try {
        const { releaseHeldJobs } = await import("./model-budget.js");
        const typed = db;
        return (releaseHeldJobs(typed, "extraction_rules_invalid") +
...
async function applyRulesDoc(doc, auditAction, opts) {
    const { applyOverlayChange } = await import("./overlay-admin.js");
    const result = await applyOverlayChange("extraction-rules", { doc }, {
...
export async function rollbackExtractionRules(revision, opts) {
 ...
 */
export function extractionRulesChecks(heldJobs = []) {
    const checks = [];
    ...
    if (rules.disabledByEnv) {
        ...
        checks.push({
            name: "extraction-rules-overlay",
            status: "fail",
            detail: "invalid — EXTRACTION IS HELD (no attempts consumed): " +
 ...
 */
export function observeOverlayBenchmarkEnvironment() {
    const exists = (file) => {
        try {
