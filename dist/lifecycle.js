...
function skippedCaptureLine(rows) {
    ...
    const failing = done.filter((row) => !HEALTHY_HOOK_OUTCOMES.has(String(row.outcome ?? "")) &&
    ...
    if (legacyNoTranscript.length > 0) {
        parts.push(`${legacyNoTranscript.length} legacy no-transcript row(s) ` +
    ...
    if (waited.length > 0) {
        ...
        return {
            ...
            detail: `hooks waited on the database — ${waited.length}/${paired} runs over ` +
                `${HOOK_DB_WAIT_WARN_MS} ms (worst ${worst.event} ${Math.round(Number(worst.db_wait_ms))} ms, ${worst.ts})` + holder + skippedText,
        };
    }
