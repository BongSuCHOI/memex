...
export function setupHooks({ dryRun = false, root = pluginRoot() } = {}) {
            ...
            for (const event of HOOK_EVENTS) {
                ...
                for (const block of blocks) {
                    ...
                    block.hooks = block.hooks.filter((h) => {
                        if (h[OWNERSHIP_KEY] !== true) return true;
                        const m = h.command
                            ? h.command.match(/"([^"]+)"/)
                            : null;
                ...
                hooks.hooks[event] = blocks.filter(
                    (b) => (b.hooks ?? []).length > 0,
                );
    ...
    if (!dryRun && diff.add.length > 0) {
        ...
        for (const { event, command, async } of desiredEntries(root).filter(
            (x) => diff.add.some((a) => a.command === x.command),
        )) {
...
export function doctor() {
    ...
    return {
        json: checks.map(({ name, status, detail }) => ({
            name,
            status,
            detail,
        })),
