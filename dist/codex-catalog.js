...
export function catalogPathFromConfig(home = codexHome()) {
    try {
...
function reasoningLevelsOf(value) {
...
function optionFromEntry(entry) {
...
function parseCatalogDocument(text) {
    ...
    for (const entry of entries) {
        ...
        if (!option || seen.has(option.slug))
            continue;
 ...
 */
export function readCodexCatalog(home = codexHome()) {
    return (readFrom(catalogPathFromConfig(home), 'model_catalog_json') ??
...
export function listableModels(catalog) {
    return catalog.models.filter((model) => model.visible);
}
export function findCatalogModel(catalog, slug) {
 ...
 */
export function reasoningEffortsForModel(slug, catalog = readCodexCatalog()) {
