    ...
    function armYieldWatch() {
        ...
        yieldWatch = setTimeout(() => {
            ...
            void probeInjectDaemon(sockPath)
                ...
                if (probe.listening || owning || !retired || releasedOwnership)
                    return;
    ...
    const warmUp = () => {
        ...
        void (async () => {
            try {
                ...
                if (elapsed >= 3_000)
                    note(`embedding model warm after ${elapsed}ms`);
            }
            catch (error) {
                warmState = 'failed';
                note(`embedding warm-up failed (requests will load the model themselves): ${error instanceof Error ? error.message : String(error)}`);
            }
        })();
    };
    const onListen = () => {
