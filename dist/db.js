...
import { countStaleExchangeContentHashes, ensureContinuitySchema, exchangeContentHash, storedToolInput, storedToolResult, } from "./continuity-store.js";
...
function initializeConnection(db, mode, busyTimeoutMs) {
    try {
        ...
        const timeout = Number.isFinite(busyTimeoutMs)
            ? Math.max(0, Math.trunc(busyTimeoutMs))
...
export function openWriteDb(dbPath = getDbPath(), busyTimeoutMs) {
