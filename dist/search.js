...
// missing new ones. Test overrides (TEST_DB_PATH / MEMEX_DB_PATH) switching
// mid-process also get a fresh handle. Short-lived CLI processes
...
export async function searchConversations(query, options = {}) {
    ...
    let results = [];
    {
        ...
        if (mode === 'vector' || mode === 'both') {
            ...
            const vecQuery = (vecDtype, fetchCount) => {
        ...
        `);
                ...
                const rows = stmt.all(embeddingToVecBlob(queryEmbedding, vecDtype), fetchCount, EMBEDDING_VERSION, ...timeParams);
                for (const r of rows)
            ...
            for (;;) {
                ...
                if (results.length >= limit || nextFetchCount <= fetchCount)
                    break;
