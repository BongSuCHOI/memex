  ...
  `).get(jobId);
    ...
    `).run(expectedGeneration, new Date().toISOString(), checkpoint.checkpoint_id);
        ...
        if (evidence.length === 0) {
            if (!timeWorkerTransaction("completeEmptyCapsuleCheckpoint", (markStart) => completeEmptyCapsuleCheckpoint(db, {
                checkpointId: checkpoint.checkpoint_id,
                jobId,
                owner,
                leaseGeneration: claim.lease_generation,
        ...
        const applied = timeWorkerTransaction("applyWorkCapsulePatch", (markStart) => applyWorkCapsulePatch(db, {
      ...
      `).run(detail.slice(0, 1_000), new Date().toISOString(), claim.checkpoint_id);
          ...
          WHERE checkpoint_id = ?
        `).run(detail.slice(0, 1_000), new Date().toISOString(), skipped, attemptPage.fromSeq, claim.checkpoint_id);
            }
            else {
