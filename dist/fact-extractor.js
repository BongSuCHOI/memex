    ...
    const appendRun = (start, end) => {
        ...
        for (let anchor = start; anchor < end; anchor++) {
            ...
            const hasAntecedentContext = [anchor - 1, anchor - 2].some((index) => {
                ...
                return booleanFlag(antecedent.context_only_due_to_watermark) && (antecedent.assistant_message.trim() !== "" ||
...
function hasClaimBinding(fact, authoritativeText) {
    const claimTokens = bindingTokens(fact);
    const evidenceTokens = bindingTokens(authoritativeText);
    return claimTokens.some((claim) => evidenceTokens.some((evidence) => claim === evidence ||
...
function validateExtractedFactCandidateDetailed(candidate, exchanges) {
    ...
    for (const index of contextIndices) {
        ...
        const isBoundedPredecessor = ratificationIndices.some((ratificationIndex) => index < ratificationIndex && ratificationIndex - index <= 2);
        const material = contextBindingMaterial(exchange);
        if (!isBoundedPredecessor ||
            !material ||
...
export function buildFactEntailmentVerifierPrompt(candidates, exchanges) {
    return JSON.stringify({
        ...
        candidates: candidates.map((candidate, candidateIndex) => ({
            ...
            ratification_context: candidate.evidence?.some((evidence) => evidence.kind === "ratification")
                ? (candidate.context_exchange_indices ?? []).map((exchangeIndex) => ({
                    exchange_index: exchangeIndex,
                    non_authoritative_human_context: truncatePromptData(exchanges[exchangeIndex - 1]?.user_message, HUMAN_MESSAGE_LIMIT),
                    non_authoritative_referent: truncatePromptData(contextBindingMaterial(exchanges[exchangeIndex - 1]), ASSISTANT_MESSAGE_LIMIT),
                }))
                : [],
        })),
    }, null, 2);
}
export async function verifyExtractedFactCandidates(candidates, exchanges, modelCall) {
    if (candidates.length === 0)
        return [];
    const response = await modelCall(FACT_ENTAILMENT_VERIFIER_PROMPT, buildFactEntailmentVerifierPrompt(candidates, exchanges));
...
function recordCandidateObservation(observability, result) {
    ...
    if ((result.fact.context_exchange_indices?.length ?? 0) > 0 &&
        ...
        try {
            ...
            if (Array.isArray(extracted)) {
                ...
                for (const candidate of extracted) {
                    const validation = validateExtractedFactCandidateDetailed(candidate, window);
                ...
                if (structurallyAccepted.length > 0) {
                    renewLease?.();
                    const entailment = await verifyExtractedFactCandidates(structurallyAccepted.map((validation) => validation.fact), window, modelCall);
