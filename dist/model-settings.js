...
export function isReasoningEffort(value) {
    return typeof value === 'string' &&
        ALLOWED_REASONING_EFFORTS.includes(value);
}
...
export function invalidateModelSettingsCache() {
    memo = null;
}
...
function parseSettings(raw) {
    ...
    if (object.version !== MODEL_SETTINGS_VERSION) {
        ...
        if (!warnedUnsupportedVersion) {
            ...
            console.error(`[memex] ${modelSettingsPath()}: unsupported version ${String(object.version)} ` +
    ...
    const llmRaw = object.llm && typeof object.llm === 'object' && !Array.isArray(object.llm)
        ? object.llm
        : {};
    const embeddingRaw = object.embedding && typeof object.embedding === 'object' && !Array.isArray(object.embedding)
        ? object.embedding
        : {};
    ...
    if (Object.keys(stagesRaw).length > 0 && !warnedStages) {
        warnedStages = true;
        console.error(`[memex] ${modelSettingsPath()}: llm.stages is reserved and ignored in this release`);
    }
 ...
 */
export function readModelSettings(options) {
...
function readRawSettings() {
    try {
        const parsed = JSON.parse(fs.readFileSync(modelSettingsPath(), 'utf8'));
 ...
 */
export function writeModelSettings(patch, now = new Date()) {
    ...
    const llmRaw = raw.llm && typeof raw.llm === 'object' && !Array.isArray(raw.llm) && !rawUnsupported
        ? { ...raw.llm }
        : {};
    const embeddingRaw = raw.embedding && typeof raw.embedding === 'object' && !Array.isArray(raw.embedding) && !rawUnsupported
        ? { ...raw.embedding }
        : {};
    if (patch.llm && 'model' in patch.llm) {
        ...
        if (model === null || model === undefined) {
            ...
            if (!isValidModelId(model)) {
                throw new Error(`invalid model id ${JSON.stringify(String(model))} — expected 1-256 characters matching [\\w./:@+-]`);
            }
    ...
    if (patch.llm && 'reasoning' in patch.llm) {
        ...
        if (reasoning === null || reasoning === undefined) {
            ...
            if (!normalized) {
                throw new Error(`invalid reasoning effort ${JSON.stringify(String(reasoning))} — expected one of ${ALLOWED_REASONING_EFFORTS.join(', ')}`);
            }
...
function envReasoning() {
    ...
    if (!normalized) {
        ...
        if (warnedEnvReasoning !== raw) {
            ...
            console.error(`[memex] ${LLM_REASONING_ENV}=${raw} is not one of ${ALLOWED_REASONING_EFFORTS.join(', ')} — ignored`);
        }
        return null;
    }
    return normalized;
 ...
 */
export function resolveLlmSelection(overrides, options) {
    let model;
    let modelSource;
 ...
 */
export function llmSelectionFingerprint(overrides, options) {
