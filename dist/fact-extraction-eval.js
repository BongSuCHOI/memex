...
import { extractFactsFromExchanges, } from "./fact-extractor.js";
...
const NEGATIVE_TAXONOMY = new Set([
    "DROP-noise",
    "DROP-unsupported",
    "WRONG-scope",
]);
...
function parseToolEvidence(value, path) {
...
function parseExchange(value, path) {
    const input = record(value, path);
    ...
    return {
        id: stringValue(input.id, `${path}.id`),
        user_message: typeof input.user_message === "string"
            ? input.user_message
            : (() => {
                throw new Error(`${path}.user_message must be a string`);
            })(),
        assistant_message: typeof input.assistant_message === "string"
            ? input.assistant_message
            : (() => {
                throw new Error(`${path}.assistant_message must be a string`);
            })(),
        assistant_learnable: optionalBoolean(input.assistant_learnable, `${path}.assistant_learnable`),
        has_memex_recall: optionalBoolean(input.has_memex_recall, `${path}.has_memex_recall`),
        tool_evidence: (input.tool_evidence ?? []).map((tool, index) => parseToolEvidence(tool, `${path}.tool_evidence[${index}]`)),
    };
}
function parseExpectedFact(value, path) {
    const input = record(value, path);
...
function parseExpectation(value, path) {
    const input = record(value, path);
    if (input.outcome === "facts") {
        ...
        return {
            outcome: "facts",
            facts: input.facts.map((fact, index) => parseExpectedFact(fact, `${path}.facts[${index}]`)),
        };
    }
    if (input.outcome === "none") {
        const taxonomy = stringValue(input.false_positive_taxonomy, `${path}.false_positive_taxonomy`);
...
function parseCase(value, index) {
    ...
    const exchanges = input.exchanges.map((exchange, exchangeIndex) => parseExchange(exchange, `${path}.exchanges[${exchangeIndex}]`));
        ...
        for (const [factIndex, fact] of expected.facts.entries()) {
            for (const exchangeId of fact.authoritative_exchange_ids) {
                if (!exchangeIds.has(exchangeId)) {
                    throw new Error(`${path}.expected.facts[${factIndex}] references unknown exchange ${exchangeId}`);
                }
            }
        }
    }
    ...
    if (input.watermark_after_exchange_index !== undefined) {
        if (!Number.isInteger(input.watermark_after_exchange_index) ||
            Number(input.watermark_after_exchange_index) < 1 ||
            Number(input.watermark_after_exchange_index) >= exchanges.length) {
            throw new Error(`${path}.watermark_after_exchange_index must identify a prefix exchange`);
        }
...
export function parseFactExtractionFixture(value) {
...
function scoreCase(testCase, facts) {
    if (testCase.expected.outcome === "none") {
        return {
            passed: facts.length === 0,
            issues: facts.length === 0
                ? []
    ...
    for (const expected of testCase.expected.facts) {
        ...
        if (!exactSetMatch(candidate.source_exchange_ids ?? [], expected.authoritative_exchange_ids)) {
    ...
    if (falsePositives > 0)
        issues.push("DROP-noise");
    return {
        passed: matchedFacts === testCase.expected.facts.length &&
            falsePositives === 0 &&
            issues.length === 0,
...
function summarize(reports) {
    ...
    const positives = curated.filter((entry) => entry.expected?.outcome === "facts");
    const negatives = curated.filter((entry) => entry.expected?.outcome === "none");
    const expectedFactCount = positives.reduce((sum, entry) => sum + entry.expected_fact_count, 0);
    const matchedFactCount = positives.reduce((sum, entry) => sum + entry.matched_fact_count, 0);
    const observedFactCount = reports.reduce((sum, entry) => sum + entry.observed_facts.length, 0);
    const falsePositiveCount = reports.reduce((sum, entry) => sum + entry.false_positive_count, 0);
    const tokenObserved = calls.filter((call) => call.input_tokens !== null && call.output_tokens !== null);
    const tokenStatus = calls.length === 0 || tokenObserved.length === 0
        ? "NOT_PROVEN"
    ...
    return {
        ...
        negative_no_fact_accuracy: ratio(negatives.filter((entry) => entry.observed_facts.length === 0).length, negatives.length),
        ...
        output_tokens: tokenStatus === "NOT_PROVEN"
            ? null
            : tokenObserved.reduce((sum, call) => sum + (call.output_tokens ?? 0), 0),
        cached_input_tokens: tokenStatus === "NOT_PROVEN"
            ? null
            : tokenObserved.reduce((sum, call) => sum + (call.cached_input_tokens ?? 0), 0),
        token_usage_status: tokenStatus,
        input_characters: calls.reduce((sum, call) => sum + call.input_characters, 0),
        output_characters: calls.reduce((sum, call) => sum + call.output_characters, 0),
        total_latency_ms: calls.reduce((sum, call) => sum + call.latency_ms, 0),
    };
}
export async function evaluateFactExtractionFixture(fixture, options) {
    const caseReports = [];
    for (const testCase of fixture.cases) {
        const db = createCaseDatabase(testCase);
        const calls = [];
        let callIndex = 0;
        const modelCall = async (systemPrompt, userMessage) => {
        ...
        let executionError;
        try {
            facts = await extractFactsFromExchanges(db, testCase.id, { droppedBatches: 0 }, undefined, {
        ...
        caseReports.push({
            ...
            passed: score.passed,
            issues: score.issues,
            expected_fact_count: testCase.expected.outcome === "facts"
...
export async function evaluateFactExtractionArchiveSessions(db, sessionIds, options) {
    ...
    for (const sessionId of uniqueSessionIds) {
      ...
      `)
        ...
        let executionError;
        try {
            facts = await extractFactsFromExchanges(db, sessionId, { droppedBatches: 0 }, undefined, { modelCall });
        }
        catch (error) {
            executionError = error instanceof Error ? error.message : String(error);
        }
...
export function compareFactExtractionReports(current, baseline) {
    if (current.mode !== "curated" ||
    ...
    for (const entry of current.cases) {
        ...
        if (!prior || entry.passed === null || prior.passed === null)
            continue;
        if (!prior.passed && entry.passed)
            improvements.push(entry.id);
        else if (prior.passed && !entry.passed)
            regressions.push(entry.id);
        else if (entry.passed)
            unchangedPasses.push(entry.id);
        else
            unchangedFailures.push(entry.id);
    }
    const tokenDelta = (currentValue, baselineValue) => currentValue === null || baselineValue === null
        ? null
    ...
    return {
        ...
        deltas: {
            matched_fact_count: current.summary.matched_fact_count - baseline.summary.matched_fact_count,
            false_positive_count: current.summary.false_positive_count - baseline.summary.false_positive_count,
            self_amplification_leakage_count: current.summary.self_amplification_leakage_count -
                baseline.summary.self_amplification_leakage_count,
            model_calls: current.summary.model_calls - baseline.summary.model_calls,
            input_tokens: tokenDelta(current.summary.input_tokens, baseline.summary.input_tokens),
            output_tokens: tokenDelta(current.summary.output_tokens, baseline.summary.output_tokens),
            total_latency_ms: current.summary.total_latency_ms - baseline.summary.total_latency_ms,
        },
    };
}
