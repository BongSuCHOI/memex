...
export function recordLocalMeaningEvidence(db, factId, text, method, sourceIds) {
    ...
    ON CONFLICT(fact_id) DO UPDATE SET semantic_generation = excluded.semantic_generation,
      ...
      method = excluded.method, verified_at = excluded.verified_at`).run(factId, row.semantic_generation, createHash('sha256').update(text).digest('hex'), JSON.stringify(sources), method, new Date().toISOString());
}
