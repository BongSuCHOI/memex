...
export function deferMemoryJobForModelBudget(db, input) {
    ...
    `).run(input.budgetId ?? null, input.parentWaveId ?? budget?.parentWaveId ?? null, availableAt.toISOString(), reason, nowIso, input.jobId, input.owner, input.leaseGeneration, nowIso, input.budgetId ?? row.budget_id).changes;
        if (changed !== 1)
            return false;
        if (row.target_id) {
            db.prepare(`
