...
export function getModelWorkContext() {
    return modelWorkStorage.getStore();
}
/** Run work with context merged into the current async context. */
export async function withModelWorkContext(context, fn) {
    const current = modelWorkStorage.getStore();
    return modelWorkStorage.run({ ...(current ?? {}), ...context }, async () => await fn());
}
...
function parseWaveId(parentWaveId) {
    ...
    if (parentWaveId.includes(":run:"))
        return { root, seq: null };
 ...
 */
export function ensureModelBudgetSchema(db) {
    ...
    `);
       ...
       WHERE cleared_at IS NULL AND last_observed_at < ?`).run(new Date().toISOString(), new Date(Date.now() - MODEL_CONFIG_HOLD_TTL_MS).toISOString());
        ...
        if (legacyRows.length > 0) {
            ...
            const runKey = (root, seq) => `${root} ${seq}`;
            const runOwner = new Map();
            for (const row of db
                .prepare(`SELECT budget_id, root_wave_id, run_seq FROM model_work_budgets
           WHERE root_wave_id IS NOT NULL AND run_seq IS NOT NULL`)
            ...
            for (const row of legacyRows) {
                ...
                while (true) {
                    ...
                    if (owner === undefined || owner === row.budget_id)
                        break;
                    seq += 1;
                }
    ...
    try {
        ...
        console.warn(`[memex] model budget lineage index not created (${error instanceof Error ? error.message : String(error)}); run numbers are still recorded`);
    }
}
...
function envDeadlineAt(now = Date.now()) {
    ...
    if (absolute && !Number.isNaN(Date.parse(absolute)))
        return new Date(absolute).toISOString();
    const duration = envInt(["MEMEX_MODEL_BUDGET_DEADLINE_MS", "MEMEX_MODEL_DEADLINE_MS"], DEFAULT_DEADLINE_MS, MAX_DEADLINE_MS);
    return new Date(now + duration).toISOString();
}
export function modelBudgetLimitsFromEnv(now = Date.now()) {
    return {
        maxAttempts: envInt(["MEMEX_MODEL_BUDGET_MAX_ATTEMPTS", "MEMEX_MODEL_MAX_ATTEMPTS"], DEFAULT_MAX_ATTEMPTS, 100_000),
        maxInputChars: envInt(["MEMEX_MODEL_BUDGET_MAX_INPUT_CHARS", "MEMEX_MODEL_MAX_INPUT_CHARS"], DEFAULT_MAX_INPUT_CHARS, 10_000_000),
        maxOutputChars: envInt(["MEMEX_MODEL_BUDGET_MAX_OUTPUT_CHARS", "MEMEX_MODEL_MAX_OUTPUT_CHARS"], DEFAULT_MAX_OUTPUT_CHARS, 10_000_000),
 ...
 * durable wave.
 */
export function registerModelWorkTargets(db, input) {
    ...
    if (!stage)
        throw new Error("model work target stage must not be empty");
...
export function settleModelWorkTargets(db, input) {
    ensureModelBudgetSchema(db);
    const budget = readBudgetById(db, input.budgetId);
    if (!budget)
        throw new ModelBudgetNotFoundError(input.budgetId);
...
export function getModelWorkTargets(db, filter = {}) {
    if (!tableExists(db, MODEL_TARGET_TABLE))
        return [];
    const where = [];
    const params = [];
    if (filter.budgetId) {
...
function rebindPendingModelWorkTargetsInTransaction(db, input) {
    ...
    if (input.jobState) {
        clauses.push("EXISTS (SELECT 1 FROM memory_jobs j WHERE j.job_id = t.job_id AND j.state = ?)");
  ...
  `);
    for (const row of rows) {
        upsert.run(randomUUID(), input.toBudgetId, String(row.stage), String(row.target_id), row.job_id == null ? null : String(row.job_id), String(row.created_at), now, row.reason == null ? null : String(row.reason));
    }
 ...
 */
export function getOrCreateModelWorkBudget(db, input) {
    ensureModelBudgetSchema(db);
    ...
    if (existing) {
        ...
        if (input.budgetId && input.budgetId !== budget.budgetId) {
            throw new Error(`parent wave ${parentWaveId} is already bound to budget ${budget.budgetId}`);
        }
        return budget;
    }
...
export function bindMemoryJobToBudget(db, input) {
    ensureModelBudgetSchema(db);
 ...
 */
export function rebindMemoryJobToBudget(db, input) {
    ensureModelBudgetSchema(db);
...
export function startNewModelWorkRunForJob(db, input) {
    ensureModelBudgetSchema(db);
    const old = db.prepare("SELECT budget_id FROM memory_jobs WHERE job_id = ?").get(input.jobId);
    if (!old)
        throw new Error(`memory job ${input.jobId} does not exist`);
 ...
 */
export function startNewModelWorkRunForBudget(db, input) {
    ensureModelBudgetSchema(db);
    const now = input.now ?? new Date();
    ...
    if (!["exhausted", "cancelled"].includes(previousBudget.state)) {
        throw new Error(`model work budget ${input.budgetId} is still active; resume requires an exhausted or cancelled budget`);
    }
    const requestedWave = input.parentWaveId?.trim();
    ...
    for (const row of rows) {
        ...
        try {
            ...
            if (error instanceof ModelBudgetAffinityError ||
    ...
    const moveCompletedJobTargets = db.transaction(() => rebindPendingModelWorkTargetsInTransaction(db, {
    ...
    const moveJoblessTargets = db.transaction(() => rebindPendingModelWorkTargetsInTransaction(db, {
        fromBudgetId: previousBudget.budgetId,
        toBudgetId: budget.budgetId,
        jobId: null,
        now,
...
export function deferMemoryJobForModelBudget(db, input) {
    ...
    const now = input.now ?? new Date();
    const nowIso = now.toISOString();
    const unspentClaim = (input.reason === "deadline" || input.reason === "window") &&
    ...
    `).run(input.budgetId ?? null, input.parentWaveId ?? budget?.parentWaveId ?? null, unspentClaim ? "pending" : "retry", availableAt.toISOString(), reason, nowIso, unspentClaim ? 1 : 0, input.jobId, input.owner, input.leaseGeneration, nowIso, input.budgetId ?? row.budget_id).changes;
        if (changed !== 1)
            return false;
        if (row.target_id) {
            ...
            const refundTargetAttempt = unspentClaim && columnNames(db, "extraction_targets").has("attempts");
            db.prepare(`
        UPDATE extraction_targets
      ...
      `).run(unspentClaim ? "pending" : "retry", reason, nowIso, row.target_id, input.owner, input.leaseGeneration);
        }
...
function budgetExhaustion(budget, now = Date.now()) {
    if (budget.state === "cancelled")
        return "cancelled";
    if (budget.reservedAttempts >= budget.maxAttempts)
        return "attempts";
...
function resolveBudgetExhaustion(db, budget, now) {
    return (budgetExhaustion(budget, now.getTime()) ??
 ...
 */
export function reserveModelAttempt(db, input) {
    ensureModelBudgetSchema(db);
    ...
    `).run(nextReserved, nextState, nowIso, budget.budgetId, budget.reservedAttempts);
        if (input.jobId) {
            ...
            if (job && !job.budget_id) {
                db.prepare("UPDATE memory_jobs SET budget_id = ?, maintenance_wave_id = COALESCE(maintenance_wave_id, ?) WHERE job_id = ? AND budget_id IS NULL").run(input.budgetId, budget.parentWaveId, input.jobId);
            }
        }
        return {
            attemptId,
    ...
    if (result instanceof ModelBudgetExhaustedError)
        throw result;
    return result;
}
export function finishModelAttempt(db, input) {
    ensureModelBudgetSchema(db);
...
export function exhaustModelBudget(db, input) {
    ensureModelBudgetSchema(db);
...
export function isModelBudgetExhausted(error) {
    if (error instanceof ModelBudgetError)
        return true;
    const reason = error;
    if (reason && reason.reason !== error && isModelBudgetExhausted(reason.reason))
        return true;
    return error?.code === "MEMEX_MODEL_BUDGET";
}
export function modelBudgetErrorFromUnknown(error) {
    if (error instanceof ModelBudgetError)
        return error;
    const reason = error;
    if (reason && reason.reason !== error)
        return modelBudgetErrorFromUnknown(reason.reason);
    return null;
}
    ...
    const maintain = db.transaction(() => {
      ...
      `).all(rootWaveId, budget.budgetId);
        ...
        const strandedLineageJobs = (activeBudgetId) => {
            if (!tableExists(db, "memory_jobs"))
                return 0;
            if (!columnNames(db, "memory_jobs").has("budget_id"))
                return 0;
            const row = db.prepare(`
      ...
      `).get(rootWaveId, activeBudgetId, activeBudgetId, nowIso);
        ...
        // 아니다(계보당 한 번).
        if (input.holdActive === true && latest)
            return latest;
...
function movableWaveJobSql(db) {
    const holdClause = tableExists(db, "memory_jobs") && columnNames(db, "memory_jobs").has("hold_reason")
...
function rebindMovableWaveJobs(db, input) {
  ...
  `);
    ...
    for (const job of jobs) {
        const changed = move.run(input.toBudgetId, input.toParentWaveId, input.nowIso, input.nowIso, job.job_id, input.fromBudgetId).changes;
        if (changed === 1)
...
export function rolloverSpentWaveBudgets(db, input = {}) {
    ...
    `).all(nowIso, ...prefixParams, nowIso, nowIso);
        ...
        for (const candidate of candidates) {
            ...
            if (latestIsOther && latest.state === "cancelled")
                continue;
...
export function getOrCreateWorkerModelBudget(db, input) {
    ...
    if (envBudgetId) {
        ...
        if (!budget)
            throw new ModelBudgetNotFoundError(envBudgetId);
        return budget;
    }
    return getOrCreateWaveModelBudget(db, {
        parentWaveId: input.parentWaveId?.trim() ||
...
export function getModelWorkDiagnostics(db, filter = {}) {
    ...
    `).all(...ids);
        for (const row of rows) {
            ...
            attempts.push({
                ...
                tokenUsageStatus: row.token_usage_status == null
                    ? null
    ...
    if (tableExists(db, "extraction_targets") &&
    ...
    `).all(...ids);
        for (const row of rows) {
            const fact = derivedFactQueue
                ? db.prepare("SELECT is_active, ontology_category_id, needs_consolidation FROM facts WHERE id = ?").get(String(row.target_id))
                : undefined;
            if (fact && (fact.is_active !== 1 ||
    ...
    `).all(...ids);
        for (const row of targetIds) {
            const fact = db.prepare("SELECT is_active, needs_consolidation, ontology_category_id FROM facts WHERE id = ?").get(row.target_id);
            if (!fact || fact.is_active !== 1)
                continue;
    ...
    if (hasDerivedFactQueue(db)) {
        ...
        const targetLink = (stage) => targetTable
            ? `EXISTS (
          SELECT 1 FROM model_work_targets t
          WHERE t.target_id = f.id AND t.stage = '${stage}'
        )`
            : "0";
        const attemptLink = (stages) => attemptTable
            ? `EXISTS (
          SELECT 1 FROM model_work_attempts a
    ...
    const totals = {
        ...
        tokenUsageUnknown: attempts.filter((attempt) => attempt.tokenUsageStatus === "NOT_PROVEN" || attempt.tokenUsageStatus == null).length,
    ...
    for (const attempt of attempts) {
        ...
        stage.inputChars = sumKnown(attempts.filter((item) => item.stage === attempt.stage).map((item) => item.inputChars));
        stage.outputChars = sumKnown(attempts.filter((item) => item.stage === attempt.stage).map((item) => item.outputChars));
        stage.inputTokens = sumKnown(attempts.filter((item) => item.stage === attempt.stage).map((item) => item.inputTokens));
        stage.outputTokens = sumKnown(attempts.filter((item) => item.stage === attempt.stage).map((item) => item.outputTokens));
        stage.cachedInputTokens = sumKnown(attempts.filter((item) => item.stage === attempt.stage).map((item) => item.cachedInputTokens));
        stage.tokenUsageObserved = attempts.filter((item) => item.stage === attempt.stage && item.tokenUsageStatus === "observed").length;
        stage.tokenUsagePartial = attempts.filter((item) => item.stage === attempt.stage && item.tokenUsageStatus === "partial").length;
...
export async function withResolvedModelWorkContext(requested, fn) {
    const parent = modelWorkStorage.getStore();
    const environment = envContext();
    const requestedDefined = Object.fromEntries(Object.entries(requested).filter(([, value]) => value !== undefined));
    const merged = {
        ...environment,
        ...(parent ?? {}),
        ...requestedDefined,
    ...
    try {
        ...
        let boundJob;
        if (jobId && tableExists(db, "memory_jobs")) {
            boundJob = db.prepare("SELECT budget_id, maintenance_wave_id FROM memory_jobs WHERE job_id = ?").get(jobId);
            if (boundJob?.budget_id) {
                ...
                if (!boundBudget)
                    throw new ModelBudgetNotFoundError(budgetId);
        ...
        return await modelWorkStorage.run({ ...merged, db, budgetId, parentWaveId, stage, jobId }, async () => await fn());
    }
    finally {
        if (ownsDb)
            db.close();
