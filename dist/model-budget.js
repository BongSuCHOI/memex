...
export function exhaustModelBudget(db, input) {
    ...
    if (!row)
        throw new ModelBudgetNotFoundError(input.budgetId);
    const now = input.now ?? new Date();
    db.prepare("UPDATE model_work_budgets SET state = ?, updated_at = ? WHERE budget_id = ? AND state IN ('active','exhausted')").run(input.reason === "cancelled" ? "cancelled" : "exhausted", now.toISOString(), input.budgetId);
