...
export function isDefaultBranchName(branch, defaultBranch) {
    if (!branch)
        return true;
    if (defaultBranch)
...
export function branchSignalFor(input) {
    ...
    if (!branch)
        return { kind: "none", branch: null, tierReason: "no-branch-signal" };
 ...
 */
export function deterministicWorkstreamId(projectId, branch) {
    return branch
...
function audit(db, input) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(auditId, input.action, input.projectId ?? null, input.workspaceId ?? null, input.workstreamId ?? null, input.sessionId ?? null, input.reason, detail, at);
}
...
export function inspectWorkspaceLocation(cwd) {
    ...
    if (!gitDir)
        return { gitCommonDir: null, remoteFingerprint: null, locationKind, branch: null, defaultBranch: null, gitCommonIdentity: null, gitDirIdentity: null };
...
export function resolveProjectWorkspace(db, input) {
    ...
    const inspected = input.gitCommonDir === undefined && input.remoteFingerprint === undefined
        ...
        : { gitCommonDir: input.gitCommonDir ?? null, remoteFingerprint: input.remoteFingerprint ?? null, locationKind: input.locationKind ?? "directory", branch: input.branch ?? null, defaultBranch: null, gitCommonIdentity: null, gitDirIdentity: null };
    ...
    `).get(device, canonicalPath);
      ...
      `).all(device, inspected.gitDirIdentity);
          ...
          WHERE workspace_id = ?
        `).run(canonicalPath, gitCommonDir, inspected.gitCommonIdentity, remoteFingerprint, locationKind, input.branch ?? inspected.branch ?? null, inspected.defaultBranch, at, row.workspace_id);
        ...
        if (input.projectId) {
            ...
            if (!selectedProject)
                throw new Error("explicit project_id does not exist");
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(workspaceId, selectedProject.project_id, device, canonicalPath, gitCommonDir ? canonicalizeProjectPath(gitCommonDir) : null, remoteFingerprint, inspected.gitCommonIdentity, inspected.gitDirIdentity, locationKind, input.branch ?? inspected.branch ?? null, inspected.defaultBranch, at, at);
...
export function approveRemoteProjectMapping(db, projectId, remoteFingerprint, now) {
    if (!/^[a-f0-9]{8,128}$/i.test(remoteFingerprint))
        throw new Error("invalid remote fingerprint");
    if (!projectRow(db, projectId))
...
export function linkWorkspaceToProject(db, input) {
    const at = nowIso(input.now);
    const tx = db.transaction(() => {
        ...
        if (!projectRow(db, input.targetProjectId))
    ...
    `).run(input.targetProjectId, input.workspaceId, sourceProjectId, workspace.canonical_path);
        const remaining = db.prepare("SELECT COUNT(*) AS n FROM workspaces WHERE project_id = ?").get(sourceProjectId).n;
        if (remaining === 0) {
...
export function splitWorkspace(db, input) {
    const at = nowIso(input.now);
    const tx = db.transaction(() => {
  ...
  `).get(input.sessionId);
    ...
    if (input.explicitWorkstreamId) {
        const explicit = db.prepare("SELECT project_id FROM minimal_workstreams WHERE workstream_id = ?")
            .get(input.explicitWorkstreamId);
        if (!explicit || explicit.project_id !== input.projectId)
            throw new Error("explicit workstream is outside the resolved project");
    ...
    const deterministicId = deterministicWorkstreamId(input.projectId, signal.kind === "branch" ? signal.branch : null);
    ...
    if (!workstreamId) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)
    `).run(workstreamId, input.projectPath, input.sessionId, signal.branch, deterministicReason, input.projectId, input.workspaceId, at, at);
    }
    db.prepare(`
...
export function createWorkstream(db, input) {
    ...
    if (!/^[A-Za-z0-9_-]{4,128}$/.test(workstreamId))
        throw new Error("invalid workstream_id");
  ...
  `).run(workstreamId, canonicalizeProjectPath(input.projectPath), input.ownerSessionId, input.branch ?? null, input.projectId, input.workspaceId, topicFingerprint, at, at);
    return workstreamId;
}
export function rebindSessionWorkstream(db, input) {
    const at = nowIso(input.now);
    const tx = db.transaction(() => {
    ...
    `).get(input.sessionId);
        ...
        if (session.workstream_id === input.workstreamId)
            return;
        db.prepare(`
  ...
  `);
    ...
    for (const row of rows) {
        ...
        if (userText) {
            ...
            saved += insert.run(`hot-${hash(exchangeId, "human", contentHash).slice(0, 32)}`, row.project_id, row.workspace_id ?? null, row.workstream_id ?? row.session_workstream ?? null, sessionId, exchangeId, "human", "human_assertion", userText.slice(0, 2_000), contentHash, at, expires).changes;
        }
    ...
    `).all(exchangeId);
        for (const tool of tools) {
            ...
            saved += insert.run(`hot-${hash(exchangeId, tool.source_type, contentHash).slice(0, 32)}`, row.project_id, row.workspace_id ?? null, row.workstream_id ?? row.session_workstream ?? null, sessionId, exchangeId, "trusted_tool", tool.source_type, text.slice(0, 2_000), contentHash, at, expires).changes;
        }
    }
    db.prepare("DELETE FROM hot_evidence WHERE expires_at <= ?").run(at);
    return saved;
}
export function readHotEvidence(db, input) {
...
export function assignFactSubject(db, input) {
    if (!/^[a-z][a-z0-9_.-]{2,160}$/.test(input.subjectKey))
        throw new Error("invalid subject_key");
    ...
    if (input.promotionState === "workstream" && !input.workstreamId)
        throw new Error("workstream state requires workstream_id");
    if (input.promotionState === "workstream" && input.evidence !== "experimental")
        throw new Error("workstream state must remain experimental");
...
export function sessionProjectRevisionState(db, sessionId) {
...
export function markSessionProjectRevisionSeen(db, sessionId, expectedRevision) {
    return db.prepare(`
