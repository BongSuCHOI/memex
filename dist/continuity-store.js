 ...
 */
export function storedToolInput(value) {
...
export function storedToolResult(value) {
    if (value === undefined || value === null || value === "")
        return null;
    return String(value);
}
...
function contentHashOfStoredRow(row) {
    return hashExchangeShape(row, [...row.tools].sort(byIdBinary).map((tool) => ({
        id: tool.id,
        name: tool.toolName,
 ...
 */
const LEGACY_INPUT_RENDERINGS = [null, "", 0, false];
/**
 ...
 */
const LEGACY_RESULT_RENDERINGS = [null, ""];
/**
 ...
 */
export const LEGACY_RECONSTRUCTION_MAX_HASHES = LEGACY_RENDERING_LIMIT * LEGACY_INPUT_RENDERINGS.length * 2;
/**
 ...
 */
export function legacyContentHashes(row) {
    ...
    while (head < slots.length && head < LEGACY_EXHAUSTIVE_SLOTS) {
        ...
        if (next > LEGACY_RENDERING_LIMIT)
            break;
        headTotal = next;
        head++;
    }
    ...
    const localeOrder = [...indices].sort((l, r) => row.tools[l].id.localeCompare(row.tools[r].id));
      ...
      `).run(workspaceId, projectId, device.value, canonical, inspected.gitCommonDir, inspected.gitCommonIdentity, inspected.gitDirIdentity, inspected.remoteFingerprint, inspected.locationKind, inspected.branch, inspected.defaultBranch, nowIdentity, nowIdentity);
        ...
        if (columnNames(db, "projects").has("quarantined")) {
            ...
            for (const row of suspect) {
                if (isUntrustedProjectPath(row.canonical_path))
                    quarantine.run(row.project_id);
            }
        }
 ...
 */
export function countStaleExchangeContentHashes(db) {
    const rows = db
        .prepare("SELECT id, user_message, assistant_message, line_end, content_hash FROM exchanges")
        .all();
    ...
    for (const row of rows)
        if (row.content_hash && hashOf(row) !== row.content_hash)
            stale++;
    return stale;
}
...
export function refreshExchangeMetadata(db, sessionId) {
  ...
  `);
    ...
    if (migratedFormat > 0) {
        ...
        process.stderr.write(`[memex] migrated ${migratedFormat} pre-0.7.26 content hash(es) in place; ` +
 ...
 */
export function settleStaleOpenExchanges(db, sessionId) {
...
function targetFromRow(row) {
    return {
        ...
        factLanguage: row.fact_language === "ko" || row.fact_language === "en" || row.fact_language === "mixed"
            ? row.fact_language
            : null,
    };
}
 ...
 */
export function setExtractionTargetFactLanguage(db, targetId, language) {
    if (!columnNames(db, "extraction_targets").has("fact_language"))
        return false;
    return db.prepare("UPDATE extraction_targets SET fact_language = ? WHERE target_id = ? AND IFNULL(fact_language, '') IS NOT ?").run(language, targetId, language ?? "").changes === 1;
}
