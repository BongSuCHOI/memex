...
function isValidScope(scopeType, scopeProject) {
    ...
    // project
    return (typeof scopeProject === "string" &&
        !!scopeProject.trim() &&
        ...
        if (newFacts.length > 0) {
            ...
            for (const f of newFacts) {
                try {
                    ...
                    console.error(`sync-import: embedding failed for fact ${f.id} — 다음 run 재시도:`, e instanceof Error ? e.message : e);
                }
            }
        ...
        if (fs.existsSync(relationsPath)) {
            ...
            for (const line of lines) {
                try {
                    ...
                    const src = db
                        .prepare("SELECT id, scope_type, scope_project FROM facts WHERE id = ?")
                        .get(r.source_fact_id);
                    const tgt = db
                        .prepare("SELECT id, scope_type, scope_project FROM facts WHERE id = ?")
                        .get(r.target_fact_id);
                    if (!src || !tgt)
                        continue;
