...
| `test/jobs-drain-cli-slice.test.mjs` (0.7.22) | `memex jobs drain`이 설치 루트의 Continuity worker를 전경에서 돌리는지 — 청구 가능한 capsule_update 2건 중 `--max 1`이 **정확히 1건만** 처리하는지, `--json` stdout이 JSON 배열 하나뿐인지, `--max`의 값 없음/비숫자/비정수/1 미만이 exit 2 usage인지, 32 초과가 32로 clamp되며 stderr에 그 사실이 남는지, `jobs`/`recover` help·recover 후행 안내·`model-work resume`의 worker 명령이 `memex jobs drain`을 가리키고 npm 전용 worker bin 이름을 더는 말하지 않는지 (#156) |
| `test/capsule-last-error-clear.test.ts` (0.7.22) | capsule page 성공이 `memory_jobs.last_error`를 지우는지 — 실패 1회 뒤 성공한 시도가 완료 시점에 이미 NULL인지, partial 재오픈으로 `pending`·`attempts=0`이 된 뒤에도 NULL이고 checkpoint state와 모순되지 않는지, 지워진 문구가 `retry_history`에 `action: "success"` 항목으로 보존되고 `memex jobs show`의 `retryHistory`에 나오는지, 옛 버전이 `completed`로 남긴 stale failure가 재오픈 시 `action: "reopen"` 항목으로 보존되는지, 선행 실패가 없던 성공은 아무것도 추가하지 않는지, 실제로 실패한 시도는 여전히 오류를 기록하는지 (#157) |
...
- universal operating gate는 constraint, project-limited collaborative adoption은 decision으로 보존
