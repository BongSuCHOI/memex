...
| `test/capsule-last-error-clear.test.ts` (0.7.22) | capsule page 성공이 `memory_jobs.last_error`를 지우는지 — 실패 1회 뒤 성공한 시도가 완료 시점에 이미 NULL인지, partial 재오픈으로 `pending`·`attempts=0`이 된 뒤에도 NULL이고 checkpoint state와 모순되지 않는지, 실제로 실패한 시도는 여전히 오류를 기록하는지 (#157) |
