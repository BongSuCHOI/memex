...
파라미터입니다: 파라미터가 없으면 공통(글로벌) 기억, `?scope=all`은 전체 프로젝트 + 공통 기억,
`?scope=project&project=<encoded-canonical-absolute-cwd>`는 프로젝트 하나입니다. 대화 원장·활동·해석
맥락 행은 글로벌 전용 범위에서 보이지 않으므로 그때는 `?scope=all`이나 프로젝트 범위를 씁니다.
