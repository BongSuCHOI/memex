...
- `fact_revisions` = Chronicle(단일 append-only history table, D-018): 10 event kind(0.6.0에서 `PROMOTED`/`DEMOTED`, 0.6.3에서 로컬 전용 `SYNC_IMPORTED` 추가), content-hash event id, `effective_at`(source) vs `recorded_at`, grounded cause vs classifier note, `reverts_event_id`, `projection_applied`. 정책은 `FACT-LIFECYCLE.md` §13.
