...
회수 게이트 화면: **적용 중인 규칙**(해시·revision·패턴 수·덮어쓴 임계값 수), **의도별
패턴**(내장 · 사용자 · 비활성 · 격리를 한 표에 상태 배지로), **사용자 패턴 추가**,
**어휘**(확인 · 이어하기 · 간투사), **게이트 임계값**, **이 프롬프트로 테스트**, 격리 목록,
변경 이력.

**게이트 임계값 카드**(#120)는 8개 행을 kv로 그린다. 각 행은 **내장값과 지금 판정에 쓰는 값을
같은 줄에서** 보여 주고, 범위(`min`·`max`·정수/실수)는 코어 카탈로그가 준 값을 그대로 number
입력에 싣는다 — 화면이 범위를 복제하면 저장은 코어가 거절하는데 화면만 통과시키는 상태가 생긴다.
저장은 **한 번에 한 행**이라 감사 줄과 이력 항목이 무엇이 바뀌었는지 말할 수 있고, 범위 위반은
`config.<key>` path를 단 422로 돌아와 같은 카드 안에 행별 사유로 그려진다. "내장값으로" 버튼은
**실제로 덮어쓴 행에만** 붙는다 — 내장값 행에는 되돌릴 것이 없다. 마지막 덮어쓰기를 되돌리면
`config` 블록 자체가 문서에서 사라진다.

추출 규칙 화면: **이 규칙이 건드리는 것과 못 건드리는 것**, **규칙 편집기**(선호 언어 · 저장 금지
패턴 · 결정으로 우대할 패턴 · 제외 주제), **사용자 정의 기억 종류**(읽기 전용), **제약 절 미리보기**
(프롬프트에 실제로 덧붙는 텍스트), **영향 시뮬레이션**, **추출이 이 규칙 때문에 대기 중입니다**,
**다른 규칙으로 추출된 것**(드리프트 → `memex extract rules reextract --dry-run`), 격리 목록, 변경 이력.

### 사용자 정의 기억 종류 (#121)

`custom_fact_kinds`(GUIDE §22.2)는 내장 5종 위에 최대 8개의 `facts.category` 값을 더한다. 화면에서
결정된 것 세 가지:

- **읽기 전용 표다**(`#rules-kinds`: id · 라벨 2개 · 설명 · 추출 힌트 + 정의 개수/한도). 편집을 파일·
  CLI에만 둔 것은 미완성이 아니라 의도다 — id는 그대로 이미 저장된 기억의 `category`이므로, 화면에서
  이름을 바꾸거나 지우면 그 기억들이 아무 데도 정의되지 않은 종류를 달게 된다. 그 마이그레이션 경로가
  생기기 전까지 화면은 무엇이 정의돼 있는지만 말하고, 그 이유를 캡션으로 적는다.
- **라벨은 사전이 아니라 오버레이가 갖는다.** `badge.custom.<id>`는 **존재하지 않는 사전 키**다 —
  운영자가 만든 id에 번역 키를 발급할 방법이 없다. 배지(`ui.mjs` `name()`/`badge()`)와 도움말
  (`help.mjs` `badgeHelp()`)은 **사전을 먼저 보고**, 없을 때만 `label_en`/`label_ko`를 로케일로 고른다.
  그래서 `badge.*.{label,help}` 1:1 계약(`ui/test/help.test.cjs`)은 런타임 종류가 있어도 그대로다.
  정의되지 않은 종류는 코어 원문(id)으로 보인다 — "모르는 값의 이름을 지어내지 않는다".
- **값의 출처는 `/api/v2/bootstrap`의 `customFactKinds`**이고 `boot()`가 첫 render 앞에 한 번 꽂는다.
  오버레이 탭이 아니라 부트스트랩인 이유는 배지가 **모든 화면**에서 맞아야 하기 때문이다. DB 없이도
  실리고(오버레이는 파일이다), 코어 미빌드·파일 손상이면 빈 목록이며 그 사실은 이 탭의 `issues[]`와
  doctor가 말한다. 기억 목록(`/facts`)은 내장 5종 **뒤에** 같은 종류를 유형 칩으로 붙이고, 칩의 값은
  곧 서버가 `f.category=?`로 거르는 저장값이다.
...
| 메서드 | 동작 |
| --- | --- |
| `GET` | `status` — 두 오버레이의 적용 상태, 내장+사용자 패턴, 어휘, **임계값 카탈로그(이름·종류·범위·내장값)와 지금 적용 중인 값**, 격리, 이력, 대기 작업 수 |
| `POST` | `{action, …}`. 읽기 action `validate`·`test`·`simulate`는 **`confirm`을 요구하지 않고 감사 줄도 남기지 않는다**(dry-run은 어떤 기록에도 쓰지 않는다는 계약). 쓰기 action `patch`·`set`·`reset`·`rollback`·`quarantine-clear`는 `confirm: true`가 필수이고, 관리 명령이 실행 중이면 409로 거절하며 성공·실패 모두 감사 1줄을 남긴다. `patch`는 **한 요청에 한 가지 변경**만 받는다 — `patternsAdd`·`patternsDisable`·`patternsEnable`·`words`·`config`·`configReset` 중 정확히 하나 |
