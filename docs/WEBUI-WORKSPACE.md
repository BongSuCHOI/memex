...
- **내보내기**는 새 세대를 만든 뒤 그 세대를 zip으로 **데이터 루트 안**
  ...
  파일을 덮어쓰지 못하게 하는 경계다. 동기화가 꺼져 있어도 동작하고, `export-status.json`은
  건드리지 않는다(그 기록은 *공유 폴더*에 무엇이 도달했는지에 대한 진술이다).
- **가져오기**는 항상 **검증 → 미리보기 → 확인** 순서다. 미리보기는 보낸 기기·세대, 적용 시
  `+N / ~N / -N`, 충돌 목록, 거부된 세대의 사유를 보여주고 **아무것도 바꾸지 않는다**(계획 계산은
  롤백하는 트랜잭션 안에서 돌린다). 미리보기의 `~N`은 **기억 개수**이고 적용 결과의 `~N`은 의미·
  근거·활성 상태를 축별로 세므로 더 클 수 있다 — 화면에 그 차이를 적는다.
