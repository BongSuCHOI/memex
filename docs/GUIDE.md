...
반면 **첫 DB 호출 전의 고정 비용**(node 기동·`dist` import·DB open과 그 migration·marker fsync)이
느린 머신에서는 1.45~1.9초였습니다. 그래서 잠금 대기 없이(`db_wait_ms: 0`) capture 3건이 전부
건너뛰어졌습니다(#166). 예산은 훅이 호스트에게 timeout을 안기지 않게 하는 장치이지 capture를 더
빨리 포기시키는 장치가 아니므로, 허용되는 만큼 host timeout을 올리고 예산은 거기서 파생합니다.
그 고정 비용은 done row의 `startup_ms`와 doctor의 `max startup N ms`로 머신마다 확인할 수 있습니다. inject 훅의 done row는 실패한 경우 `stage`(`receipt`/`compute`/`startup`)와 `context_delivered`도 남깁니다 — 컨텍스트가 나간 뒤 영수증만 실패한 것과 아무것도 전달되지 못한 것은 전혀 다른 사건입니다(0.7.25, #166).

예산은 연결할 때 한 번만 보는 것이 아니라 **국면
...
최대 2,500 ms이고 exit margin 150 ms는 언제나 남깁니다). capture와 rehydration은 **트랜잭션 본문 안에서도** 예산을 다시 봅니다 — 본문은 잠금을
...
ingest 사전 점검(0.7.25, #166): 남은 예산에서 reserve 100 ms를 뺀 창과 `pending bytes / 20,000 B/ms`를
비교해 **정말 큰 delta만** `oversize`로 거절합니다. 남은 시간이 reserve보다 적으면 그것은 크기 문제가
아니라 예산 문제이므로 `deadline`입니다 — 0.7.24는 reserve를 뺀 값이 음수가 되는 것만으로 132 KB
(약 6.6 ms 분량)를 `oversize`라고 보고했습니다.
...
(capture gap row는 어느 경로에서도 **정확히 한 번** 씁니다). 예외는 하나입니다: capture event인데
**payload에 `transcript_path`가 아예 없으면**(`codex exec --ephemeral`처럼 transcript 파일이 없는
...
이 기기의 이름을 바꾸거나 지우면 다음 자동 내보내기가 새 세대를 게시해 상대 기기에 전달합니다
(0.6.6 #98 — 전에는 `--force` 없이는 `unchanged`로 건너뛰어 상대에는 옛 이름이 그대로 남았습니다).
상대 기기에 내가 붙인 이름은 이 기기에만 있는 표시이므로 내보내기를 일으키지 않습니다.
...
- `--archive` 내보내기는 **공유 폴더를 쓰지도, 만들지도 않습니다**(0.6.6 #95). 세대는 데이터 루트 안의
  전용 staging에서 만들어 zip으로 묶고 staging은 지워집니다. 따라서 동기화가 꺼져 있거나 공유 폴더를
  지워 둔 상태에서 이 명령을 써도 폴더가 되살아나지 않고, 상대 기기가 가져갈 세대가 새로 생기지도
  않습니다. `export-status.json`(마지막 내보내기 기록)도 그대로입니다 — 그 기록은 *공유 폴더*에 무엇이
  도달했는지에 대한 진술이기 때문입니다.
- 내보내기 경로는 심볼릭 링크를 따라가서도 데이터 루트를 벗어날 수 없습니다(0.6.6 #101). 기본 경로
  `<data root>/sync/exports/`도 같은 검사를 받으므로, 그 디렉터리를 iCloud 폴더로 링크해 두면 조용히
  밖으로 쓰지 않고 거부합니다.
- `--dry-run`은 **로컬 인덱스가 이미 있어야** 합니다(0.6.6 #97). 인덱스가 전혀 없는 기기에서는 DB를
  새로 만들지 않고 `no local index yet — run \`memex sync\` once before previewing an import`으로
  거부합니다. 먼저 `memex sync`를 한 번 돌리십시오. 적용(`--dry-run` 없이)은 전과 같습니다.
...
- **자동 export 시점**: SessionEnd(별도 항목, 3초 timeout)와 자동 유지보수 wake. 두 경우 모두
  ...
  항상 자기 첫 세대를 받습니다. durable 변경 판정에는 recall 영수증의 `status`/`emitted_at`도
  들어갑니다(0.6.2) — `prepared → emitted` 전이만으로도 다음 export가 나갑니다.
...
의존성 materialize 다음에는 **스키마 migration을 한 번 미리 적용**합니다(0.7.25, #166):
새 설치본의 `dist`로 data root DB를 한 번 열고 닫으며, `Schema migrated for <version>` 또는
`Schema already current`를 출력합니다. 업데이트 직후 첫 세션에서는 SessionStart의 훅 5개가 동시에
...
`memex doctor`가 출력하는 점검 항목은 다음 순서로 **항상 22개**이고(0.7.0에서 `llm-model` 1개와
오버레이 5개, 0.7.24에서 `capture-gap`·`hook-latency` 2개, 0.7.25에서 `schema-version` 1개가
추가됐습니다), `ontology-index`는 repair marker가 있을 때만 추가되어 최대 23개입니다.
...
| `inject-output` | 최근 20줄의 마지막 상태. `error`/`receipt-failed`면 fail, 창 안에 `receipt-failed`가 섞이면 warn. 정상으로 보는 상태는 `injected`, `context-only`, `no-match`, `deduped`, `skipped`입니다 — `context-only`(fact 0개, Capsule/연속성 컨텍스트만 발행)는 주입이 성공한 정상 결과이므로 여기서 warn하지 않고, fact 0개가 계속되는지는 `injection-yield`가 봅니다(0.7.25, #165). 그 밖의 상태는 "알 수 없음"으로 warn |
| `capture-gap` (0.7.24, #162) | `<data root>/continuity/gaps/`의 marker 목록. 없으면 ok. **무엇이 걸려 있는지는 event가 정합니다**: capture event(Stop/Interrupt/PreCompact/SessionEnd)는 `capture skipped at <event> <ts> (<bytes> uncaptured bytes); continuity/extraction of the tail is pending #163`, `SessionStart(clear\|compact)`는 `epoch advance skipped at SessionStart(<source>) <ts>; repaired by the next injection`, capture event인데 **transcript도 byte 수도 없는** marker(`transcript_path` 없음 + `transcriptBytes`가 0/없음 — `codex exec --ephemeral` 세션)는 `no transcript at <event> <ts> (ephemeral session; nothing to capture)`, 그 밖(PostCompact 등 telemetry 전용)은 `hook did not finish at <event> <ts> (no capture at stake)`입니다. **앞의 두 종류**(capture, epoch)가 하나라도 있으면 warn, 남은 것이 뒤의 두 종류뿐이면 ok — **ok 줄은 `N marker(s), nothing at stake, oldest …`로 셉니다**(0.7.27, #171: 걸린 것이 없다고 판정한 줄이 `11 skipped capture(s)`로 시작하면 안 됩니다). warn 줄은 그대로 `N skipped capture(s)`입니다 — 0.7.24/0.7.25가 남긴 transcript 없는 marker는 이 분류로 30일 뒤 조용히 소멸합니다(0.7.26, #168). transcript 경로는 있고 byte 수만 모르는 marker는 여전히 capture입니다(그 훅은 할 일이 있었습니다). 개수·`oldest`·**종류별 집계**는 모두 **전체 marker를 ts로 정렬한 뒤** 계산하므로(반환 목록만 500개로 자릅니다) 500개 뒤에 숨은 오래된 marker도 정확히 세고 정리합니다. warn/ok도 이 전체 집계로 정하고, warn을 만든 종류의 **가장 오래된 marker를 상세 문장 맨 앞에 적습니다** — 오래된 PostCompact marker 500개가 반환 목록을 채우면 그 뒤의 미처리 Stop 하나가 `501 skipped capture(s)`로 세어지면서도 ok으로 보고되던 문제입니다(0.7.25, #165). "내용은 절대 유실되지 않는다"는 표현은 쓰지 않습니다([§5](#5-lifecycle-hooks)) |
| `hook-latency` (0.7.24, #162) | `hook-events.jsonl` 마지막 200줄을 `invocation_id`로 start/done 짝지어 봅니다. done이 없는 start가 `예산 + 10초`를 넘었고 같은 pid의 이후 행도 없으면 **호스트가 죽인 것**(`killed by host`)으로 warn하고 marker 수를 함께 적습니다. `db_wait_ms > 1,000`인 done row가 있으면 `hooks waited on the database`로 warn. `<data root>/logs/worker-transactions.jsonl`이 있으면 최근 100행 중 **문제가 된 훅이 돌던 시간과 겹치는**(`[ts - held_ms, ts]` ∩ 훅 구간, ±250 ms) 행만 후보로 삼아 `held_ms`가 가장 큰 행을 "held the write lock for N ms"로 덧붙이고, 겹치는 행이 없으면 `no worker transaction overlapped this hook`이라고 적습니다 — 시간을 보지 않으면 한 시간 전에 끝난 트랜잭션이 지금 훅의 잠금 보유자로 지목됩니다. 이 행은 트랜잭션이 **끝날 때** 기록되므로 `ts`는 종료 시각이고 보유 구간은 `[ts - held_ms, ts]`입니다 — `ts`를 시작으로 읽으면 훅이 포기할 때까지 실제로 잠금을 붙들고 있던 트랜잭션은 구간 밖으로 밀려 누락되고, 이미 끝난 트랜잭션이 보유자로 지목됩니다(0.7.25, #165). 죽은 훅의 구간은 `start + host timeout + 500 ms`까지입니다 — kill 판정용 10초 grace를 여기에 쓰면 **호스트가 이미 죽인 뒤에 시작한** 트랜잭션이 보유자로 지목됩니다(0.7.25, #166). 정상(ok) 줄에는 지목할 훅이 없으므로 보유자 문장을 붙이지 않습니다(backlog는 workstream마다 트랜잭션이 하나이므로 `scheduleCapsuleBacklog#<n>`처럼 각각 한 행입니다 — 여러 트랜잭션을 한 구간으로 재면 첫 트랜잭션 이후의 대기가 전부 held로 잡힙니다)(`wait_ms`는 잠금을 **기다린** 쪽이므로 절대 이 문장에 쓰지 않습니다). UserPromptSubmit에는 host timeout이 없으므로 그 이벤트에는 절대 timeout을 주장하지 않습니다. **완료된 행의 outcome이 ok가 아니면**(busy/deadline/oversize/error) 건너뛴 capture이므로 warn하고 `N skipped (busy 1, deadline 1, oversize 1) last error: …`를 덧붙입니다. `no-transcript`는 캡처할 transcript가 없었던 **완료된** 훅이므로 ok 계열로 보아 이 집계에서 제외합니다 — 포함하면 `codex exec` 세션 11건이 `11 skipped (error 11) last error: capture hook requires transcript_path`로 나옵니다(0.7.26, #168). 0.7.24/0.7.25가 남긴 행(`outcome: error` + error 문구가 `capture hook requires transcript_path` + **`stage` 없음**)은 skipped 집계에서 빼되 **숨기지 않고 중립 버킷으로 적습니다**: `N legacy no-transcript row(s) (pre-0.7.26; strict-mode failures indistinguishable)`. 0.7.24/0.7.25의 strict 실패도 같은 모양(같은 outcome·문구·stage 없음)으로 기록됐으므로 그 행이 무해하다고 증명할 수 없고, 조용히 버리면 무해한 ephemeral 행들과 함께 실제 실패까지 묻힙니다(0.7.27, #171). 이 버킷은 **그 자체로 warn이 아니고**, skipped라고 부르지 않으며, 줄 끝의 `last error`도 제공하지 않습니다. 단 **`stage: "no-transcript"`가 붙은 행은 제외하지 않습니다**: 그 stage는 0.7.26 strict 모드만 쓰고, strict의 증거를 doctor가 보고하게 하는 것이 #168의 요지입니다. 단 UserPromptSubmit의 `error`는 done row의 **`stage`**로 세 가지를 구분합니다(0.7.25, #166): `receipt`(컨텍스트는 전달되고 recall 영수증만 `prepared`에 남은 문서화된 fallback #44)는 `N receipt failure(s) (context delivered)`, `compute`/`startup`(retrieval 실패·import 실패로 **아무 컨텍스트도 전달되지 않음**)은 `N injection failed (no context delivered)`, `stage`가 없는 0.7.25 이전 행은 `N inject error (stage unknown)`으로 적습니다 — 어느 쪽도 주장하지 않습니다. 셋 다 건너뛴 capture가 아닙니다 — kill/대기 판정을 대체하지 않고 함께 적습니다(0.7.25, #166: capture 3건을 건너뛰고도 ok라고 말했습니다). 정상 줄에는 done row의 `startup_ms` 최대값을 `max startup N ms`로 적어 그 머신의 고정 비용을 보여 줍니다 |
| `schema-version` (0.7.25, #166) | 파일의 `PRAGMA user_version`을 **읽기 전용**으로 읽어 `CURRENT_SCHEMA_VERSION`과 비교합니다(doctor는 절대 migration을 실행하지 않습니다). 같으면 ok로 `current (v10)`, 낮으면 warn으로 `pending migrations: v9 < v10 — will retry on next open or run memex update`, DB 파일이 아직 없으면 ok로 `unknown (no database yet at <path>)`, 읽을 수 없으면 warn입니다. `memex update`가 exit 3(건너뛴 migration)으로 끝났을 때 상태를 확인하는 창구입니다. v10(0.7.26, #168/#169)은 transcript 없는 세션의 `capture_gaps` 행을 닫고, 같은 pass의 `refreshExchangeMetadata`가 0.7.25 형식의 `content_hash`를 **세대를 올리지 않고** 제자리에서 갱신합니다(형식 변경은 내용 변경이 아닙니다 — 옮겨 적은 개수를 pass마다 한 줄 적습니다) |
...
0.6.5부터 `identify` 응답에 `warming` 필드가 실립니다. 같은 빌드의 소유자가 아직 embedding model을
적재 중이면 판정은 **ok**이고 detail이 "워밍 중 — 프롬프트는 `daemon.reason=warming`으로 in-process
fallback" + `memex deps warm` 안내를 적습니다. 정상 소유자의 일시 상태이므로 daemon 실패로 세지
않습니다(#92).
...
기존 설치(0.4.0 이하 DB)를 최신 plugin으로 올리면 Continuity schema `7`으로 additive migration이 한 번 실행됩니다(`continuity_schema_meta`의 `schema_version`으로 확인). 0.7.25부터는 `memex update`가 이 migration을 미리 적용하고, 그 뒤의 hook/MCP/CLI 실행은 파일의 `PRAGMA user_version`이 현재 전체 스키마 버전(`9`)이면 migration 목록 자체를 건너뜁니다(#166) — 열 때마다 backfill을 다시 돌리던 비용(92 MB·15,000 exchange 기준 약 415 ms, 쓰기 잠금 보유)이 약 2 ms로 줄어듭니다. migration이 하나라도 바뀌면 `CURRENT_SCHEMA_VERSION`을 올려야 하며, 그러지 않으면 테스트가 실패합니다. 실패를 일부러 삼키는 migration(taxonomy 유일성 인덱스)이 건너뛰어지면 버전을 **기록하지 않아** 다음 열기에서 다시 시도합니다(#166 리뷰). 데이터 정규화 statement는 `ROW_NORMALIZATION_INVARIANTS`(src/db.ts) 목록에서 실행되며, **현재 writer가 쓴 행에는 전부 no-op**이어야 합니다 — `test/row-normalization.test.ts`가 그 불변식을 지킵니다. 이 불변식이 깨지면 fast path가 writer의 버그를 그대로 노출합니다(0.7.25 게이트: `insertFact`가 `lifecycle_updated_at`을 비워 둬서 갓 export한 facts.jsonl이 protocol v4 검증에서 거부됐습니다). 중단되면 다음 실행에서 이어서 재실행되며 released row와 rowid는 보존됩니다. 절차는 §12와 같고, 완료 후 Codex를 재시작하십시오. v7은 현재 남은 evidence를 한 번 replay하며 기존 Capsule은 첫 새 projection commit까지 유지합니다. 구버전 진행 중 Capsule job의 lease는 폐기됩니다. Migration·cursor 계약은 [SCHEMA.md](SCHEMA.md#sequence-cursors-schema-v7)에 있습니다.
...
Capsule 한 세대의 bounded storage size는 기본 12,000자입니다(`MEMEX_CAPSULE_MAX_CHARS`, 하한 2,000자). 초과분은 job을 죽이지 않고 우선순위대로 절단해 저장하며, 무엇이 줄었는지 남깁니다. 리스트 항목 수 상한(8개)도 같습니다(0.6.3): 앞 8개만 남기고 버린 개수를 `truncated_fields_json`의 `itemCaps`에 기록합니다. 0.6.3부터 절단은 `finalChars <= maxChars`를 보장하며(제어문자 이스케이프까지 고려한 마지막 수단 루프), 그래도 넘는 드문 경우에는 `overBudget: true`로 그 사실을 밝힙니다.
...
memex doctor          # dependencies / inject-output / recall-provenance / injection-yield / embedding-cache / inject-daemon ...
```
...
| 첫 프롬프트가 60–70초 (0.6.5, #92) | `doctor`의 `embedding-cache: warn` (`missing — … Run: memex deps warm`), 로그의 `duration_ms` 68,000–74,000, `daemon.reason: "warming"` | 안정 캐시에 embedding model(약 129 MB)이 없어 첫 모델 적재가 다운로드. 0.6.4까지는 캐시가 설치본 `node_modules` 안이어서 업데이트마다 비었고, 세션 daemon과 훅 fallback이 **같은 129 MB를 동시에** 받았습니다 | `memex deps warm` (진행 표시, 이미 있으면 즉시 종료). `memex update`/`memex deps materialize`가 캐시가 비어 있을 때 자동으로 수행하므로 보통은 직접 칠 일이 없습니다. 레거시 per-root 캐시가 남아 있으면 다운로드 대신 **복사**라 몇 초입니다 |
...
프로젝트 override가 **다르게** 정의하면 `KIND_ID_SHADOWS_GLOBAL`로 **저장을 거부**합니다 — 쓴 라벨이
조용히 무시되는 대신 어디를 고쳐야 하는지 알려 주는 쪽입니다. 전역 정의를 **그대로** 반복하는 것은
"이 프로젝트도 그 종류를 씁니다"라는 뜻이므로 허용됩니다. 이미 그렇게 적혀 있는 **기존 파일은 계속
읽힙니다**: 충돌에는 정해진 답(전역 우선)이 있으므로 읽기는 경고 하나로 끝나고 — `validate`와
doctor가 그 경고를 보여 줍니다 — 다음 저장에서 고르게 합니다. 읽기까지 거부하면 한 프로젝트의
override가 전 프로젝트의 추출을 `extraction_rules_invalid`로 세웁니다.)
