...
COMMANDS:
  ...
  backfill    Run extraction/ontology/embeddings backlog explicitly
...
async function main() {
  try {
    ...
    switch (command) {
      case 'setup':
        await runScript(join(__dirname, '..', 'scripts', 'setup-memex.js'), args);
        break;
      ...
      case 'sync':
        ...
        break;
      case 'update':
        await runScript(join(__dirname, '..', 'scripts', 'update-plugin.js'), args);
        break;
      case 'setup-hooks': {
        ...
        for (const a of result.diff.add) console.log(`Add: ${a.event} -> ${a.command}`);
        if (result.diff.staleOwnedEntries > 0) {
          console.log(`Stale Memex entries detected: ${result.diff.staleOwnedEntries} (re-run setup to repair paths)`);
        }
        console.log(`Existing non-Memory-Bank entries preserved: ${result.diff.preservedForeignEntries}`);
        if (dryRun) {
          console.log('No files changed.');
        } else {
          console.log(result.changed ? 'Lifecycle configured.' : 'Lifecycle already up to date (idempotent no-op).');
      ...
      case 'remove-hooks': {
        ...
        console.log(`${dryRun ? '[dry-run] Would remove' : 'Removed'}: ${result.removed} Memex hook entr${result.removed === 1 ? 'y' : 'ies'}`);
        console.log(`Non-Memory-Bank entries preserved: ${result.preservedForeignEntries}`);
        if (!dryRun) console.log('Memex data root and Codex rollouts untouched.');
        break;
      }
      ...
      case 'doctor': {
        ...
        if (args.includes('--json')) {
          console.log(JSON.stringify({ overall: report.overall, checks: report.json }, null, 2));
        } else {
          for (const c of report.json) console.log(`${c.status.toUpperCase().padEnd(5)} ${c.name}: ${c.detail}`);
      ...
      case 'migrate-projects': {
        ...
        const { planMigration, applyMigration } = await import(join(distDir, 'project-migration.js'));
        ...
        console.log(`Exchanges: ${plan.totalExchanges} total / ${plan.alreadyCanonical} already canonical`);
        console.log(`Movable (cwd evidence): ${plan.movable.length}`);
        console.log(`Ambiguous (no cwd evidence, NOT moved): ${plan.ambiguous.length}`);
        console.log(`Facts to re-scope (lexical normalization only): ${plan.factsRescope.length}`);
        for (const a of plan.ambiguous.slice(0, 10)) {
          console.log(`  ambiguous exchange ${a.id}: project=${a.project} (${a.reason}) — recover by re-sync from source rollout`);
        }
        if (plan.movable.length > 0) {
          const sample = plan.movable[0];
          console.log(`  e.g. exchange ${sample.id}: ${sample.from} -> ${sample.to}`);
        }
        if (!dryRun) {
          const result = applyMigration(db, dbPath);
          console.log(`Applied: exchanges=${result.exchangesUpdated} facts=${result.factsUpdated} archive_paths=${result.archivePathsUpdated}`);
      ...
      case 'facts': {
        ...
        try {
          if (sub === 'list') {
            ...
            if (!['global', 'all'].includes(scope)) throw new Error('usage: --scope global|all');
            ...
            for (const r of rows) {
              console.log(`${r.is_active ? 'active  ' : 'inactive'} ${r.id}  [${r.category}] ${String(r.fact).slice(0, 90)}${String(r.fact).length > 90 ? '…' : ''}`);
            }
            ...
            const id = optValue('--id'), text = optValue('--text');
            if (!id || !text) throw new Error('usage: memex facts edit --id <uuid> --text "new text" [--reason "why"]');
            const r = await fm.editFact(db, id, { text, reason: optValue('--reason'), sourceExchangeId: optValue('--source-exchange') });
            ...
            const id = optValue('--id');
            if (!id) throw new Error('usage: memex facts deactivate --id <uuid>');
            ...
            for (const r of revs) {
              console.log(`${r.created_at}  "${String(r.previous_fact).slice(0, 60)}" -> "${String(r.new_fact).slice(0, 60)}"${r.reason ? `  (${r.reason})` : ''}`);
            }
            console.log(`(${revs.length} revisions)`);
          } else if (sub === 'delete') {
            const id = optValue('--id');
            if (!id) throw new Error('usage: memex facts delete --id <full-uuid> --hard --yes');
            if (!flag('--hard')) {
              console.error('Default delete is deactivate. For permanent deletion use: delete --id <uuid> --hard --yes');
              process.exitCode = 1;
              break;
            }
            const impact = fm.hardDeleteImpact(db, id);
            if (!impact.exists) throw new Error(`fact not found: ${id}`);
            console.log(`Impact: revisions=${impact.revisions} relations=${impact.relations} vectors=1`);
            if (!flag('--yes')) {
              console.error('Refusing to hard delete without --yes. Deactivate is the default safe delete.');
              process.exitCode = 1;
              break;
            }
            const r = fm.hardDeleteFact(db, id, { confirm: true });
            console.log(`Deleted: ${id} (revisions=${r.impact.revisions}, relations=${r.impact.relations})`);
          } else {
            console.error('Usage: memex facts <list|show|edit|deactivate|restore|history|delete> [--id <uuid>] ...');
            process.exitCode = 1;
          }
        } finally {
          db.close();
      ...
      case 'status': {
        const { getPipelineStatus, formatPipelineStatus } = await import(join(distDir, 'pipeline-status.js'));
      ...
      case 'backfill': {
        ...
        if (!target || !['extract', 'ontology', 'embeddings'].includes(target)) {
          console.error('Usage: memex backfill <extract|ontology|embeddings> [--foreground|--background]');
          process.exitCode = 1;
          break;
        }
        const scriptMap = {
        ...
        const script = join(__dirname, '..', 'scripts', scriptMap[target]);
        if (!fsSync(script)) {
          console.error(`Worker script missing: ${script} — run npm run build first.`);
          process.exitCode = 1;
          break;
        }
        if (foreground) {
          console.log(`Running ${target} backfill in foreground...`);
          const child = spawn(process.execPath, [script], { stdio: 'inherit' });
          child.on('exit', (code) => {
            process.exitCode = code === 0 ? 0 : 1;
            if (code !== 0) console.error(`${target} backfill exited with code ${code}`);
          });
        } else {
          const { spawn: spawnBg } = await import('child_process');
          const child = spawnBg(process.execPath, [script], { detached: true, stdio: 'ignore' });
          child.unref();
          console.log(`${target} backfill started in background (pid ${child.pid}). Check progress: memex status`);
        }
        break;
      }
