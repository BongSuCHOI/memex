...
(function () {
  ...
  function bootError(msg) {
    $('#boot').innerHTML = '<div class="bl" style="color:#ff5c7f">로드 실패</div><div class="bs">' + esc(msg) + '</div>';
  }
  fetch(API_URL)
    .then((r) => {
      if (r.status === 404) throw Object.assign(new Error('live-api-unavailable'), { code: 'fallback' });
    ...
    .then((d) => {
      ...
      if (!d || !Array.isArray(d.domains) || !Array.isArray(d.facts) || !Array.isArray(d.rel)) {
        throw new Error('Invalid graph data: missing domains/facts/rel arrays');
      }
      DATA = d;
      init();
    })
    .catch((e) => {
      if (e && e.code === 'fallback') {
        ...
        fetch('data.json').then((r2) => r2.json()).then((d2) => {
          if (!Array.isArray(d2.domains) || !Array.isArray(d2.facts) || !Array.isArray(d2.rel)) throw new Error('Invalid static data.json');
          DATA = d2; init();
        })
  ...
  function buildEmptyState() {
    ...
    try { buildHeader(); } catch {}
    const ov = $('#overlay');
    if (ov) {
      ov.innerHTML = '<div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;flex-direction:column;gap:12px;color:#9aa4b2;text-align:center;padding:24px">'
  ...
  function computeLayout() {
    ...
    if (D === 0) { factPos = new Float32Array(0); return; }
  ...
  function openPanel(idx) {
    ...
    if (!f || !f[4]) return;
    try {
      const r = await fetch('/api/fact-provenance?id=' + encodeURIComponent(f[4]));
      if (!r.ok) return;
      const j = await r.json();
      if (j.error) return;
      let html = '<div class="rp-sec">PROVENANCE</div>';
      html += '<div class="rp-item" style="cursor:default"><span class="nm" style="font-size:11px;opacity:.8">fact id</span><span class="ty">' + esc(String(f[4]).slice(0, 8)) + '</span></div>';
      for (const src of (j.sources || [])) {
        html += '<div class="rp-item" style="cursor:default"><span class="nm">' + esc((src.user_message || '').slice(0, 60)) + '</span><span class="ty">' + esc(src.project || '') + '</span></div>';
      }
      for (const rev of (j.revisions || [])) {
        html += '<div class="rp-item" style="cursor:default"><span class="nm">' + esc((rev.previous_fact || '').slice(0, 60)) + ' → ' + esc((rev.new_fact || '').slice(0, 40)) + '</span><span class="ty">REV</span></div>';
      }
      $('#rpInner').insertAdjacentHTML('beforeend', html);
    } catch (err) { void err; }
  }
function closePanel() {
