...
export class KnowledgeGraph{
  ...
  layout(){
    ...
    for(const [key,items]of groups){
      ...
      this.clusters.push({name:domains.get(key)?.name||'미분류',center,color:hue,count:items.length});
  ...
  initGL(){
    ...
    if(!gl){this.renderer='Canvas 2D';this.fallback=this.canvas.getContext('2d');if(!this.fallback)throw new Error('이 브라우저에서 그래픽 캔버스를 사용할 수 없습니다. 왼쪽 기억 목록으로 탐색하세요.');return;}this.renderer='WebGL';this.gl=gl;
  ...
  drawLabels(){const c=this.ctx;c.setTransform(this.dpr,0,0,this.dpr,0,0);c.clearRect(0,0,this.width,this.height);c.textAlign='center';c.textBaseline='middle';
    for(const cl of this.clusters){const[x,y]=this.project(cl.center);c.fillStyle='#8eabb6';c.font='10px system-ui';c.fillText(cl.name.slice(0,30),x,y-36*this.zoom);c.fillStyle='#557783';c.font='9px system-ui';c.fillText(String(cl.count)+' memories',x,y-22*this.zoom);}
    ...
    const n=this.byId.get(selected);if(n){const[x,y]=this.project(n.pos);c.strokeStyle=n.hue;c.lineWidth=1;c.beginPath();c.arc(x,y,11,0,Math.PI*2);c.stroke();c.fillStyle='#d2e6e8';c.font='11px system-ui';c.fillText((n.fact_kr||n.fact||'').slice(0,45),x,Math.max(70,y-25));}
  }
  ...
  events(){const signal=this.abort.signal;let down=null;
    ...
    this.canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();this.options.onError?.('WebGL 컨텍스트가 중단됐습니다. 브라우저 복구 후 다시 그립니다.');},{signal});
