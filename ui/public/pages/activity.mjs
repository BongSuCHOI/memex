...
/**
 * 운영 이력의 명령 이름 (#109 · 설계 §5.3).
 *
 * 라벨은 프로즈가 아니라 **사전 키**다: 서버 카탈로그(`ui/lib/operations.cjs`)가 `labelKey`만
 * 싣고, 0.6.x가 `~/.memex/ui/operations.json`에 저장해 둔 항목에만 `label` 원문이 남아 있다.
 * 그래서 순서는 사전 → 저장된 원문 → 명령 이름이고, 없는 키로 t()를 부르지 않는다
 * (관리 화면의 `c.labelKey?t(c.labelKey):(c.label||key)`와 같은 규칙).
 */
function operationLabel(ctx,operation){
 const entry=ctx.bootstrap?.commands?.[operation.command];
 if(entry?.labelKey)return t(entry.labelKey);
 return operation.label||entry?.label||operation.command;
}
export async function render(ctx){
 ...
 else if(tab==='operations')body=banner(tHtml('activity.operations.banner'))+(page.items.length?table([esc(t('activity.operations.th.command')),esc(t('activity.operations.th.state')),esc(t('activity.operations.th.started')),esc(t('activity.operations.th.exitCode')),th(t('activity.nextAction'),'nextAction'),''],page.items.map(o=>`<tr><td><button class="text-link" data-operation="${esc(o.id)}">${esc(operationLabel(ctx,o))}</button><div class="meta mono">${esc(short(o.id))}</div></td><td>${badge(o.status)}</td><td>${esc(date(o.started_at))}</td><td>${number(o.exit_code)}</td><td>${guidanceCell(operationGuidance(o),ctx)}</td><td><button class="icon-btn" data-operation="${esc(o.id)}" aria-label="${esc(t('activity.operations.a11y.output'))}">${icon('terminal')}</button></td></tr>`)):empty(t('activity.operations.empty.title'),t('activity.operations.empty.body'),linkBtn(t('activity.action.openAdmin'),'settings',ctx.href('/settings',{tab:'actions'})),'terminal'));
