...
// 서버가 싣는 프로즈는 두 모양뿐이고 둘 다 전용 소비자가 있다(설계 v4 §5.2·§5.3):
// 오류 봉투는 `errorText(e)`(key+params, 없으면 코어 원문), 200 본문의 필드는 `payloadText`.
import {errorText,payloadText} from '../api.mjs';
...
export function migrationCard(ctx,env,runs={}){
 ...
 ${output?`<pre class="terminal mt" id="tier-migration-output">${esc(output)}</pre>`:preview?.outputLost?`<p class="caption mt">${esc(t('op.output.lostAcrossRestart'))}</p>`:preview&&preview.status==='completed'?`<p class="caption mt">${esc(t('settings.migration.noOutput'))}</p>`:''}
