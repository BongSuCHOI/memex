...
| 기억·사실 | `/facts` | 기억 검색, 상세 패널에서 수정·비활성화·복원·삭제. 목록 행과 상세에 승격 상태가 한국어 이름으로 표시됩니다(`작업 흐름`·`워크스페이스` = 브랜치 계층, `프로젝트 현행` = 프로젝트 공용, 범위가 `공통 기억`이면 글로벌). `이전 방식 배치`(legacy-project)는 표시하지 않습니다. 계층 이동 버튼은 없습니다 — `memex facts promote\|demote`가 유일한 경로입니다 |
