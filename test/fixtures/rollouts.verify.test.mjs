...
const { parseRolloutStream, readRolloutMeta } = await import(
  path.join(REPO, "src/codex-rollout.ts")
);
...
test("fixture: main-thread ingests user/assistant/tool exchanges", async () => {
  const { exchanges, isSubagent } = await parseRolloutStream(
    stream("main-thread.jsonl"),
    { archivePath: "x" },
  );
...
test("fixture: subagent thread is flagged for whole-thread isolation", async () => {
  const hdr = await readRolloutMeta(
    path.join(FIXTURES, "subagent-thread.jsonl"),
  );
  ...
  const { isSubagent } = await parseRolloutStream(
    stream("subagent-thread.jsonl"),
    { archivePath: "x" },
  );
...
test("fixture: tool-only turn preserved", async () => {
  const { exchanges } = await parseRolloutStream(
    stream("tool-only-turn.jsonl"),
    { archivePath: "x" },
  );
  assert.equal(exchanges.length, 2);
...
test("fixture: same-basename A/B carry distinct cwds", async () => {
  for (const team of ["team-a", "team-b"]) {
    const { meta, isSubagent } = await readRolloutMeta(
      path.join(FIXTURES, `same-basename-${team}.jsonl`),
    ).then((h) => ({ meta: h.meta, isSubagent: h.isSubagent }));
    assert.equal(isSubagent, false);
...
test("fixture: internal context prefixes excluded from user turns", async () => {
  const { exchanges } = await parseRolloutStream(
    stream("internal-context-prompt.jsonl"),
    { archivePath: "x" },
  );
  assert.equal(exchanges.length, 1);
...
// SCX-1: `compacted` records (real Codex 0.149.x shape observed in
// $CODEX_HOME/sessions) carry a nested replacement_history of user/assistant/
// developer messages plus an encrypted compaction payload. None of that may
// re-enter the index — compression must not launder recalled facts or agent
// synthesis back into what looks like human evidence.
test("fixture: compacted record and its replacement_history are never ingested", async () => {
  const { exchanges, isSubagent } = await parseRolloutStream(
    stream("compacted-thread.jsonl"),
    { archivePath: "x" },
  );
  assert.equal(isSubagent, false);
  // Only the pre- and post-compaction turns survive; nothing from the record.
  assert.equal(exchanges.length, 2);
  assert.ok(exchanges[0].userMessage.includes("before compaction"));
  assert.ok(exchanges[1].userMessage.includes("after compaction"));
  const all = JSON.stringify(exchanges);
  assert.ok(!all.includes("recalled-fact-marker"));
  assert.ok(!all.includes("encrypted_content"));
  assert.ok(!all.includes("replacement_history"));
  assert.ok(!all.includes("<environment_context>"));
  assert.ok(
    !all.includes("post-compact answer") ||
      all.includes("follow-up question after compaction"),
  );
});
...
test("fixture: worker prompt matches the real worker-guard prefixes", async () => {
  ...
  const text = fs.readFileSync(
    path.join(FIXTURES, "worker-prompt.jsonl"),
    "utf8",
  );
