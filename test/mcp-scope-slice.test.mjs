...
async function callTool(name, args) {
  const { handleToolCall } = await import(path.join(REPO, 'dist/mcp-server.js'));
...
test('tools/list publishes project and scope for every scoped graph tool', async () => {
  ...
  for (const name of ['search_ontology', 'graph_stats', 'explore_graph']) {
    ...
    assert.deepEqual(properties.scope?.enum, ['project', 'global', 'all'], `${name} must publish the scope enum`);
  }
});
...
test('cross_project_insights without current_project errors instead of using cwd', async (t) => {
  await seedDb(t);
  const reply = await callTool('cross_project_insights', { query: 'database decisions' });
  assert.equal(reply.isError, true);
...
test('ask_avatar / trace_fact without project error instead of using cwd', async (t) => {
  ...
  for (const [tool, args] of [
    ...
    assert.ok(reply.content[0].text.includes('is required'), `${tool}: ${reply.content[0].text.slice(0, 120)}`);
  }
});
...
test('explicit scope:"global" returns only global facts; project A never sees B', async (t) => {
  ...
  // is the plugin root.
  const a = await callTool('search_facts', { query: 'database replication decisions', project: '/tmp/w/team-a/shared' });
  assert.notEqual(a.isError, true);
  const aText = JSON.stringify(a);
  assert.ok(!aText.includes('MySQL read replicas'), 'project A leaked project B fact');
  assert.ok(aText.includes('/tmp/w/team-a/shared') || aText.includes('Scope'));

  const g = await callTool('search_facts', { query: 'small pull requests', scope: 'global' });
  assert.notEqual(g.isError, true);
  const gText = JSON.stringify(g);
  if (!gText.includes('Results: 0')) {
    assert.ok(gText.includes('global'), 'scope global output should be labeled global-only');
    assert.ok(!gText.includes('Postgres row-level'), 'global scope leaked a project fact');
  }

  const all = await callTool('search_facts', { query: 'database', scope: 'all' });
