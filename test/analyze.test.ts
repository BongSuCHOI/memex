...
describe("analyze", () => {
  ...
  it("aggregates coverage, projects, facts, and timeline", async () => {
    ...
    const convA = join(
      archiveDir,
      "rollout-2026-05-01-11111111-1111-1111-1111-111111111111.jsonl",
    );
    const convB = join(
      archiveDir,
      "rollout-2026-06-01-22222222-2222-2222-2222-222222222222.jsonl",
    );
    writeFileSync(convA, "{}");
    writeFileSync(convB, "{}");
    ...
    const convC = join(
      testDir,
      "rollout-2026-06-15-33333333-3333-3333-3333-333333333333.jsonl",
    );
    const continuedConv = join(
      testDir,
      "rollout-2026-06-15-44444444-4444-4444-4444-444444444444.jsonl",
    );
    writeFileSync(convC, "{}");
    ...
    // Extraction: sess-1 processed (watermark covers max rowid=2), sess-2 seeded,
    // sess-3 pending. settled 세션은 워터마크가 max(rowid) 를 덮어야 완료로 인정된다
    // (pending 쿼리의 watermark 분기 — FACT-LIFECYCLE 계약).
    db.prepare(
      "INSERT INTO extraction_log (session_id, processed_at, extracted, saved, last_exchange_rowid) VALUES ('sess-1', '2026-06-01T00:00:00Z', 3, 2, 2)",
    ).run();
    db.prepare(
      "INSERT INTO extraction_log (session_id, processed_at, extracted, saved) VALUES ('sess-2', '2026-06-01T00:00:00Z', -1, -1)",
    ).run();

    // Facts: 2 for project A (cwd and basename forms), 1 global, 1 inactive
    db.prepare(
      `INSERT INTO facts VALUES ('f1', 'Fact one', 'decision', 'project', '/tmp/proj-a', 1, 'cat-1')`,
    ).run();
    db.prepare(
      `INSERT INTO facts VALUES ('f2', 'Fact two', 'pattern', 'project', '/tmp/proj-a', 1, 'cat-1')`,
    ).run();
    db.prepare(
      `INSERT INTO facts VALUES ('f3', 'Fact three', 'preference', 'global', NULL, 1, NULL)`,
    ).run();
    db.prepare(
      `INSERT INTO facts VALUES ('f4', 'Old fact', 'decision', 'project', '/tmp/proj-a', 0, NULL)`,
    ).run();

    // Ontology: one domain with one category holding f1/f2
    db.prepare(
      "INSERT INTO ontology_domains VALUES ('dom-1', 'Infrastructure')",
    ).run();
    db.prepare(
      "INSERT INTO ontology_categories VALUES ('cat-1', 'dom-1', 'Databases')",
    ).run();
    ...
    expect(
      report.facts.byCategory.find((c) => c.category === "decision")?.count,
    ).toBe(1);
    expect(report.facts.byScope.find((s) => s.scope === "project")?.count).toBe(
      2,
    );
    expect(report.facts.byScope.find((s) => s.scope === "global")?.count).toBe(
      1,
    );
    ...
    expect(
      report.recommendations.some((r) => r.includes("backfill-extract-worker")),
    ).toBe(true);
    expect(report.recommendations.some((r) => r.includes("memex sync"))).toBe(
      true,
    );
  });
  ...
  it("recommends nothing when coverage is complete", async () => {
    ...
    `).run(conv);
    db.prepare(
      "INSERT INTO extraction_log (session_id, processed_at, extracted, saved, last_exchange_rowid) VALUES ('sess-1', '2026-06-01T00:00:00Z', 1, 1, 1)",
    ).run();
    db.close();
    ...
    expect(report.recommendations).toEqual([
      "Analysis coverage is complete — no backfill needed.",
    ]);
  });
  ...
  it("respects topProjects and timelineMonths options", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      insert.run(
        `e${i}`,
        `proj-${i}`,
        `2026-0${i + 1}-01T00:00:00Z`,
        "q",
        "a",
        join(testDir, `c${i}.jsonl`),
        1,
        2,
        `s${i}`,
      );
    ...
    expect(report.timeline.map((m) => m.month)).toEqual([
      "2026-03",
      "2026-04",
      "2026-05",
    ]);
  ...
  describe("formatAnalysisMarkdown", () => {
    it("renders all sections with data", async () => {
      ...
      db.prepare(
        `INSERT INTO facts VALUES ('f1', 'Fact', 'decision', 'project', 'proj-a', 1, NULL)`,
      ).run();
      db.close();
    ...
    it("escapes pipe characters from DB-sourced values in table cells", async () => {
      ...
      `).run(join(testDir, "x.jsonl"));
      db.prepare(
        `INSERT INTO facts VALUES ('f1', 'Fact', 'cat | injected', 'project', 'p', 1, NULL)`,
      ).run();
      db.prepare(
        "INSERT INTO facts VALUES ('f2', 'Fact2', 'cr' || char(13) || '## injected-heading', 'project', 'p', 1, NULL)",
      ).run();
      db.close();
