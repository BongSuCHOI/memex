...
describe("워커 ↔ dist 계약", () => {
  ...
  it("dist 표가 src 표와 키·필드가 동일하다 (stale dist 스큐 탐지)", async () => {
    ...
    expect(
      Object.keys(dist.FAILURE_REPORT).sort(),
      "분류 키 불일치 → dist 재빌드 필요",
    ).toEqual(Object.keys(src.FAILURE_REPORT).sort());
    for (const k of Object.keys(src.FAILURE_REPORT) as Array<
      keyof typeof src.FAILURE_REPORT
    >) {
      expect(
        dist.FAILURE_REPORT[k],
        `${k} 필드 불일치 → dist 재빌드 필요`,
    ...
    for (const k of Object.keys(src.FAILURE_REPORT) as Array<
      keyof typeof src.FAILURE_REPORT
    >) {
      expect(dist.failureConsumesBudget(k), `${k} 예산 판정 불일치`).toBe(
        src.failureConsumesBudget(k),
      );
    }
  });
});
...
describe("R24: pending 판정은 단일 소스만", () => {
  it("pendingExtractionCoreQuery 외에 NOT EXISTS 사본이 없다", async () => {
    ...
    for (const f of ["src/analyze.ts", "scripts/backfill-extract-worker.js"]) {
      ...
      expect(
        src.includes("FROM extraction_log l"),
        `${f} 에 pending 술어 사본이 있다`,
      ).toBe(false);
      if (src.includes("pending")) {
        expect(src, `${f} 는 단일 소스를 써야 한다`).toContain(
  ...
  it("analyze 의 pending 수치가 워커 선정 결과와 일치한다", async () => {
    const { pendingExtractionCoreQuery, getExtractionConfig } = await import(
      "../src/pending-extraction.js"
    );
    ...
    try {
      ...
      for (const sid of ["fresh", "expired", "done"])
        for (let i = 0; i < 5; i++)
          ins.run(sid, "2026-08-02T00:00:00Z", "/tmp/p");
      // 워터마크는 세션의 max(rowid) 를 덮어야 "완료"다 — rowid 는 테이블 전역이므로
      // 하드코딩 대신 서브쿼리로 계산한다.
      const L = db.prepare(
        "INSERT INTO extraction_log (session_id, processed_at, extracted, saved, claim_owner, last_exchange_rowid) VALUES (?,?,?,?,?,(SELECT COALESCE(MAX(rowid),0) FROM exchanges WHERE session_id = ?))",
      );
      L.run("fresh", new Date().toISOString(), -3, 0, "alive", "fresh"); // 처리 중
      L.run(
        "expired",
      ...
      L.run("done", new Date().toISOString(), 3, 3, null, "done"); // 완료(워터마크 = max rowid)

      const { sql, params } = pendingExtractionCoreQuery(getExtractionConfig());
      const rows = (
        db.prepare(sql).all(...params) as Array<{ sid: string }>
      ).map((r) => r.sid);
      const count = (
