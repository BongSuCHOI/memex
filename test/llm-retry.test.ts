...
/**
 * callMemoryModel 복구 계약 (사용자 피드백: "에러나거나 0바이트인데 재시도·복구가 없다").
 *
 * codex-exec 의 runCodex 를 모킹해 실제 LLM 호출 없이 재시도 의미론만 검증한다:
...
describe('callMemoryModel 재시도/복구', () => {
  it('AC1: 빈 응답을 재시도하고, 재시도가 성공하면 결과를 반환한다', async () => {
    ...
    const out = await callMemoryModel('sys', 'user');
  ...
  it('AC1b: result 메시지 없이 끝난 스트림도 실패로 보고 재시도한다', async () => {
    ...
    expect(await callMemoryModel('sys', 'user')).toBe('recovered');
  ...
  it('AC2c: transient 가 회복되면 재시도로 성공한다', async () => {
    ...
    expect(await callMemoryModel('sys', 'user')).toBe('after-recovery');
    expect(queryCalls).toBe(2);
  });
